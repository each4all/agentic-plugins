...
- Use `$runtime:doctor` to verify whether the installed Codex CLI reports generic `hooks` and the `plugin_hooks` stage (`removed` on current Codex), whether the orchestrator manifest exposes hooks, and whether the installed plugin cache carries the manifest-declared hook file.
