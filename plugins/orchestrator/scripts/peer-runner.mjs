#!/usr/bin/env node
// plugins/orchestrator/scripts/peer-runner.mjs
//
// ADR-0023 PR-D: caller-side peer-runner supervisor mirror for the
// orchestrator plugin. This script owns operational lifecycle state around
// companion dispatch (ledger, status, cancel, sweep, retention) while
// leaving companions/contract.md v0.1.1 unchanged.
//
// Orchestrator preserves its resolve-before-record graceful-degradation
// ordering: companion resolution happens before pending_ensemble
// registration, so missing companions do not create orphan pending rows.
...
// orchestrator-specific PRE-LEDGER failure return (companion missing before
// any handle exists — emitted directly from that early return, since no
// handle write ever happens for it), cancelPeerRun's cancelled finalize, and
...
const SELF_SENSOR_SOURCE = 'peer-runner-orchestrator';
...
export async function emitPeerRunTerminal(args) {
  try {
    ...
    const event = {
      ...
      title: `orchestrator peer run ${status}`,
...
export function validateHandleShape(handle) {
  ...
  if (handle.plugin !== 'orchestrator') {
    return { ok: false, reason: `plugin must be orchestrator: ${handle.plugin}` };
  }
...
async function materializePrompt({ paths, promptText, promptFile, retainPrompt }) {
  ...
  const dir = await mkdtemp(join(tmpdir(), 'orchestrator-peer-runner-prompt-'));
...
export async function runPeer(args) {
  ...
  if (!companionPath) {
    ...
    // return (there is no later handle-side transition to observe).
    await emitPeerRunTerminal({
      repoRoot: options.repoRoot,
      runId,
      status: 'failed',
      errorKind: 'peer_cli_not_found',
