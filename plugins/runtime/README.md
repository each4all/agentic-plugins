...
| `/runtime:consensus plan\|record\|synthesize\|decide\|cancel\|next-round\|execute\|status ...` | shipping | Runtime-owned consensus artifact manager and explicit companion executor. Creates fanout/rebuttal prompts, records or executes raw peer output as files, records owner decisions for unresolved consensus, records artifact-only cancellation for stopped runs, and emits only sanitized execution metadata, synthesized summary, durable disagreements, evidence pointers, artifact paths, owner-decision/cancellation pointers, and `status --latest` guidance from the newest readable manifest. |
