...
- read-only aggregate operator dashboard over persona workflow/peer-run ledgers, macro subtask progress, consensus runs, the snapshot-only arbitrated entry advisory, recorded operator-health evidence, and notify-state health.
...
Migration is explicit and dry-run by default. Two subcommands ship, with
deliberately different mutation boundaries:

```sh
$runtime:migrate workflow-storage [--plugin all|engineer|orchestrator] [--apply]
$runtime:migrate legacy-egress-intents [--root <path>]... [--skip <path>]...
```

`legacy-egress-intents` is READ-ONLY and has no apply mode. It closes the
ADR-0048 residual (d) gap: the egress intent WAL moved from a repo-scoped path
to a machine-global one, and `runtime:doctor` fences only the checkout it runs
in — so a proof the older runtime sent from a *different* checkout is invisible
to it and could still permit a re-send. The guidance it prints is uniform for
every location and its unit is the individual reviewed record, never the
directory: a pre-upgrade record may carry no process identity, and this runtime
reads a missing identity as *unknown* rather than dead, so an older sender may
...
credential-free shim artifact on Claude — egress launcher, and **both** hosts'
permission plans) with per-fragment backup/verify/manual-revert guidance, and presents —
