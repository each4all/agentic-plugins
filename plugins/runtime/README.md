...
- read-only omcc cutover readiness aggregation.
...
| `/runtime:compat snapshot\|check\|ingest-release-notes\|plan ...` | shipping scaffold | Runtime-owned host-version compatibility artifact manager. Records Claude Code / Codex CLI version snapshots, compares them to the remembered host-parity baseline, stores explicit release-note files, URL pointers, or explicitly fetched URL content, and emits compatibility update plans without fetching URLs by default or mutating host state. |
...
| `/runtime:cutover [--format text\|json] [--footer-state <state>] [--omcc-dev-active yes\|no\|unknown]` | shipping scaffold | Read-only omcc cutover readiness report. Aggregates ADR-0012 condition state, scorecard rows, host parity baseline freshness, installed/cache plugin versions, latest compat/consensus/context artifacts, explicit footer evidence, and explicit omcc-dev activity evidence. It can only emit `cutover-ready-candidate`; final cutover remains a user declaration. |
...
1. `plan`: create `<repo>/.agentic-plugins/runs/consensus/<run-id>/manifest.json`, `task.md`, and round-1 peer prompt files. The peer roster may include companion-backed peers (`claude`, `codex`) plus manual/subagent peer labels such as `security` or `release`; manual labels are record-only lanes. Each lane records an explicit role such as `claude_companion_peer` or `security_manual_subagent_peer`.
