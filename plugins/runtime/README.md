...
| `/runtime:settings [--format text\|json] [--target repo\|user\|both] [--model <id>] [--effort <level>] [--claude-model <id>] [--claude-effort <level>] [--codex-model <id>] [--codex-effort <level>] [--apply] [--apply-codex-plugin-hooks] [--execute-plugin-management] [--execute-plugin-cleanup] [--plugin-management-host all\|claude\|codex] [--run-id <settings-run-id>]` | shipping | Dry-run settings planner for marketplace/plugin/CLI readiness and agentic-plugins-owned model/effort config. `--apply` writes only `.agentic-plugins/config.toml`; `--execute-plugin-management` runs only allowlisted host-native plugin install/update/add/upgrade commands; `--execute-plugin-cleanup` runs only doctor-detected retired/unknown `agentic-plugins` Claude plugin cleanup commands; all explicit executors omit raw stdout/stderr and write sanitized execution artifacts under `.agentic-plugins/runs/settings/<run-id>/`. |
...
It invokes commands as argv arrays, never through a shell, and records only status, exit code, byte counts, timing, retry classification, and sanitized error metadata. Raw stdout and stderr are omitted from settings output and artifacts. `--plugin-management-host all|claude|codex` scopes install/update execution. Settings writes `.agentic-plugins/runs/settings/<run-id>/settings.json` plus `.agentic-plugins/runs/settings/latest.json` for explicit plugin-management, plugin-cleanup, or Codex plugin-hook executions; `runtime:doctor` reads the latest artifact and reports failed action types and retryability. Settings still does not write host-native Claude config, auth, secrets, sandbox/permission settings, or execute general plugin uninstall commands.
