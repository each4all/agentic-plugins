...
3. Present the result without hiding host asymmetry. In particular:
   ...
   - Use `Experience Parity` / `experience_parity` when the user asks for current goal progress. Treat its score as observed runtime readiness, not a completion claim for the entire project goal. ⚠ **The denominator changed** (ADR-0056 §Decision 8): `host_compatibility_assurance` was the ninth criterion, carried weight 15, and is removed with the layer — so the criterion count, the total weight, the score, and possibly the headline all moved. That is a scoring change, not a field deletion, and `runtime-experience-parity` bumped for it. One criterion now carries compatibility: `runtime_handoff_artifacts`, which is about whether the recorded artifacts are READABLE for handoff. A `partial` handoff row therefore means a genuine artifact problem — and, since the removal, may also mean a run from an earlier schema era that this runtime does not read as current.
...
- Host-parity baseline **integrity** and **exactness** as two separate facts (ADR-0053 §Decision 3, narrowed by ADR-0056 §Decision 1). Integrity is a hard stop: a missing, unreadable, escaped, or headerless packaged baseline is reported as itself and never as a freshness answer. Exactness is unchanged strict normalized equality between the packaged header and the observed host versions, with the drift direction (`exact` / `ahead` / `behind` / `same-precedence-nonexact` / `mixed-direction` / `unparseable`) recorded beside it as evidence and never consulted by any verdict.

  ⚠ **There is no third fact.** ADR-0053 added *assurance* — a human-authored grant naming an explicit cohort of host-version tuples — and ADR-0056 removed it after measuring that the matcher never once answered `covered` on a real machine: Claude moved 18 versions at a median 2.1 days apart while the review round trip took 10.8 hours, so each grant was correct and stale before or shortly after it shipped. Never report, imply, or await a machine verdict of "this host pair was reviewed". Exactness is **not** relaxed by that removal, and it is **not** promoted into a gate at doctor's level either; it stays in the non-gating `observations` channel that `runtime:cutover` reads, and `overall` is unchanged.

  Historical assurance results in retained doctor artifacts are still decoded, reported under `historical_assurance` with their `schema_era`, and never mapped onto a current status.
