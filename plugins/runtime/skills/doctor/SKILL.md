...
3. Present the result without hiding host asymmetry. In particular:
   ...
   - Use `Experience Parity` / `experience_parity` when the user asks for current goal progress. Treat its score as observed runtime readiness, not a completion claim for the entire project goal. Two criteria carry compatibility, and they answer different questions: `runtime_handoff_artifacts` is about whether the recorded artifacts are READABLE for handoff, while `host_compatibility_assurance` reads the LIVE assurance verdict. An unassured host loses the second and not the first — one fact charged once — so a `partial` handoff row means a genuine artifact problem, not an ungranted host pair.
...
- Host-parity baseline **integrity**, **exactness**, and **assurance** as three separate facts (ADR-0053 §Decision 3). Integrity is a hard stop: a missing, unreadable, escaped, or headerless packaged baseline is reported as itself and never as a freshness answer. Exactness is unchanged strict normalized equality between the packaged header and the observed host versions, with the drift direction (`exact` / `ahead` / `behind` / `same-precedence-nonexact` / `mixed-direction` / `unparseable`) recorded beside it as evidence and never consulted by any verdict. Assurance is `covered` only when a human-authored grant in the packaged record names this exact host pair and every package it binds is installed, enabled, unambiguous, and at the reviewed version on every host the plugin set declares it for; a broken baseline, an unknown record schema, an unreadable host version, a non-authoritative installed-plugin list, an installed runtime below the ADR-0054 §Decision 5 floor on either host, or a corrupt packaged plugin set report `blocked`, and everything else reports `unassured`. Doctor never derives a grant — it evaluates membership in one a reviewer wrote, and no version match, upgrade, or elapsed time produces coverage.

  **The assurance result now GATES** (ADR-0053 §Decision 4). Both readiness paths key on it: the `host_compatibility_assurance` experience-parity criterion reads the live verdict — so a revoked grant or a changed installed package moves parity the moment it happens, rather than when compat is next run — and `runtime:cutover` carries `host_parity_assurance`, a hard `assurance_runtime_floor` evaluated for **both hosts' installed runtime**, and a `latest_compat_snapshot` check that additionally requires collection integrity and an identity binding between the recorded observation and this machine. Exactness is still computed and still reported, in a non-gating `observations` channel; `overall` is deliberately unchanged.
