---
name: context
description: "Runtime-owned ADR-0024 context hygiene artifact scaffold. Use when the user wants a bounded context summary, risk level, artifact pointers, and recommended next-session prompt/action without mutating host session context."
---

# Context (runtime framework primitive)

`runtime:context` is the first ADR-0024 context hygiene scaffold. It writes runtime-owned artifacts and keeps the main session output bounded.
...
3. Present only the returned context summary, risk level, artifact pointers, and recommended next-session prompt/action.
   ...
   - Treat the scaffold as advisory. It does not trim, rewrite, compact, or mutate host session context.
