...
| Skills | Skills are the reusable workflow authoring format. `SKILL.md` is the required skill entry point, and plugins are the distribution unit for reusable skills and apps. | Codex-facing runtime commands stay packaged as skills under `plugins/runtime/core/skills/`. Runtime docs should mention `$runtime:*` skill invocation rather than Claude slash-command parity. |
