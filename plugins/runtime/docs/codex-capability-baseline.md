...
- `~/.codex/config.toml` `[hooks.state."<plugin>@<marketplace>:<hooks-path>:<event>:0:0"]`
  ...
  is written as a `trusted_hash` line and **no `enabled` key**. The interactive
  `/hooks` view renders `Event` / `Matcher` / `Source` / `Command` / `Timeout` /
  `Trust` and exposes **no enable/disable toggle**, so an absent `enabled` key is
  the only reachable state for a newly-trusted plugin hook, and it means
  **enabled**. `enabled = true` appears only in configs written by an earlier
  Codex. `enabled = false` is written on an explicit opt-out. Verified by
  behavior, not inference: `designer`'s `Stop` hook — whose entry carried a
  `trusted_hash` and no `enabled` key — executed during a `codex exec` turn and
  archived a terminal designer workflow.
