...
Observed on 2026-05-16 with Claude Code `2.1.143`, Codex CLI
...
| Hooks | Claude hooks are configured in settings, plugin `hooks/hooks.json`, and skill/subagent frontmatter. They support rich lifecycle events, including `SessionStart`, `Stop`, `SubagentStop`, `PreCompact`, `PostCompact`, task/team-related events, worktree events, and JSON decision control. Plugin hooks use `${CLAUDE_PLUGIN_ROOT}` and `${CLAUDE_PLUGIN_DATA}` for bundled scripts and persistent plugin data. Claude Code 2.1.143 caps repeated Stop-hook blocking at 8 by default, with an environment override. | Codex hooks are documented, and bundled plugin hooks are supported behind the `plugin_hooks` feature flag. Local `codex features list` reports generic `hooks` stable/enabled and reports `plugin_hooks` with stage `under development`; its effective value depends on config or `--enable plugin_hooks`. Plugin hook commands receive `PLUGIN_ROOT`/`PLUGIN_DATA` and Codex also sets `CLAUDE_PLUGIN_ROOT`/`CLAUDE_PLUGIN_DATA` as compatibility aliases. Local non-interactive help does not expose a `codex hooks` trust/query command. | Runtime must diagnose generic hooks, plugin_hooks enablement, manifest hook exposure, installed hook packaging, hook command portability, and hook trust/review separately. `/hooks` `Installed` counts are packaging evidence only; `Active=0` output and `Trust: New hook - review required` are not enough to attest. Runtime may surface enablement plans, can explicitly write `~/.codex/config.toml` `[features].plugin_hooks = true` only when `--apply-codex-plugin-hooks` is requested, and can record an operator attestation with `--attest-codex-hook-review` after the active Codex `/hooks` review/trust step. `CLAUDE_PLUGIN_ROOT` inside a Codex-exposed hook command is compatibility telemetry, not a portability warning by itself; `adapters/claude/hooks` paths still are warnings. The attestation is not host-native proof and is valid only while the hook-bearing plugin set and source versions still match. Runtime should not assume a portable Stop-hook block cap exists across hosts; if it adds hook retry policy, it must remain explicit and host-observed. |
...
| Settings/config | Claude uses `~/.claude/settings.json`, `.claude/settings.json`, `.claude/settings.local.json`, and managed policy files. Plugin enablement can live under `enabledPlugins` settings. Claude Code 2.1.142/2.1.143 background-session dispatch can propagate settings, MCP config, plugin dirs, strict MCP config, and additional dirs. | Codex uses `~/.codex/config.toml`, trusted project `.codex/config.toml`, system config, profiles, and CLI `-c` overrides. | Runtime `settings --apply` writes only `.agentic-plugins/config.toml`; the narrow `settings --apply-codex-plugin-hooks` executor may write only `~/.codex/config.toml` `[features].plugin_hooks = true`. Host-native `.claude` config and broader `.codex` config mutation remain out of scope. Runtime should observe Claude dispatch flags but not use them to justify hidden host-native settings mutation. |
