...
Observed on 2026-06-07 with Claude Code `2.1.168`, Codex CLI
`0.137.0`, official OpenAI Codex developer docs, and official Claude Code docs.
...
- `codex plugin --help` (0.137.0: `add`, `list`, `marketplace`, `remove` —
  per-plugin `add`/`list`/`remove` are new beyond the prior marketplace-only
  surface)
- `codex plugin marketplace --help` (0.137.0: `add`, `list`, `upgrade`, `remove`)
- `codex features list` (0.137.0: `plugin_hooks` reported `removed`, generic
...
| Plugin distribution | `claude plugin` exposes per-plugin `install`, `list`, `update`, `enable`, `disable`, `uninstall`, `details`, `validate`, and marketplace management. In Claude Code 2.1.143, enable/disable also enforces plugin dependencies, `details` reports component inventory plus projected token cost, and `prune`/`autoremove` can remove unused auto-installed dependencies. | Official Codex plugins bundle skills, apps, and MCP servers. Local non-interactive CLI in `0.137.0` exposes per-plugin `codex plugin add` (install from a configured marketplace snapshot), `list`, and `remove`, plus `codex plugin marketplace add`/`list`/`upgrade`/`remove`; interactive `/plugins` owns browse/install/toggle UX. It still has no per-plugin `update`, `enable`, `disable`, `details`, `validate`, or `prune`. | Runtime settings may execute allowlisted Claude plugin commands and Codex marketplace commands. As of Codex `0.137.0` the CLI does expose per-plugin `add`/`list`/`remove` (marketplace-snapshot-sourced), so runtime must not call the surface marketplace-only; it must also not claim full Claude-style parity (no per-plugin `update`/`enable`/`disable`/`details`/`validate`/`prune`). Runtime `doctor`/`settings` now recognize the per-plugin surface as `per-plugin-and-marketplace`, keyed on the observed `codex plugin --help` (ADR-0032); wiring `codex plugin add` as an executable cache-materialization action remains a deferred follow-up (see `follow-ups.md`). Runtime cleanup must distinguish retired/unknown user-requested cleanup from Claude's dependency-aware plugin enable/disable/prune behavior. |
...
- A Claude plugin command plan that says `codex plugin install runtime` is still
  wrong for local Codex CLI `0.137.0` — the per-plugin install verb is `codex
...
- A Codex hook plan that treats a `[features].plugin_hooks` value or marketplace
  cache metadata as proof of hook trust is overstated — and on 0.137.0
  `plugin_hooks` is still a removed flag, so its value is meaningless. Local CLI
  `0.137.0` does not expose a non-interactive hook trust query; `/hooks` remains
