...
`claude mcp --help` re-checked live on the fix/perf-only `2.1.227` patch
...
- `claude --help` (the global `--safe-mode` flag / `CLAUDE_CODE_SAFE_MODE`
  ...
  and MCP servers disabled for troubleshooting — is still present on 2.1.227)
- `claude plugin --help` (re-checked live on 2.1.227, verb set unchanged from
...
- `claude agents --help` (re-checked live on 2.1.227: `--json` with `--all`
  ...
  does not use)
- `claude mcp --help` (re-checked live on 2.1.227: `add`,
