...
`0.150.1`, official OpenAI Codex developer docs, and official Claude Code docs.
...
2026-08-28: dual drift Claude `2.1.233`→`2.1.250` / Codex
`0.147.0`→`0.150.1` — a **full re-run of this evidence block**, not a spot-check,
because the Codex move crosses three minors (`0.148.0`, `0.149.0`/`0.149.1`,
`0.150.0`/`0.150.1`) and 0.148.0 rewrote plugin skill loading. `claude
--version`, `claude --help`, `claude plugin --help`, `claude agents --help`,
`claude mcp --help`, `codex --version`, `codex --help`, `codex plugin --help`,
`codex plugin marketplace --help`, `codex plugin list --json`, `codex features
list`, and `~/.codex/config.toml` `[hooks.state]` were re-run live. **The
enumerated surfaces are the scope**: this re-run did not exercise skill
invocation, subagent behavior, sandboxing, approvals, team mode, or worktrees,
and the rows below that rest on those stand from their own dated observations.
Within that scope Claude's **plugin and MCP** verb sets and Codex's **plugin
and plugin-marketplace** verb sets are unchanged, and `--safe-mode` is still
present, while Codex's *top-level* help gains three verbs. `codex mcp --help`
is not in this evidence block and was not run. One recorded claim was falsified — see the `[hooks.state]` bullet — and
the 0.148.0 half of this window was already reviewed once under the
now-removed assurance path, whose surviving open items (`R13`, `R14`, `R16` in
[`follow-ups.md`](follow-ups.md)) this refresh does **not** close):
...
- `claude --help` (the global `--safe-mode` flag / `CLAUDE_CODE_SAFE_MODE`
  ...
  and MCP servers disabled for troubleshooting — is still present on 2.1.250.
  New in the 2.1.234–2.1.250 window and observed here: a global `--restricted`
  / `CLAUDE_CODE_RESTRICTED=1` flag that drops the command- and code-running
  built-ins plus `WebFetch`, keeps file tools inside the working directory,
  refuses `bypassPermissions`, and ignores user/project/local settings files.
  The two restrict **different axes** and neither is a version of the other.
  `--safe-mode` names what it disables — CLAUDE.md, skills, plugins, hooks, MCP
  servers, commands, agents, output styles, workflows, themes, keybindings —
  and states that built-in tools and permissions still work normally.
  `--restricted` names tool removal and ignored settings files and says
  **nothing about plugins or skills**, so their availability under it is
  unprobed and is not asserted here. That silence is not a reason to treat the
  flag as irrelevant: this repository's skills run Node scripts through the
  execution tools `--restricted` removes, so a restricted session is a
  plausible way for runtime commands to stop working for reasons a plugin
  diagnosis would not explain. Runtime sets neither, and whether the two
  compose was not probed)
- `claude plugin --help` (re-checked live on 2.1.250, verb set unchanged from
...
- `claude agents --help` (re-checked live on 2.1.250: `--json` with `--all`
  ...
  does not use. This re-run also repairs three long-standing omissions in the
  list above rather than recording new flags: `--permission-mode`,
  `--plugin-dir`, and `--dangerously-skip-permissions` have been on `claude
  agents` since 2.1.142/2.1.143 and were simply never written down here. The
  only genuinely new option in this drift window is `--restricted`, the
  dispatch-session counterpart of the global flag above. The command remains
  options-only with no subcommands)
- `claude mcp --help` (re-checked live on 2.1.250: `add`,
  `add-from-claude-desktop`, `add-json`, `get`, `list`, `login`, `logout`,
  `remove`, `reset-project-choices`, `serve` — verb set unchanged from the
  2.1.206 observations and unchanged again at 2.1.250; `login`/`logout` cover
  MCP OAuth flows)
- `codex --version` -> `codex-cli 0.150.1`
- `codex --help` (re-checked live on 0.150.1: top-level surface `exec`, `review`,
  ...
  `cloud`, `exec-server`, `features` — the previously recorded enumeration,
  first taken at 0.144.1 and every entry still present at 0.150.1; `delete` is
  ...
  0.147.0 passed in both directions. The top-level surface is **not**
  unchanged at 0.150.1: three verbs are additive — `agents` (the 0.149.0
  interactive task dashboard), `queue` (send a message to an existing local or
  remote session, 0.149.0), and `migrate-rollouts` — none of which runtime
  calls. `exec` remains the only Codex verb the companion contract depends on,
  and the deep-peer smoke of `doctor-20260828T035114Z-44ccdf` passed in both
  directions on this pair)
- `codex plugin --help` (re-checked live on 0.150.1: `add`, `list`, `marketplace`,
...
- `codex plugin list --json` (re-checked live on 0.150.1: root object is
...
- `codex plugin marketplace --help` (re-checked live on 0.150.1: `add`, `list`,
...
- `codex features list` (re-checked live on 0.150.1, **124 rows**. The stage
  vocabulary is wider than the three stages this file used to name: alongside
  `stable`, `under development`, and `removed` the inventory also carries
  `experimental` and `deprecated`. Counting only the first three returns 119
  and omitting digit-bearing flag names returns 116 — both were produced here
  before the full list was read, and the count is recorded at 124 so a future
  reader can tell a real inventory change from a narrower reader.
  `plugin_hooks` still reported `removed`, generic `hooks` stable/true,
  ...
  there. `recommended_plugins` stays `stable`/false. Observed additions across
  0.148.0–0.150.1 are mostly skill-side — `skill_search` and
  `skill_mcp_dependency_install` stable/true, `skill_env_var_dependency_prompt`
  `removed` — but not exclusively: `compaction_image_budget` stable/true
  arrives with 0.150.1's remote-compaction image budget. **A flag inventory
  reports names, stages, and enabled state and nothing else**, so no claim is
  made here about whether any of them can affect plugin packaging or hook
  loading; that would need source evidence or a controlled toggle test.
  There is still **no** `plugin_commands` row at any stage — the
  command-migration path described in the ADR-0013 Trigger Watch is not a flag
  awaiting enablement)
- `~/.codex/config.toml` `[hooks.state]` (read live on 0.150.1. **This bullet
  previously said all four hook-bearing plugins carry `enabled = true`; that is
  false and was corrected here on measurement.** Engineer, orchestrator, and
  founder carry `enabled = true` plus a `trusted_hash`; **designer carries the
  `trusted_hash` alone, with no `enabled` key at all**. What this refresh read
  is the file; **no hook was executed to see whether it fires**, so no
  behavioral claim is made here. Runtime currently classifies all twelve
  entries as enabled — `doctor-20260828T035114Z-44ccdf` reports `expected=12`,
  `enabled=12`, `disabled=0` — but that is runtime applying its own
  absent-`enabled` interpretation, not independent evidence that Codex agrees.
  The correction is therefore narrow: the previous sentence's `enabled = true`
  is not what the file says for designer. The asymmetry itself is **not** a new
  discovery and should not be re-derived — the "Hook-state `enabled` semantics"
  paragraph in [`codex-capability-baseline.md`](codex-capability-baseline.md)
  already records why, from the 2026-07-09 fix that established it: designer
  was the first hook-bearing plugin trusted after the ADR-0035 §6 host-config
  writer was removed, so Codex wrote it a bare `trusted_hash`, while the
  `enabled = true` lines on engineer, founder, and orchestrator are residue
  from an earlier Codex. What is new here is only that this bullet had gone on
  asserting the residue as the current shape for all four. What the absent key
  *means* to Codex remains a live-probe question, and the one behavioral
  observation this repository holds was made on `0.142.5`, before 0.148.0's
  hook-execution changes.
  `[hooks.state]` itself is an empty table whose
  content lives in the quoted sub-tables
  (`[hooks.state."<plugin>@<marketplace>:<hooks.json path>:<event>:0:0"]`), so
  a reader that stops at the next `[` sees nothing and would report zero
  trusted hooks — which is how this bullet's previous claim survived. The
  per-event hash is identical across the four
...
| Permissions and sandbox | Claude's permission modes are `default`, `acceptEdits`, `plan`, `bypassPermissions`, `dontAsk` ("don't prompt; deny if not pre-approved") and `auto` ("use a model classifier to approve/deny permission prompts") — the complete canonical list, read out of the `2.1.248` binary rather than from docs, which is why it is stated as an enumeration and not as "such as". The last two are the reason this row was corrected by ADR-0057: `auto` is a per-call decision mode, not a list to accrete against, and it is what made the ADR-0038 permission advisor's premise obsolete — a baseline that listed every mode except the decisive one is how that went unnoticed. Teammates inherit the lead's permissions at spawn. The 2.1.228–2.1.233 window is a worked example of why this file records an **observed** version rather than a recent one: 2.1.232 permission-checked Bash input redirections (`< file`) on all platforms and required approval for writes through Windows Cygwin-style symlinks, and 2.1.233 **reverted both** ("a narrower version will return in a later release") after they regressed ordinary `cd <dir> && <command> > file` approvals on Windows. A baseline refreshed to 2.1.232 — the version this loop originally targeted — would have recorded two permission behaviors that no longer exist. | Codex separates sandbox mode and approval policy. Defaults include workspace-write with network off; subagents inherit the current sandbox policy and non-interactive approval gaps fail back to the parent workflow. | Runtime must classify permission/sandbox/auth preconditions as operator action. Settings must not silently relax host permissions, sandbox, approval, or network policy for companions or subagents. |
...
**Partial re-check 2026-08-28 on Codex `0.150.1` — two signals only; the
verdict was NOT reassessed.** Deliberately not titled an amendment to the
assessment above, because it does not carry one. Two of the facts the trigger
turns on were re-read on the installed host and both remain absent: `codex
plugin --help` is still `add`/`list`/`marketplace`/`remove`, and the full
124-row `codex features list` carries no `plugin_commands` entry at any stage.
The assessment above turns on more than those two — the embedded
`plugin-json-spec.md` field set, the `plugin-creator` scaffolder flags, the
official plugin documentation, and the `migrated-command-skills` artifact check
(that last being the undocumented migration path this very trigger discovered
at 0.147.0, i.e. exactly the class a two-signal check cannot see). **None of
those was re-run**, so the 2026-08-08 `NOT FIRED` verdict stands as a
2026-08-08 verdict and not as a `0.150.1` one. What the partial check does
establish is worth recording on its own terms: after three Codex minor
releases the two most visible registration signals are still absent **at the
checked `0.150.1` endpoint** — the intervening releases were not each probed —
in a window where the
plugin subsystem itself moved substantially (0.148.0 rewired plugin skill
loading through the host skills service; 0.149.0 removed the workspace-settings
gate for apps and plugins). A future assessment that needs a current verdict
must re-run the full evidence set against the binary rather than extend this
note.
...
| 2026-08-28 | `2.1.250` | `0.150.1` | Re-observed as a full re-run **of this file's evidence block** during the ADR-0026 compat loop that opened when the previous run projected `legacy_era` (compat run `compat-20260828T055652Z-fde36e`; content-backed ingest of the Claude changelog and the Codex releases atom feed, supplemented with `gh release view` bodies for the stable releases the feed window omits - of the feed's ten entries only `0.150.1` carried a real body, the other nine being alpha stubs). Dual drift: Claude patch `2.1.234`-`2.1.250`, of which 14 of the 17 positions publish a changelog entry (`2.1.242`, `2.1.244`, and `2.1.249` publish none, as `2.1.230` did not in the preceding window); Codex three minors `0.148.0`/`0.149.x`/`0.150.x`. **Scope is the enumerated evidence block** - skill invocation, subagent behavior, sandboxing, approvals, team mode, and worktrees were not exercised, and the rows resting on those stand from their own dated observations. Within that scope Claude's plugin and MCP verb sets and Codex's plugin and plugin-marketplace verb sets are unchanged - `claude plugin` 13, `claude mcp` 10, `codex plugin` 4, `codex plugin marketplace` 4, each count excluding that command's built-in `help`; `codex mcp --help` is not part of this evidence block and was not run - and `--safe-mode` is still present; Codex's **top-level** help is *not* unchanged, gaining `agents`, `queue`, and `migrate-rollouts`. `claude agents` gains `--restricted`; the three options recorded alongside it (`--permission-mode`, `--plugin-dir`, `--dangerously-skip-permissions`) are **omissions repaired, not additions** - they have been on the host since 2.1.142/2.1.143. `codex plugin list --json` keeps a nine-key installed entry plus an optional `marketplaceSource`, measured here as ten keys on the eight git-sourced plugins and nine on the locally-sourced `github` plugin. `codex features list` keeps `plugin_hooks` `removed` and generic `hooks` `stable`/true across 124 rows spanning five stages. The ADR-0013 trigger was **not** reassessed: two of its signals were re-checked and remain absent, the rest were not re-run, so the 2026-08-08 verdict stays a 2026-08-08 verdict. **Unlike the two preceding rows this one corrects a recorded claim rather than only extending one**: the `[hooks.state]` bullet asserted that all four hook-bearing plugins carry `enabled = true`, and designer carries a `trusted_hash` with no `enabled` key - an asymmetry the 2026-07-09 hook-`enabled` fix already explained (designer was trusted after the ADR-0035 §6 writer removal and got a bare `trusted_hash`; the other three carry residue from an earlier Codex), so what this row corrects is the bullet having gone on asserting that residue as the current shape for all four. This refresh read the file and executed no hook, so it makes no behavioral claim: runtime classifies all twelve entries as enabled (`doctor-20260828T035114Z-44ccdf`: `expected=12`, `enabled=12`, `disabled=0`), which is runtime applying its own absent-`enabled` interpretation rather than evidence that Codex agrees. Two Codex capabilities are recorded without adoption - `Interrupt` hooks (0.150.0) and asynchronous / MCP-invoking hook commands (0.148.0) - and whether the three existing handlers behave identically under the new execution model was not probed. The 0.148.0 half of this window was already reviewed once under the now-removed assurance path; its surviving open items `R13`, `R14`, and `R16` in [`follow-ups.md`](follow-ups.md) are **not** closed here. |
