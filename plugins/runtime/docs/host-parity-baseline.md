...
| Hooks | Claude hooks are configured in settings and support rich lifecycle events, including `SessionStart`, `Stop`, `SubagentStop`, `PreCompact`, task/team-related events, and JSON decision control. Some hook stdout can be injected into Claude context. | Codex hooks are documented, but local `codex features list` reports generic `hooks` stable/enabled and `plugin_hooks` under development/disabled. Codex hook configuration is host config, not proven plugin-local packaging here. | Runtime can diagnose hook readiness and document gaps. It must not use Codex plugin hooks for automatic SessionStart/Stop parity until official docs and local CLI evidence show plugin-hook support. |
...
| Settings/config | Claude uses `~/.claude/settings.json`, `.claude/settings.json`, `.claude/settings.local.json`, and managed policy files. Plugin enablement can live under `enabledPlugins` settings. | Codex uses `~/.codex/config.toml`, trusted project `.codex/config.toml`, system config, profiles, and CLI `-c` overrides. | Runtime `settings --apply` writes only `.agentic-plugins/config.toml`; host-native `.claude` and `.codex` config mutation must remain an explicit future executor. |
...
- A Claude SessionStart/Stop hook design cannot be copied directly into Codex
  plugin packaging while local `plugin_hooks` is disabled. Codex generic hooks
  and Codex plugin hooks are different readiness questions.
...
- runtime starts writing host-native `.claude` or `.codex` config, packaging
  hooks, or launching host-native subagent/team workflows automatically.
