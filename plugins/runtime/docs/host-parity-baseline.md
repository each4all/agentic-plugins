...
- A Codex hook plan that treats `plugin_hooks=true` or marketplace cache metadata
  ...
  `/hooks` `Installed` counts are packaging evidence only; `Active=0` output is
  not enough to attest.
