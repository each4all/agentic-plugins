---
description: Runtime consensus scaffold and explicit companion executor for ADR-0024 peer fanout, disagreement tracking, synthesis, owner decisions, converged-run owner ratification, and artifact-only cancellation
argument-hint: "plan|record|synthesize|decide|ratify|cancel|next-round|execute|status [--format text|json] [--task <text>|--task-file <path>] [--run-id <id>|--latest|--latest-open] [--peer <id>] [--peers <ids>] [--input-file <path>] [--summary-file <path>] [--decision-file <path>] [--ratification <text>|--ratification-file <path>] [--ratified-by <id>] [--lever <text>] [--reason <text>|--reason-file <path>] [--confirm-no-active-process] [--disagreements-file <path>] [--contradictions-file <path>] [--convergence-state <state>] [--max-rounds <n>] [--max-peers <n>] [--token-budget <n>] [--time-budget-ms <n>] [--process-budget <n>] [--timeout-ms <n>] [--execute]"
---
...
/runtime:consensus ratify --run-id <id> --ratification-file <owner-ratification.md> --ratified-by owner --lever "fs-scoping timing: wait for a trigger"
...
- `ratify` is the converged-run mirror of `decide`: when a run converged (`aligned`/`complementary`) but the synthesis flagged a residual owner sub-lever (an "owner's call" point preserved as a durable disagreement that does not rise to non-consensus), `ratify` records the owner's resolution of that lever as `owner-ratification.md` plus `owner-ratification.json` with an optional single-line `--lever` summary. A plain ratification of an `aligned` run without any residual lever is also allowed — the lever is the motivating case, not a requirement. It refuses unresolved runs ("use decide"), already-ratified runs, owner-decided runs, and cancelled runs. It keeps the manifest status `converged`, never rewrites `consensus.json` or `convergence_state`, and does not print the ratification text into status or footer output. The `--lever` summary IS printed by status output as displayed metadata (like `decided_by`/`cancelled_by`) — keep sensitive owner-resolution detail in the ratification body, which stays pointer-only.
- Terminal artifacts gate the mutators: once a run has a cancellation, owner-decision, or owner-ratification artifact, `record`, `synthesize`, `next-round`, and `execute` refuse it ("preserve it and start a new consensus run"), so a recorded owner resolution can never end up attached to evidence or synthesis it never covered. `decide` and `ratify` each refuse runs carrying any of the three terminal artifacts (including their own — re-deciding or re-ratifying needs a new consensus run), and `cancel` refuses owner-decided and ratified runs; all gates key on the artifact pointers, not the manifest status, so a status-drifted manifest stays protected.
- When a run is unresolved, `status` prints an `owner_decision_briefing` ("Owner decision required:" in text) that guides `decide`: the synthesized durable disagreements, the exact `decide` command, and the `owner-decision.md` template sections (Context, Open Question, Considered Options, Decision, Rationale, Rollback). The briefing is derived from synthesized disagreements only and never includes raw peer output.
- When a run converged with durable disagreements still preserved (e.g. a `complementary` run carrying a residual owner lever) and no ratification exists, `status` prints an advisory `owner_ratification_briefing` ("Owner ratification available (optional):" in text): the synthesized durable disagreements, the exact `ratify` command, and the `owner-ratification.md` template sections (Context, Ratified Consensus, Residual Owner Lever, Owner Resolution, Rationale). Recording a ratification is optional — converged runs without owner levers need no follow-up.
