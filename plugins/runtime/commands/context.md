---
description: Runtime-owned context hygiene artifact scaffold, read-only budget check, ADR-0044 session-capture note staging, hook-fired slot publication, and validated slot inspection
argument-hint: "capture|status|check|note|publish-session [--format text|json] [--summary <text>|--summary-file <path>] [--risk green|yellow|red] [--token-budget <n>] [--used-tokens <n>|--remaining-tokens <n>] [--artifact kind:<repo-path>] [--next-action <text>] [--next-session-prompt <text>|--next-session-prompt-file <path>] [--run-id <id>|--latest|--slot] [--stale-after-hours <n>] [--text <text>|--file <path>|--clear] [--host claude|codex] [--session-id <id>] [--workflow-evidence none|fresh]"
---
