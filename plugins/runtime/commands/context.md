...
argument-hint: "capture|status|check [--format text|json] [--summary <text>|--summary-file <path>] [--risk green|yellow|red] [--token-budget <n>] [--used-tokens <n>|--remaining-tokens <n>] [--artifact kind:<repo-path>] [--next-action <text>] [--next-session-prompt <text>|--next-session-prompt-file <path>] [--run-id <id>]"
---
...
Run the runtime-owned context hygiene artifact scaffold and read-only budget check. This command does not trim, rewrite, or mutate host session context. It also does not compact host session context. `capture` writes a bounded repo-local artifact; `status` reads one; `check` only computes an advisory green/yellow/red risk from caller-supplied inputs.
