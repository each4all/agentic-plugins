...
- `--format json` emits the machine-readable report (`runtime-dashboard-1.3`; 1.0 + the additive snapshot-only `tier1.entry_advisory`, + the snapshot-only `tier2.retention` projection, + the `tier2.assurance` compatibility-assurance facts and `tier2.doctor.latest.assurance`). This token was stale at `1.1` across the 1.2 bump; it is corrected here rather than bumped by one.
