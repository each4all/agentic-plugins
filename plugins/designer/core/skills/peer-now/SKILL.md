---
name: peer-now
description: "Dispatches a verbatim prompt at the cross-host companion (claude or codex) outside a verb skill's normal ensemble flow — the designer plugin's peer-now meta skill (ADR-0022 meta-skill category, ADR-0042 SD7). A side-channel meta operation, not a cognitive verb and not a lifecycle macro. Use for a quick ad-hoc cross-host design consultation without opening a full ensemble dispatch. Trigger phrases include 'ask the peer', 'consult codex', 'consult claude', 'peer consultation', '피어 자문', '크로스 호스트 질의'. Symmetric on both sides — Claude can ask Codex; Codex can ask Claude; companions/ ships both bridges as first-party (ADR-0001 §COMPANION bidirectional design). The privacy gate applies: genericize before dispatch, and the peer path carries no image — screenshots never leave the local host as bytes."
---

# Peer-now (designer persona, meta skill)
...
field is reserved for verb-skill structured ensemble verdicts.

The exclusion is **structural, not conventional**. Two mechanisms hold it:

1. `peer-runner.mjs` registers a `pending_ensemble` row only when
   `kind === 'ensemble'` (`scripts/peer-runner.mjs`, the
   `handle.kind !== 'ensemble'` early return). `--kind peer-now` therefore
   cannot touch `pending_ensemble` or `ensemble_results` even by accident.
2. peer-now omits the three ensemble-accounting flags —
   `--workflow-path` / `--phase` / `--ensemble-type` — so there is nothing
   to account against. It **does** pass `--run-id`: that flag is the
   peer-run **ledger** key (it names the
   `.agentic-plugins/state/designer/peer-runs/<run_id>/` directory and lets
   `peer-runner.mjs status/cancel` find the run), not an ensemble key.

See `../_shared/references/ensemble-protocol.md` § State Bookkeeping.
...
**verbatim** prompt to the peer host — that is an external transmission.
Before dispatch:

- **Genericize** the prompt the same way you would for a WebSearch query —
  no unreleased feature or flow names, no customer identifiers, no internal
  product names, no secrets/tokens/endpoint URLs pasted from frontend code.
  The pre-genericization value MUST never leave the local host.
- **Screenshots are sensitive by default**, and the companion has no image
  channel: `codex-companion` exposes no `--image` flag, so a screenshot is
  never sent as bytes. Describe the screen in genericized terms, or — only
  when the user has cleared it — pass a **verified-local absolute file
...
See `../investigate/references/design-brief-spec.md` § Privacy Gate for
...
| `peer-runner.mjs run --kind peer-now --peer codex` (Claude asking Codex) | Native — `codex-companion.mjs` invoked and tracked under `.agentic-plugins/state/designer/peer-runs/<run_id>/` | N/A (Codex would not ask itself) |
| `peer-runner.mjs run --kind peer-now --peer claude` (Codex asking Claude) | N/A (Claude would not ask itself) | Native — `claude-companion.mjs` invoked and tracked under `.agentic-plugins/state/designer/peer-runs/<run_id>/` |
| **Image / screenshot in the prompt** | Not available — no `--image` on the companion path | Not available — same. Vision is host-direct only (Claude natively; `codex exec --image` on the active host) |
...
exist or is not readable. There is no `--image` flag to reject — the
companion path has no image channel at all.
...
dispatch contract. `peer-now` is for casual cross-host design probes where
the user wants the raw answer.

Treat the peer's answer as a **code/text opinion**. It never saw the screen.
An assertion about contrast, spacing, or visual hierarchy in a peer-now
response is an INFERENCE, not an observation — confirm it with a host-direct
look before acting on it.
...
- **Trying to send a screenshot.** The companion path has no image channel.
  Describe the screen, or pass a verified-local absolute file path the peer
  reads on its own host after the user clears it; never inline image bytes.
...
- **Mutating `current_phase` or `next_action`.** peer-now is a side-channel.
  Phase progression belongs to verb skills or `designer:start`.
