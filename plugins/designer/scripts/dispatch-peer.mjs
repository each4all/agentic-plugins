#!/usr/bin/env node
// plugins/designer/scripts/dispatch-peer.mjs
//
// Host-shared canonical companion dispatch wrapper for the designer plugin.
//
// Responsibilities:
 ...
 * AND the designer ensemble-protocol § Required blocks (which adds
 ...
 *   <structured_output_contract>. Required by the designer ensemble
 ...
 *   (optional companion-contract field; the designer protocol prefers
...
export async function dispatchPeer({
  ...
  // response, and `/designer:resume` Step 5b will scrub orphaned pending
  ...
  if (promptText !== undefined) {
    const dir = await mkdtemp(join(tmpdir(), 'designer-prompt-'));
    cleanupTmpDir = dir;
