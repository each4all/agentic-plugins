...
// ADR-0031 — designer session-handoff projection + ADR-0043 S4 activation
...
//
// Designer (L3) computes its OWN bounded workflow projection from its own
...
// runtime never shell-reads designer state; the dependency direction stays
...
// Default routing recommendation for an active designer workflow: resume it.
const DEFAULT_ROUTING = '/designer:resume';
 ...
 * + parsed content). Shared by the branch-resolved `computeDesignerProjection`
 * and the path-targeted `computeDesignerProjectionForPath`. The runtime seam
  ...
  for (const field of ['workflow_id', 'current_phase', 'next_action']) {
    ...
    if (typeof value !== 'string' || value.trim() === '') {
      return {
        projection: null,
        status: 'fail_closed',
        error: `designer workflow missing required field: ${field}`,
...
/**
 * Compute the designer bounded workflow projection for a branch.
 *
 ...
 */
export async function computeDesignerProjection({
  repoRoot,
  branch,
  routing,
  headSha,
 ...
 */
export async function computeDesignerProjectionForPath({
  repoRoot,
  workflowPath,
  routing,
  headSha,
...
/**
 * Default projection-file path: `<designer state root>/last-session-handoff.json`.
 * designer trim — canonical home only (ADR-0042 SD7: no legacy dual-home), so
...
/**
 * The candidate one-shot files the SessionStart backstop reads. designer trim —
...
// the attention follow-up; regression-pinned by tests/designer/test-footer-activation.mjs):
//   filename : `${projectionFile}.footer-rendered` (the engineer slot shape —
//              designer shares engineer's single-projection-slot structure)
...
//
// TOMBSTONE (designer divergence from the engineer copy, Codex Plan-verify
// blocker): a 'rendered' marker SURVIVES `consumePendingHandoff`. designer's
...
// manually-published lifecycles): designer terminalizes WITHOUT auto-committing
// — the owner publishes the deliverable manually — so designer does NOT copy
// engineer's blocked / next-work-available dichotomy. The per-gate rule from
// designer's own evaluator (evaluateStopArchive: terminal_marker /
...
const BLOCKED_GATE_NEXT_ACTIONS = {
  ...
  head_moved: 'Save/commit the design deliverable so HEAD moves past the workflow baseline — designer never auto-commits; publishing is the owner\'s manual step (a failed git probe also reports this gate).',
  no_active_children: 'Settle the incomplete child-completion entries recorded on this workflow before archiving it (designer workflows normally carry none — an unexpected entry indicates external state mutation).',
...
export function mapCompletionFlags(projection, gateFailures = []) {
  ...
  const reason = publishNeeded
    ? `Workflow at terminal phase ${phase} awaits the owner's manual publish: the only unmet archive gate is head_moved (HEAD has not moved past the baseline, or the git probe failed — designer never auto-commits).`
    : gate === 'blocked'
...
// (footer.mjs is L1 runtime; designer L3 cannot import it — ADR-0010 §5) and
...
/**
 * ADR-0031 activation sidecar (ADR-0043 S4) — fired from the must-run designer
 ...
 * terminalized (by path, via `computeDesignerProjectionForPath` — not by current
 ...
 * designer footer floor is the ADR-0043 §4 S2-containing release, see
...
export async function emitTerminalHandoffSidecar({ repoRoot, workflowPath, projectionFile, host, origin = 'backstop' } = {}) {
  ...
  try {
    ...
    const result = await computeDesignerProjectionForPath({ repoRoot, workflowPath });
...
// ADR-0031 hook backstop (designer-hook-backstop) — the SessionStart / Stop
...
export async function readPendingHandoff(repoRoot, projectionFile) {
  ...
  for (const target of candidates) {
    try {
      ...
      /* candidate unreadable — fail-closed (designer has a single canonical home) */
    }
  }
  return null;
}
...
/**
 * Build the bounded `[designer-handoff-pending]` re-injection line for a pending
...
export async function pendingHandoffReinjectionLine(repoRoot, projectionFile) {
  ...
  return {
    line: `[designer-handoff-pending] ${JSON.stringify(summary)} [/designer-handoff-pending]`,
 ...
 *
 * Designer divergence from the engineer copy (Codex Plan-verify blocker): a
 * completed 'rendered' marker is PRESERVED as a tombstone. designer's
...
function helpText() {
  return `designer session-handoff (ADR-0031)

Usage:
  session-handoff.mjs project [--repo-root <path>] [--branch <branch>] [--routing <route>]

Computes the designer bounded workflow projection (workflow_kind/id/path, phase,
...
This script only READS designer state; it never archives or mutates anything.
(The ADR-0043 S4 activation sidecar in this module is fired by state.mjs
...
export async function runSessionHandoff(options = {}) {
  ...
  return computeDesignerProjection({ repoRoot, branch, routing: options.routing });
}
