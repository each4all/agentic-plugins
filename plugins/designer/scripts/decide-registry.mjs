#!/usr/bin/env node
// Registry reader for designer:decide — ADR-0042 SD3 design/UX decision axes
// carried on the ADR-0027 §1 + §5.6 portable registry schema. Copied and
// adapted per ADR-0029 (copy-not-import): the reader logic is the
// engineer:decide reader; the optional `gate` veto flag + the size→preset
// map follow the founder ADR-0036 precedent, re-anchored here to designer's
// design/UX axes. The designer-local divergences are the in-code
// DEFAULT_FALLBACK design axes (the 7-axis `balanced` preset), the single
// `accessibility` veto gate, and the size→preset map (no compact tier —
// designer's matrix IS the 7-axis `balanced` preset; `--size` renders it at
// terse / standard / major depth via the SKILL @decide:* regions).
//
// Public API:
...
// In-code fallback — mirrors the `balanced` preset in decision-axes.yml (the
// 7-axis designer design/UX matrix, ADR-0042 SD3). LAST-RESORT preset when the
...
// fallback context shape matches the registry-resolved shape exactly. Keep
// this in lockstep with the `balanced` preset in decision-axes.yml.
const DEFAULT_FALLBACK = Object.freeze({
  preset_id: "balanced",
  axes: Object.freeze([
    freeze({ id: "usability",       labels: freeze({ en: "Usability",       ko: "사용성" }),     question: "Can the target users accomplish their task efficiently and without confusion — is the flow learnable, are affordances and states clear, is error recovery forgiving?", role: "decisive", gate: false }),
    freeze({ id: "consistency",     labels: freeze({ en: "Consistency",     ko: "일관성" }),     question: "Does this cohere with the established design system, platform conventions, and the product's existing patterns — or introduce a one-off that fragments the experience?", role: "decisive", gate: false }),
    freeze({ id: "conversion",      labels: freeze({ en: "Conversion",      ko: "전환" }),       question: "Does this move the user toward the intended action — is the primary call-to-action clear, the path low-friction, the value legible at the decision point?", role: "supporting", gate: false }),
    freeze({ id: "desirability",    labels: freeze({ en: "Desirability",    ko: "매력도" }),     question: "Is this something users will want to use — does it feel polished, trustworthy, and appropriate for the brand and audience?", role: "supporting", gate: false }),
    freeze({ id: "content-clarity", labels: freeze({ en: "Content Clarity", ko: "명확성" }),     question: "Is the content clear, scannable, and correctly leveled for the audience — labels, microcopy, information hierarchy, and reading order?", role: "supporting", gate: false }),
    freeze({ id: "feasibility",     labels: freeze({ en: "Feasibility",     ko: "구현가능성" }), question: "Can this be built and maintained within the frontend-stack, component-library, and effort constraints — reusing existing components rather than demanding new infrastructure?", role: "supporting", gate: false }),
    freeze({ id: "accessibility",   labels: freeze({ en: "Accessibility",   ko: "접근성" }),     question: "Does this meet accessibility requirements — perceivable, operable, understandable for users with disabilities (contrast, semantic structure, keyboard, visible focus, target size)? A hard WCAG A/AA barrier is a VETO gate, not a tradeoff — candidate-level only (ADR-0042 Non-Goal 6): runtime testing of focus order, keyboard traversal, and screen-reader behavior is required to certify conformance.", role: "supporting", gate: true }),
  ]),
});
...
function validatePreset(presetId, preset, presetDiagnostics) {
  ...
  for (let i = 0; i < preset.axes.length; i++) {
    ...
    // designer-local additive field (ADR-0042 SD3, following the founder
    // ADR-0036 gate precedent): optional veto-gate flag.
...
// designer-local registry). The reader applies the §1.6 failure-mode
    ...
    const axes = Object.freeze(
      presetsRaw[id].axes.map((a) =>
        freeze({
          ...
          question: a.question,
          role: a.role,
          gate: a.gate === true || a.gate === "true",   // designer veto-gate flag (ADR-0042 SD3); yaml-mini yields string "true". default false
        }),
      ),
    );
...
export function resolvePreset({
  ...
  if (registry) {
    ...
    if (!chosen && sizeExplicit && sizeValue) {
      // designer's matrix IS the 7-axis `balanced` preset — there is no
  ...
  const context = {
    body: body ?? "",
    preset_id: resolved.preset_id ?? resolved.id ?? "balanced",
    ...
    // preset_id="balanced" is ambiguous between "user requested balanced"
...
async function main(argv) {
  ...
  if (args[0] !== "resolve") {
    process.stderr.write(
      "decide-registry.mjs — designer:decide registry reader\n" +
  ...
  // (PR4 active) as `/designer:decide`. (peer P-9 / M5 fix)
