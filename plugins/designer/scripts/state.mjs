#!/usr/bin/env node
// plugins/designer/scripts/state.mjs
//
// Host-shared canonical state I/O for the designer plugin per ADR-0011,
// copy-trimmed from plugins/engineer/scripts/state.mjs per ADR-0042 SD7
// (sibling-copy pattern; cross-plugin imports forbidden per ADR-0010 §5).
//
// designer state contract — trim deltas vs the engineer sibling
// (ADR-0042 SD7):
//   - canonical-only home: designer is new, so there is NO legacy
...
//     removed — orchestrator→designer dispatch is ADR-0042 Non-Goal 2.
//
// Used by:
//   - plugins/designer/commands/<verb>.md (PR3+; thin-shim Phase 0 + state finalize)
//   - plugins/designer/adapters/{claude,codex}/hooks/* (snapshot writes)
//
// Storage location:
//   canonical: <repo_root>/.agentic-plugins/state/designer/workflows/<workflow_id>.md
//
// Lock files:
...
export const STATE_DIR_REL = '.agentic-plugins/state/designer';
...
// the lifecycle macro shape (future `/designer:start`, ADR-0042 PR6).
...
// designer trim — canonical home only (ADR-0042 SD7: no legacy dual-home).
...
// designer trim — `mode` is accepted for caller-signature parity with the
// engineer sibling but has no effect: with a single canonical home there
// is no dual-home write conflict to block (ADR-0042 SD7).
...
function inferStorageFromWorkflowPath(workflowPath) {
  ...
  const canonicalNeedle = '/.agentic-plugins/state/designer/';
 ...
 * deletion must be visible regardless of the current branch. designer
 * trim — only the canonical home exists (ADR-0042 SD7), so this walks a
...
export async function findActiveWorkflowByBranch(repoRoot, branch) {
  if (!branch) return null;
  // designer trim — canonical home only; no legacy probe, no dual-home
  // ambiguity error (ADR-0042 SD7).
...
const FRONTMATTER_KEY_ORDER = [
  ...
  // designer trim — the ADR-0019 cross-plugin parent-linkage keys
  // (parent_workflow / originating_subtask / parent_detached) are
  // intentionally absent: orchestrator→designer dispatch is ADR-0042
  // Non-Goal 2. A designer workflow file carrying those keys is treated
  ...
  // macro workflows (future `/designer:start`, ADR-0042 PR6) write 'start'.
  'workflow_type',
  ...
  'commit_manifest',
  // designer trim — ADR-0028 §P10 parent_writeback_at is absent (no
  // parent to write back to; ADR-0042 Non-Goal 2).
];
...
function validateSchema11Fields(fm) {
  ...
  // designer trim — parent_writeback_at and the ADR-0019 parent-linkage
  // scalars (parent_workflow / originating_subtask / parent_detached)
  // are not designer schema keys (ADR-0042 Non-Goal 2); when present on
...
export async function createWorkflowUnderLock({
  ...
  nextAction = '',
  bodyTitle,
  // designer trim — no parentWorkflow / originatingSubtask create
  // options: orchestrator→designer dispatch is ADR-0042 Non-Goal 2.
  ...
  // presence flag; the future `/designer:start` macro passes 'start'.
  ...
  if (existing) {
    throw new Error(
      `Cannot create workflow — a workflow already exists on branch '${gitBaseline.branch}' (${existing}). ` +
        ...
        `Resume with /designer:resume on this branch, or archive the existing workflow first.`,
    );
  }
  ...
  const frontmatter = {
    ...
    // verb commands; the future `/designer:start` macro overrides with 'start'.
...
// (the future designer phase7-commit driver, ADR-0042 PR6). Until that PR lands,
...
// when the verb skill is invoked as a sub-step of a designer workflow
...
// verbatim, and `$ACTIVE` is empty when no designer workflow is on the
...
// `plugins/designer/scripts/validate-commit.mjs` so the WRITE boundary
...
// staging code (the future designer phase7-commit driver) MUST
...
// designer trim — the ADR-0028 §P10 setParentWritebackMarker /
// clearParentWritebackMarker pair is removed: designer workflows have no
// parent to write back to (ADR-0042 Non-Goal 2).
...
export async function setTerminal({
  ...
  event = 'updated',
  now = new Date(),
  // ADR-0031 amendment (decision 1) / ADR-0043 S4 — fire the session-handoff
...
// designer trim — the ADR-0019 §5 detachArchive mid-flight detach helper
// is removed: it exists solely for orchestrator /finalize·/abort to
// detach a dispatched child, and orchestrator→designer dispatch is
// ADR-0042 Non-Goal 2.
 ...
 * request. Used by the future `/designer:start` Phase 0 BEFORE bootstrap to flag
...
// designer plugin itself writes during normal operation).
//
...
export function evaluateCleanBaseline({
  ...
  if (typeof statusPorcelain === 'string' && statusPorcelain.length > 0) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      // counts as designer's own bookkeeping movement.
      let pathForReport;
      ...
      // Exclude entries entirely — these are designer's own bookkeeping
...
function cliPrintHelp() {
  process.stdout.write(
    [
      ...
      '         [--persona designer] [--profile <name>] [--status-digest <hex>]',
      ...
      '    designer trim — no --parent-workflow / --originating-subtask flags:',
      '    orchestrator→designer dispatch is ADR-0042 Non-Goal 2.',
      '    ADR-0020 §Sub-decision 5 — --workflow-type discriminates the',
      '    workflow shape. Omit (or pass verb-chain) for single-verb workflows',
      '    produced by the six designer verb commands. The future',
      '    `/designer:start` macro passes start. Default: verb-chain.',
      '',
      ...
      '',
      '  (designer trim — no detach-archive subcommand: the ADR-0019 PR-E',
      '   mid-flight detach exists only for orchestrator dispatch, which is',
      '   ADR-0042 Non-Goal 2 for designer.)',
      '',
      ...
      '    by the future /designer:start Phase 0 BEFORE bootstrap. Default --base-branch=origin/main.',
      ...
      '',
      '  (designer trim — no set/clear-parent-writeback-marker subcommands:',
      '   the ADR-0028 §P10 write-ahead marker exists only for parent',
      '   writeback, which is ADR-0042 Non-Goal 2 for designer.)',
      '',
      ...
      '    current-process git HEAD. designer note: retained as a general',
      '    remote-archive surface; no external cross-plugin caller exists',
      '    (orchestrator dispatch is ADR-0042 Non-Goal 2). Emits the',
  ...
  try {
    switch (subcommand) {
      ...
      case 'create': {
        ...
        // designer guard — fail loud on parent-linkage flags instead of
        // silently ignoring them: a dispatcher exporting the
        // AGENTIC_PARENT_WORKFLOW contract at designer is misconfigured
        // (orchestrator→designer dispatch is ADR-0042 Non-Goal 2), and a
        // silently-unlinked workflow would mask that bug.
        if (flags['parent-workflow'] !== undefined || flags['originating-subtask'] !== undefined) {
          throw new Error(
            'designer state.mjs create does not accept --parent-workflow/--originating-subtask: ' +
              'orchestrator→designer dispatch is ADR-0042 Non-Goal 2',
          );
        }
        // designer guard — persona is canonical: a non-designer --persona
        // landing in the designer state home would cross persona
        // boundaries silently (read-side stays tolerant for fixtures).
        if (flags.persona !== undefined && flags.persona !== 'designer') {
          throw new Error(
            `designer state.mjs create only writes persona 'designer' (got '${flags.persona}')`,
          );
        }
        const result = await createWorkflow({
          ...
          verb: flags.verb,
          host: flags.host,
          persona: flags.persona ?? 'designer',
          ...
          // designer trim — no parent-linkage flags (ADR-0042 Non-Goal 2).
          // ADR-0020 PR 2 — workflow-shape discriminator. Omitting the
          // flag defaults to 'verb-chain' inside createWorkflowUnderLock;
          // the future `/designer:start` macro passes 'start'. cliParseFlags
      ...
      // designer trim — no set/clear-parent-writeback-marker subcommands
      // (ADR-0028 §P10 markers serve only parent writeback; ADR-0042
      // Non-Goal 2).
      ...
      case 'set-terminal': {
        ...
        await setTerminal({
          ...
          // ADR-0031 amendment / ADR-0043 S4 — this CLI case is the designer
          // production completion entry point (verb Phase 2 finalize + the
          // /designer:start terminal step); fire the sidecar.
          emitHandoff: true,
        });
        process.stdout.write(`${flags['workflow-path']}\n`);
        return 0;
      ...
      // designer trim — no detach-archive subcommand (ADR-0019 PR-E is
      // orchestrator-dispatch machinery; ADR-0042 Non-Goal 2).
