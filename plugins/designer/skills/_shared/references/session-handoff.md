...
designer workflow is archived then; if any fails it stays marked and a later Stop
...
- **filename**: `<projectionFile>.footer-rendered`, i.e. the canonical
  slot's sibling
  `.agentic-plugins/state/designer/last-session-handoff.json.footer-rendered`
  (the engineer slot shape — designer shares the single-projection-slot
  structure);
...
- **tombstone semantics** (designer divergence from the engineer copy): a
  `rendered` marker **survives** SessionStart consumption of the one-shot
  projection. designer's publish-needed workflow stays active-terminal until
...
designer is a **manually-published lifecycle**: verbs terminalize without
auto-committing — the owner saves/commits the design deliverable (the spec
handed to the frontend, or the frontend change itself; designer never
auto-commits and never dispatches, ADR-0042 Non-Goal 2). The sidecar
...
- **footer floor** `MIN_RUNTIME_VERSION` (gates on `scripts/footer.mjs`) —
  ...
  runtime below it would reject `workflow_kind: designer` and render the
...
host-symmetric: a Codex `$designer:<verb>` completion runs the same
...
review/trust — and every hook-bearing designer upgrade requires a fresh
...
- **Workflow-state read-only.** `session-handoff.mjs` only reads designer
  workflow state and runs the pure evaluator; it never archives, marks
  terminal, or mutates the workflow. Its only writes are designer's own
  ...
  pointer-only.
- **Fail-closed.** A corrupt designer state yields no projection; the seam
