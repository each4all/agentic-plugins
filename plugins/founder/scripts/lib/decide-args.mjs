// Argument-parser skeleton for /founder:decide (ADR-0027 §2.3 + §2.7).
//
...
//   /founder:decide [--size=<tier>] [--preset=<id>] [--weights=<spec>] [--] <decision body>
//
// Flags registered:
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (name) {
      ...
      case "size":
        ...
        // active: the registry reader resolves the preset (founder map,
        // ADR-0036 SD3 — no 9-axis tier; the business matrix tops out at the
        // 6-axis default):
        //   --size=minor    → preset=compact (4-axis: 2 decisive + 2 veto gates)
        //   --size=standard → preset=default (6-axis business matrix)
