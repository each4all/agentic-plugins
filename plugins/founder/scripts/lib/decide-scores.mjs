...
// recommendation for /founder:decide (ADR-0027 §1.3 invariant + PR4 Task 4).
//
...
// have the same top aggregate). Order: each axis in preset document order
// (only those with weight > 0) → option-letter ASCII as the final fallback.
//
// founder copy-adapt note (ADR-0029, Plan-verify finding): the engineer
// original special-cased `practical-fit` as the first tie-break priority.
// decide-scores is a domain-agnostic pure lib, so this founder copy carries
