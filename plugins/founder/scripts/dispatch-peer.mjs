#!/usr/bin/env node
// plugins/founder/scripts/dispatch-peer.mjs
//
// Host-shared canonical companion dispatch wrapper for the founder plugin.
//
// Responsibilities:
 ...
 * AND the founder ensemble-protocol § Required blocks (which adds
 ...
 *   <structured_output_contract>. Required by the founder ensemble
 ...
 *   (optional companion-contract field; the founder protocol prefers
...
export async function dispatchPeer({
  ...
  // response, and `/founder:resume` Step 5b will scrub orphaned pending
  ...
  if (promptText !== undefined) {
    const dir = await mkdtemp(join(tmpdir(), 'founder-prompt-'));
    cleanupTmpDir = dir;
