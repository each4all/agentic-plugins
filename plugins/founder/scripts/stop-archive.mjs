...
// ADR-0036 SD7. founder trim: NO parent writeback step — founder
// workflows have no orchestrator parent (ADR-0036 Non-Goal 3), so the
...
export async function runStopArchive({
  ...
  try {
    ...
    stderr.write(`founder/stop-archive: snapshot failed: ${err.message}\n`);
  ...
  try {
    ...
    stderr.write(
      `founder/stop-archive: failed to read ${workflowPath}: ${err.message}\n`,
    );
  ...
  try {
    ...
    stderr.write(`founder/stop-archive: archive failed: ${err.message}\n`);
    return { archived: false, reason: 'archive-threw' };
  }

  // founder trim — NO Step 5 parent writeback. The engineer sibling
  // fires ADR-0019 §4 writebackParent here when parent_workflow /
  // originating_subtask are set; founder workflows never carry those
  // fields (orchestrator→founder dispatch is ADR-0036 Non-Goal 3), so
...
export async function runStopArchiveOrphanSweep({ repoRoot, host, stderr = process.stderr }) {
  ...
  for (const workflowPath of files) {
    ...
    try {
      ...
      stderr.write(`founder/stop-archive: orphan-sweep skip ${workflowPath}: ${err.message}\n`);
      continue;
    }
