#!/usr/bin/env node
// plugins/founder/scripts/peer-runner.mjs
//
// ADR-0023 PR-B: caller-side peer-runner supervisor primitive for the
// founder plugin. This script owns operational lifecycle state around
...
// founder trim — canonical peer-run home only (ADR-0036 SD5: no legacy
// dual-home, no ambiguity resolution, no migration surface).
const PEER_RUNS_DIR_RELS = Object.freeze({
  canonical: '.agentic-plugins/state/founder/peer-runs',
});
...
// founder trim — all three resolvers collapse to the canonical home
...
// Founder is included from day one (ADR-0040 §5): its peer-runner is a full
// sibling, which partially compensates founder's missing footer sidecar.
//
...
export function validateHandleShape(handle) {
  ...
  if (handle.plugin !== 'founder') {
    return { ok: false, reason: `plugin must be founder: ${handle.plugin}` };
  }
...
async function materializePrompt({ paths, promptText, promptFile, retainPrompt }) {
  ...
  const dir = await mkdtemp(join(tmpdir(), 'founder-peer-runner-prompt-'));
