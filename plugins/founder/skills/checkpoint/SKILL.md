...
`checkpoint` does NOT bootstrap a new workflow — use one of the six founder
verbs (`/founder:investigate / :frame / :decide / :compose / :critique /
:refine`) for that.
...
|---------|--------|-------|
| Plugin root | `$CLAUDE_PLUGIN_ROOT` (set by the Claude Code runtime); Claude fallback `$(find ~/.claude/plugins/cache/agentic-plugins/founder -maxdepth 1 -mindepth 1 -type d \| sort -V \| tail -1)` | Direct path: `~/.codex/.tmp/marketplaces/agentic-plugins/plugins/founder` (Codex marketplace install layout per ADR-0008) |
| Entry path | `/founder:checkpoint <one-line summary>` | `$founder:checkpoint <one-line summary>` — this SKILL.md is the runbook; the skill-mention argument string is the `$ARGUMENTS` equivalent |
...
Inspect the argument string (the full text after `/founder:checkpoint` on
Claude, or after `$founder:checkpoint` on Codex):
...
external transmission — it is NOT sent to the peer host or web search.
Still, keep it free of raw proprietary identifiers when the workflow file
may be shared; a checkpoint is a progress pointer, not a place to paste
customer data.
...
- **Treating checkpoint as a phase transition.** It does NOT mutate
  `current_phase` or `next_action`. Use a verb skill or `founder:start` to
  advance phases.
- **Bootstrapping a workflow via `checkpoint`.** It requires an active
  workflow. Bootstrap first via one of the six verb skills.
...
- **Pasting customer/financial data into the summary.** A checkpoint is a
  progress pointer, not a data store.
