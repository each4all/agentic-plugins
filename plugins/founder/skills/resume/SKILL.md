---
name: resume
description: "Re-enters an in-flight founder workflow with a clean/dirty drift report against git baseline (or archives a stale one) — the founder plugin's resume meta skill (ADR-0022 meta-skill category, ADR-0036 SD2). A workflow-continuity meta operation, not a cognitive verb and not a lifecycle macro. Use to pick up an active business-deliverable workflow after compaction, a new session, or a branch switch, or to archive a stale workflow. Trigger phrases include 'resume founder', 'continue the founder workflow', 'pick up where I left off', 'drift report', 'archive this workflow', '재개', '이어서 진행', '워크플로 정리'. Cross-host inspection (Codex looking at a Claude-started workflow, or vice versa) is a canonical Codex use case — the filesystem state is host-shared."
---

# Resume (founder persona, meta skill)

The `resume` meta skill re-enters an in-flight founder workflow without
bootstrapping a new one. Unlike the six founder verb skills (which advance
...
host-agnostic filesystem operation. (The durable business artifact a founder
workflow produces — a saved brief / plan / canvas — is preserved at its own
`<root>/YYYY-MM-DD_<topic-slug>/` location even after the workflow `.md` is
archived.)
...
**Caveat (ADR-0021 / ADR-0036 SD2 boundary)**: Codex-side continuation does
...
Inspect the argument string (the full text after `/founder:resume` on
Claude, or after `$founder:resume` on Codex):
...
- **Exit 0, empty stdout** → no active workflow on the current branch. Tell
  ...
  `/founder:investigate` (or another verb) to bootstrap a new workflow.
...
- **Exit 1, per-branch duplicate error** → corruption / external mutation
  ...
  `/founder:resume archive <id>`. Do NOT pick one yourself — per-branch
...
(`verb-chain` for the six single-verb founder commands; `start` for the
lifecycle-macro workflows produced by `/founder:start`). Legacy files
...
host-history append. The user (or the next verb skill / `founder:start`)
...
sanity-check. The durable business artifact (saved brief / plan) is NOT
affected by archiving the workflow `.md`.
...
- **Mirroring Claude's `--host claude` on the Codex side.** Use `--host
  codex` when invoked via `$founder:resume`; the host flag distinguishes
