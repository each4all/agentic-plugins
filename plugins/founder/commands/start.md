...
pointer-only completion footer (context state, completion state +
state-derived next action, workflow id/path, the saved artifact pointer,
recommended next work) and surface the continue-vs-fresh read inline: the
macro workflow is terminal, so a fresh deliverable starts a new
`/founder:start`. Do not mutate host session context. On detached HEAD,
