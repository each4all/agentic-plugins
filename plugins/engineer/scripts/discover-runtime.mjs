...
//
// ADR-0039 §5 — runtime plugin root resolver. The engineer terminal path
...
// after it writes the ADR-0031 projection. Locating `footer.mjs` at the
// installed runtime plugin root requires this resolver.
//
// COPY-NOT-IMPORT (ADR-0010 §5). footer.mjs is L1 runtime; engineer is L3. The
// persona cannot `import` a runtime module (that would break SemVer
// independence). This module lives INSIDE engineer and discovers the runtime
// plugin root by filesystem inspection only; the eventual footer.mjs invocation
// goes through `child_process` exec. It is a deliberate copy of orchestrator's
...
// IN-PROCESS on the engineer terminal hot path (no CLI boundary), so the
...
// flag the engineer wiring passes. `--workflow-projection-file` (ADR-0031)
 ...
 *   4. Sibling fallback — derive engineer's own plugin root from `import.meta.url`
 *      (this file at `<engineer-root>/scripts/...`) and look for
 *      `<engineer-root>/../runtime/scripts/footer.mjs`.
...
export async function resolveRuntimePluginRoot({
  ...
  // engineer runs from one host's install and a stale opposite-host runtime
  ...
  // 4. Sibling fallback — derive engineer's plugin root from selfUrl, then look
  ...
  if (typeof selfUrl === 'string' && selfUrl.length > 0) {
    ...
    if (here) {
      // here = <engineer-root>/scripts/discover-runtime.mjs
 ...
 * `minVersion`. A missing OR too-old runtime returns `null` — the engineer
...
// The engineer terminal path calls `discoverRuntimePluginRoot` in-process, so
// this CLI is not on any runbook's critical path; it mirrors
