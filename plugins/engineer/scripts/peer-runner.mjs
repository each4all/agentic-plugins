...
// engineer plugin. This script owns operational lifecycle state around
...
const PEER_RUNS_DIR_RELS = Object.freeze({
  canonical: '.agentic-plugins/state/engineer/peer-runs',
  legacy: '.claude/agentic-engineer/peer-runs',
});
...
async function resolvePeerRunPathsForRead(repoRoot, runId) {
  ...
  if (canonicalExists && legacyExists) {
    throw new Error(
      `Ambiguous engineer peer-run storage: run_id=${runId} exists in both ` +
...
async function resolvePeerRunsDirForSweep(repoRoot) {
  ...
  if (canonicalHasRuns && legacyHasRuns) {
    throw new Error(
      `Ambiguous engineer peer-run storage: both ${PEER_RUNS_DIR_RELS.canonical} ` +
...
export function validateHandleShape(handle) {
  ...
  if (handle.plugin !== 'engineer') {
    return { ok: false, reason: `plugin must be engineer: ${handle.plugin}` };
  }
...
async function materializePrompt({ paths, promptText, promptFile, retainPrompt }) {
  ...
  const dir = await mkdtemp(join(tmpdir(), 'engineer-peer-runner-prompt-'));
