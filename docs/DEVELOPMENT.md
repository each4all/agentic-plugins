...
> `plugin-runtime` v0.84.0 (the ADR-0047 Release A notification
> surfaces — the filterable `response-needed` kind with the Codex
> shuttle remap and chain-aware re-render, plus the §5 seeded standing
> notification watch in `runtime:compat plan` with plan schema 1.1's
> `actionable` flag — atop the ADR-0045 entry-time proposal surfaces
