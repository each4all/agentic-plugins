...
> `plugin-runtime` v0.96.0 (the ADR-0048 bootstrap-observability
...
| 2 | engineer guarantees bidirectional companion round-trip | satisfied | Claude direction on engineer's own code: ✓ (D Phase 5 dispatch-peer parallel-review). Codex direction was historically pending on a Codex CLI commands auto-trigger path, with ADR-0013 retained as the future host-native command integration candidate. **2026-05-06 Stage 2.5+ exit audit Q4 PASS adds substantial evidence (128 unit + 4 smoke 양방향 실 LLM round-trip + JSON envelope wire validation per `companions/contract.md` §4.2). [ADR-0018](adr/0018-stage3-architecture-orchestrator-and-branch-context.md) §sub-5 ✅ PR [#54](https://github.com/each4all/agentic-plugins/pull/54) ships the cross-host integration test contract (claude→codex / codex→claude in-process state.mjs round-trip + stop-archive subprocess + standalone `.github/workflows/cross-host-tests.yml` CI matrix slot). 2026-05-16 satisfied transition. Latest installed proof: `plugin-runtime` `0.96.0` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded on 2026-08-27Z as `doctor-20260827T235051Z-c11408` — the post-release record of the **assurance matcher removal** loop, whose live reading is the thing the removal most needed measured: `doctor_runs` reads `available malformed=0` across **79** retained artifacts, so the dual reader carries the whole corpus rather than turning it into a fault; `experience_parity` reports **8** criteria against a **115** total, the denominator ADR-0056 §Decision 8 moved by removing a criterion no machine could ever satisfy; and the newest compat run reads `legacy_era`, which is the era gate refusing an assurance-era verdict as history rather than re-interpreting it. The superseded 0.95.0 state was the **verdicts that never left the run** loop, which shipped two runtime slices on one fault line, each a diagnosis a run reached and could not hand to anyone: `runtime:bootstrap plan` now warns, at the only moment the operator can still act, that a proof not opted into at plan time can never be attached once the run terminalizes `7a50a6f` (PR [#741](https://github.com/each4all/agentic-plugins/pull/741)), and `runtime:doctor` reports its findings through a diagnostic exit-code ladder instead of always exiting `0` — a BREAKING change for every caller that read `0` as a verdict `16d5c41` (PR [#743](https://github.com/each4all/agentic-plugins/pull/743)), released as PR [#742](https://github.com/each4all/agentic-plugins/pull/742) squash `f8ee09a`, tag `plugin-runtime-v0.95.0`, marketplace sync `f625415`, stage-doc sync `e6c1b26`. All three execute proofs pass in both directions (`permission_proof`, `deep_peer_smoke`, `workflow_continuation_proof`) with `host_parity` `pass` under Claude Code `2.1.247` / Codex `0.150.1`. `overall` reads **`warning`**: `hard_failures` is empty and the one warning is "latest compatibility check needs follow-up" — no hard failures is not the same claim as `pass`. The process exit code is `0`, which is a reading here rather than a constant: this is the first proof recorded under the ladder the same release shipped. Observed experience parity is `partial` `81%` (105/130, 5 satisfied / 3 partial / 1 not-verified) with **zero blocked** criteria and one manual follow-up, `codex-hook-review`: this release touched no Codex hook-bearing plugin (runtime ships no hooks), so the trusted entries are unchanged and the follow-up is the owner-gated `/hooks` re-attestation the Codex CLI offers no non-interactive query for. `host_parity_baseline` reads `stale` with the 2026-08-16 `2.1.233` / `0.147.0` baseline against an observed `2.1.247` / `0.150.1` — refreshing it is protected-asset work this recovery is forbidden to do, so it stays a named follow-up rather than a silently-carried claim. This recovery also writes the ADR-0049 record its predecessor omitted: PR [#740](https://github.com/each4all/agentic-plugins/pull/740) squash `4d9805e` closed the 0.94.0 document red and wrote no evidence record, so the store had run one loop behind — `the-interview-that-carried-values` back-fills it from git and from a proof artifact still on disk, and `the-verdicts-that-never-left-the-run` records this loop.
