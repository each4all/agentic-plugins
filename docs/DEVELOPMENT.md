...
> `plugin-runtime` v0.74.0 (ADR-0040 operator observability: the
...
| 2 | engineer guarantees bidirectional companion round-trip | satisfied | Claude direction on engineer's own code: ✓ (D Phase 5 dispatch-peer parallel-review). Codex direction was historically pending on a Codex CLI commands auto-trigger path, with ADR-0013 retained as the future host-native command integration candidate. **2026-05-06 Stage 2.5+ exit audit Q4 PASS adds substantial evidence (128 unit + 4 smoke 양방향 실 LLM round-trip + JSON envelope wire validation per `companions/contract.md` §4.2). [ADR-0018](adr/0018-stage3-architecture-orchestrator-and-branch-context.md) §sub-5 ✅ PR [#54](https://github.com/each4all/agentic-plugins/pull/54) ships the cross-host integration test contract (claude→codex / codex→claude in-process state.mjs round-trip + stop-archive subprocess + standalone `.github/workflows/cross-host-tests.yml` CI matrix slot). 2026-05-16 satisfied transition. Latest installed proof: `plugin-runtime` `0.74.0` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded under the 0.74.0 install on 2026-07-06 as `doctor-20260706T092051Z-f1052a` (Claude Code `2.1.201` / `codex-cli 0.142.5`, runtime installed `0.74.0` on **both** hosts; engineer `0.20.0` / orchestrator `0.12.0` / founder `0.3.0` / attention `0.3.0` / image `0.2.0`; experience parity `ready` `100%`, 8/8 satisfied — the Codex hook-bearing set (engineer / orchestrator / founder) is unchanged by this release, so the `/hooks` review attestation `settings-20260704T170801Z-b66656` (`untrusted=0`) still covers it, and `attention` `0.3.0` remains a deliberately Claude-hook-only plugin [ADR-0040 §3, recognized `claude_adapter_only`, not a Codex packaging gap]), superseding the 0.73.0 record `doctor-20260705T174334Z-08a68f` (parity `ready` `100%`) after the ADR-0041 §2d E1 egress transport fix release (the egress transport swapped from undici `fetch` to in-process `node:https`, IPv4-preferred; macro `018abf` subtasks [#500](https://github.com/each4all/agentic-plugins/pull/500) guard-transport / [#501](https://github.com/each4all/agentic-plugins/pull/501) impl-transport / [#503](https://github.com/each4all/agentic-plugins/pull/503) acceptance-gate / [#504](https://github.com/each4all/agentic-plugins/pull/504) release-dogfood real-delivery proof, release PR [#502](https://github.com/each4all/agentic-plugins/pull/502), tag `plugin-runtime-v0.74.0`; real owner Telegram delivery was proven 2026-07-06) bumped the installed runtime and closed that proof's reuse gate. Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. The explicit engineer dispatch proof satisfies ADR-0012 condition 2; ADR-0013 remains future command-surface integration work, not a blocker for this condition.** |
