...
- **Runtime/operator control plane (ADR-0024, Accepted)** becomes the immediate Stage 3+ dogfood target. It introduces a future `plugins/runtime` L1 framework primitive for `doctor` (host/plugin/auth/model/effort/sandbox readiness), `settings` (dry-run-first install/config assistance), dynamic peer fanout and consensus loops, context-hygiene policy, and completion footers that recommend current-session vs new-session continuation.
