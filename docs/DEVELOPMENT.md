...
> `plugin-runtime` v0.97.1 (the ADR-0048 bootstrap-observability
> surfaces — run schema 1.2 + evidence-contract vnext with
> kind-discriminated fail-closed proof validators and the
> `notify.configured` / `notify.codex.configured` split, the mode-bound
> exact Codex notify wiring probe with per-OS receiver argv, the
> agentic-6 statusline adapter with per-host exact probes and a
> render-only credential-free shim, and the triple-consent
> egress-ack-proof doctor executor — the one real-network Stage-8
> proof, the provider ack and the owner's phone-receipt attestation
> recorded separately with the derived `delivery-attested` label —
> atop the ADR-0047 Release B surfaces — the §6
> bounded expired-claim sweep `sweepExpiredClaims` behind the
> non-recursive `withReclaimLock` lock repair, and the §7 citation-aware
> artifact retention: the read-only `planRetention` planner with a
> reviewed plan hash, the `applyRetention` M1 deleting executor, and the
> `runtime:retention` CLI — atop the Release A notification surfaces, the
> filterable `response-needed` kind with the Codex shuttle remap and
> chain-aware re-render, plus the §5 seeded standing notification watch
> in `runtime:compat plan` with plan schema 1.1's `actionable` flag —
> atop the ADR-0045 entry-time proposal surfaces
> S6–S8 — the read-only `runtime:context entry-brief` arbiter with
> versioned tolerant readers and bounded scans, the §16 precedence
> lattice rendering one pointer-only brief with a single host-localized
> leading command against the closed `runtime-entry-brief-1.0` schema,
> the shared `lib/host-localization.mjs` leaf, the user-scope-only
> `entry_brief` / `entry_brief_empty` keys (env > user-global > shipped
> default, repo targets structurally refused), the snapshot-only
> dashboard `tier1.entry_advisory` reusing the same executor (absent in
> `--watch`), and the `assessEntryBriefReadiness` §18 diagnosis wired as
> doctor `entry_brief` / settings `entry_readiness` — the attention
> SessionStart sensor + entry floor pin ships with S9 — atop the
> ADR-0044 session-capture exit-side stack
> S2–S4 — packaged `session-capture-contract.md` + three closed schemas +
> the `session` config family, explicit `note` staging and `status --slot`
> inspection, the hook-grade `publish-session` slot publisher, and the
> shared doctor `session_capture` / settings `session_readiness` (1.21)
> half-enabled-chain diagnosis with the dynamically-read attention
> publisher-floor declaration — atop the ADR-0046 S8 machine-bootstrap stack:
> `runtime:bootstrap` as the tenth runtime command — a machine-scoped,
> artifact-only lifecycle over the live two-host probe seam
> (`probeMachineHostState`), bundle planning with hard-dependency
> closure, Stage 0 peer-host command detection, Stage 4–5 fragment
> rendering, the §1.6 plan-hash executor handoff,
> resume-with-mandatory-re-probe, recorded-proof verification, and the
> secrets-free portable machine profile — with the packaged
> `machine-bootstrap-contract.md` content-enforced in CI, plus the
> S8a4/S8a5 Codex hook-attestation repair (canonical `bound_versions` /
> `attested_plugins` and per-handler hook-disabled evidence); plus the
> S9 completion-output contract:
...
- Minimal continuity Option III per ADR-0011: workflow state file (markdown + YAML frontmatter) + four hooks (Claude `PreCompact` mid-session, Claude `Stop` end-of-session, Claude `SessionStart` for new-session summary injection, Codex `Stop` end-of-session) for automatic snapshot and resume awareness — *amended 2026-08-08: the hook set is no longer three-Claude-plus-one-Codex, and `SessionStart` was never "new-session"; both hosts register it with `matcher: "compact"` and fire post-compact only, and Codex gained its own SessionStart/PreCompact/Stop hooks on 2026-05-15. Correction recorded in [ADR-0022 §Amendments](adr/0022-engineer-meta-skill-category.md#amendments). Further amended 2026-08-23: `Stop` is not "end-of-session" on either host — it fires at the end of **each assistant turn**, so a terminal write is evaluated for auto-archive at that turn's boundary; see [ADR-0011 §Amendments](adr/0011-workflow-continuity-storage.md)*. **Per-branch** single-active workflow constraint enforced by directory-level creation lock + branch-keyed lookup (ADR-0018 §sub-2)
...
| 2 | engineer guarantees bidirectional companion round-trip | satisfied | Claude direction on engineer's own code: ✓ (D Phase 5 dispatch-peer parallel-review). Codex direction was historically pending on a Codex CLI commands auto-trigger path, with ADR-0013 retained as the future host-native command integration candidate. **2026-05-06 Stage 2.5+ exit audit Q4 PASS adds substantial evidence (128 unit + 4 smoke 양방향 실 LLM round-trip + JSON envelope wire validation per `companions/contract.md` §4.2). [ADR-0018](adr/0018-stage3-architecture-orchestrator-and-branch-context.md) §sub-5 ✅ PR [#54](https://github.com/each4all/agentic-plugins/pull/54) ships the cross-host integration test contract (claude→codex / codex→claude in-process state.mjs round-trip + stop-archive subprocess + standalone `.github/workflows/cross-host-tests.yml` CI matrix slot). 2026-05-16 satisfied transition. Latest installed proof: `plugin-runtime` `0.97.0` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded on 2026-08-28Z as `doctor-20260828T035114Z-44ccdf` — the post-release record of the **permission advisor removal** loop, whose live reading is the one this removal specifically put at risk: the doctor inner report bumped to `runtime-doctor-1.2` by **deleting** a field — `permission_diagnosis`, present in the 0.96.0 artifact under `1.1` and absent here, which is the deletion observed against its own control — yet `doctor_runs` still reads `available malformed=0` across the **80** artifacts retained at scan time, written under the `1.0` and `1.1` eras and read here by a `1.2` reader. ⚠ The other two fields ADR-0057 removes, `permission_plan` and `permission_plan_codex`, belong to the **settings** report contract (`runtime-settings-1.25` → `1.26`) and were never in a doctor report at all; their absence here is vacuous and is deliberately not claimed as evidence, because this loop recorded no settings report. A non-additive deletion is precisely the shape that converts a corpus into faults when the reader is single-era; it did not. (The count excludes this run's own `1.2` artifact, which lands after the scan — 81 are on disk now.) `permission_proof` **survives and passed in both directions**, which is the other half of the decision rather than an incidental: ADR-0057 §Decision 5 decoupled the boundary proof from the advisory fragment its applicability used to be gated on, so a proof that would otherwise have gone permanently inapplicable is live evidence that runtime still relaxes nothing. `experience_parity` reads `partial` **86%** (99/115, 5 satisfied / 3 partial / 0 blocked, 8 criteria) — **identical to the 0.96.0 reading, not an improvement on it**: this removal touched no parity criterion, and the same three partials and one manual `codex-hook-review` follow-up stand. `overall` reads **`warning`** with `hard_failures` empty and the one pre-existing warning, "latest compatibility check needs follow-up"; `host_parity` reads `pass` under Claude Code `2.1.250` / Codex `0.150.1`. `host_parity_baseline` reads `stale` — the 2026-08-16 `2.1.233` / `0.147.0` baseline against that observed pair — and this loop deliberately did **not** refresh it: a refresh asserts a new reviewed host observation, and this loop reviewed no host. Two facts were measured rather than carried forward. The packaged baseline hash **moved a second consecutive release**, `bdb6eeb7…` → `f7e3b5fac4ee198562e322259b043a27a461e2eb0391081ddd38c5f9fb697852`, identical in the Claude cache, the Codex served tree and the repository — which is also why `validate:release-obligation` reads `fulfilled` rather than being taken on trust. And byte identity was re-run, not restated: `diff -r` between Claude's cache and Codex's served marketplace tree — the checkout under `~/.codex/.tmp/marketplaces/`, not `~/.codex/plugins/cache` — exits `0` with no output, **120** files each (down from 126 — the advisor removal), whole-tree digest `74f2779a7ba883314455e5ec4525da0215a4de4c0bb009cf26bbea005a210a07` under the scorecard's path-first recipe, which was control-verified this loop by reproducing 0.96.0's published `725ad396…` exactly before the new value was published. ⚠ Unlike its predecessor, **this recovery performed no install**: both hosts already served `0.97.0` when the loop opened, verified by re-reading the installed manifests, so this record attests the installed state and not the verbs that produced it. Released as PR [#753](https://github.com/each4all/agentic-plugins/pull/753) squash `f02a2c2`, tag `plugin-runtime-v0.97.0`, marketplace sync `9fd523c`, stage-doc sync `30af961`, carrying the removal commit `9d014b7` (PR [#752](https://github.com/each4all/agentic-plugins/pull/752)). The superseded 0.96.0 state was the **assurance matcher removal** loop (`doctor-20260827T235051Z-c11408`), which cut the compatibility-assurance grant/cohort matcher after it shipped twice and answered `covered` zero times on a real machine; its live reading was `doctor_runs` `available malformed=0` across **79** artifacts, and its `86%` 99/115 parity figure is the one this loop's identical figure continues rather than improves upon.

**Two things this loop established that are worth keeping.** The first is a review finding rather than a release outcome: the slice went out only after a two-lane review (`/code-review` plus a cross-host Codex pass) and both lanes independently found that the seed path read its `user_scope_only` trust label off the incoming, untrusted profile — a document declaring a newer schema minor could author the label while the validator logged it as ignored. It is now derived from runtime-owned policy. The second is a host asymmetry that the 0.92.0 record had documented only half of: Claude registers this marketplace as a **directory** source and needs `claude plugin marketplace update` before `claude plugin update` sees anything, while Codex registers it as a **git** source, exposes no `plugin update` verb at all, and refreshes through `codex plugin marketplace upgrade <name>` — which is why the stale-catalog trap never reached Codex. The prior record — the **first assurance grant** loop (ADR-0054 rollout R2), proof `doctor-20260820T041120Z-c4c83e` on 2026-08-20Z against plugin-runtime 0.91.2 — shipped as a single release: the grant review brief `f9af711` (PR [#713](https://github.com/each4all/agentic-plugins/pull/713)), its adversarial verification `1205258` (PR [#714](https://github.com/each4all/agentic-plugins/pull/714)) which re-ran the brief's controls and corrected four of its negative claims, the owner ratification `da7b8d9` (PR [#715](https://github.com/each4all/agentic-plugins/pull/715)) retargeted after the host moved mid-review, and the grant itself together with the R1 assertions it replaces `592fa1a` (PR [#716](https://github.com/each4all/agentic-plugins/pull/716)), released as PR [#717](https://github.com/each4all/agentic-plugins/pull/717) squash `95e5b04`, tag `plugin-runtime-v0.91.2`, marketplace sync `b92cc1d`, stage-doc sync `da60fd3`. All three execute proofs pass in both directions (`permission_proof`, `deep_peer_smoke`, `workflow_continuation_proof`) with `host_parity` `pass` under Claude Code `2.1.237` / Codex `0.148.0`. `overall` reads **`warning`**: `hard_failures` is empty and the one warning is "latest compatibility check needs follow-up" — no hard failures is not the same claim as `pass`. The byte comparison is clean and is the evidence here rather than the version string: both host caches hold an identical 0.91.2 package (124 files, whole-tree digest `4365588804c00b0d4d872894b81fc9b1d9c78a93a9387698d70e4b1f46a5ed1a`), and the packaged baseline hashes `86fdf17e4418884f01b67785eeaaacc74923a8a01efa238f514bf270da600843` in the Claude cache, the Codex cache, the `plugin-runtime-v0.91.2` tag and the repository alike — the same value doctor records as `host_parity_baseline.provenance.content_sha256`. **This is the first recorded proof taken against a shipped, non-empty grant set, and the verdict is still a refusal — a different one.** The released grant `claude-2-1-234-235-codex-0-147-0` names two cohort tuples, claude `2.1.234` / codex `0.147.0` and claude `2.1.235` / codex `0.147.0`; the machine reads claude `2.1.237` / codex `0.148.0`, which is in neither. The assurance layer therefore reads **`unassured`** with `record_status` `resolved` — the reader, the integrity check, the canonical-bytes comparison and package resolution all worked, and only the cohort match failed. That distinction is the point: R1 exercised the **empty-set** refusal, and this is the **cohort-miss** refusal, a second and separate negative path through the same gate. It is neither a matcher defect nor a grant-authoring error — the matcher declined to extend a review to a version nobody reviewed, which is precisely what ADR-0053 §Decision 5 requires, and per ADR-0053 §Decision 7 a cohort can only name tuples that were actually reviewed, so no amount of foresight at authoring time could have covered `0.148.0`. The cause is host movement, and it is measured rather than asserted: Codex moved `0.147.0`→`0.148.0` after the grant was ratified, and Claude then moved `2.1.235`→`2.1.237` during this recovery itself. Across the retained compat snapshots Claude has taken 18 distinct versions at a median 2.1 days apart (minimum 0.5) and Codex 7 at a median 10.2 days, while this loop's brief-to-install round trip was 10.8 hours — so a grant is not structurally doomed by the treadmill; this one lost to a Codex step that arrived 17 days after its predecessor and landed inside the window. The remedy is a new review and a new grant in a forward-patch loop, never an edit to the released id, which is immutable. `host_parity_baseline` reads `stale` with `baseline-direction` `ahead` on both hosts (baseline `2.1.233` / `0.147.0` cut 2026-08-16 against observed `2.1.237` / `0.148.0`), and that is the designed separation rather than a defect: exactness says no while assurance answers a different question, and refreshing the packaged baseline is protected-asset work that would open a new release obligation. Observed experience parity is **`partial` `81%`** (105/130; 5 satisfied, 3 partial, 1 not-verified, **0 blocked**, 1 manual follow-up). The headline of this recovery is the **blocked criterion closing**: `runtime_handoff_artifacts` moved 0/15 → 9/15 because content-backed release notes were ingested for both hosts — Claude's from the installed CLI's own changelog cache, Codex's fetched explicitly from the `rust-v0.148.0` release page — and that closure did **not** require assurance coverage, because `doctor` deliberately refuses to charge the same unassured fact to both the coverage criterion and the readability one. It stops at partial rather than satisfied because the compat plan is genuinely actionable: ingesting the Codex notes fired the standing `codex-notify-payload-variants` watch, which is real follow-up work and not a formality. Three items stay open with distinct next steps that must not be chained into one remedy: `runtime_handoff_artifacts` needs that notify-payload review; `plugin_management_followups` and `lifecycle_hook_continuity` need a Codex `/hooks` re-review, which is an operator action this record cannot self-attest; and `host_compatibility_assurance` needs human review plus a released grant naming this pair. Earned weight moved 106/130 → 105/130 while the status improved from `blocked` to `partial`, and the two must be read together rather than as one trend: a blocked criterion worth 0 became a partial worth 9, while the Codex `/hooks` attestation that the previous record observed as closed was staled by this very release, turning two satisfied criteria into partials. Nothing regressed in execution — every bidirectional proof in this run passes. It supersedes the 0.91.1 record re-recorded on 2026-08-18Z (doctor-20260818T115259Z-e7c325 — the **assurance plane** loop; [ADR-0053](adr/0053-baseline-exactness-and-compatibility-assurance.md) `38e59bb` (PR [#701](https://github.com/each4all/agentic-plugins/pull/701)) and [ADR-0054](adr/0054-assurance-record-schema-and-rollout.md) `48db4f3` (PR [#702](https://github.com/each4all/agentic-plugins/pull/702)), six implementation commits `e8f6d2b` record schema / `79003cc` packaged comparator / `b8ad2b3` semantic matcher / `c535c1a` doctor's three-layer verdict / `6651c5c` compat's frozen observation / `70e0461` both readiness paths gating, released as PR [#704](https://github.com/each4all/agentic-plugins/pull/704) squash `47bc9c9`, tag plugin-runtime-v0.91.0, marketplace sync `361952c`, stage-doc sync `e8a9b44`; then the adversarial-audit hardening commit `4ee8764` (PR [#710](https://github.com/each4all/agentic-plugins/pull/710)), released as PR [#711](https://github.com/each4all/agentic-plugins/pull/711) squash `48f50d2`, tag plugin-runtime-v0.91.1, marketplace sync `024ec42`, stage-doc sync `f24727b`), which read `blocked` 82% 7/9 (106/130) with `host_parity` `pass`, `host_parity_baseline` `stale`/`ahead` and assurance **`unassured`** under Claude Code `2.1.234` / Codex `0.147.0` — the empty-set refusal R1 shipped by owner decision, and the first proof in which the assurance verdict was operator-visible at all. It supersedes the 0.90.3 record re-recorded on 2026-08-16Z (doctor-20260816T144610Z-154506 — the **Claude 2.1.233 host-parity baseline refresh and withdrawn-todo-tool adaptation** loop; implementation PR [#698](https://github.com/each4all/agentic-plugins/pull/698) rebase-merged as five package-scoped commits `2eea2b3` / `0cb184e` / `feeeafb` / `e563921` / `6abe4be`, release PR [#699](https://github.com/each4all/agentic-plugins/pull/699) squash `38fedaf`, tag plugin-runtime-v0.90.3, marketplace sync `e46256b`, stage-doc sync `5ee8024`), which read `partial` 91% 6/8 with `host_parity_baseline` `current` under Claude Code `2.1.233` / Codex `0.147.0` — its two partials being the single Codex `/hooks` operator action this record observes as closed. It supersedes the 0.90.2 record re-recorded on 2026-08-14Z (doctor-20260814T083045Z-bc4fc6 — the **ADR-0051 P2 review hardening** loop; implementation PR [#694](https://github.com/each4all/agentic-plugins/pull/694) squash `aaf4744`, release PR [#695](https://github.com/each4all/agentic-plugins/pull/695) squash `271c5ae`, tag plugin-runtime-v0.90.2, marketplace sync `dbcb983`, stage-doc sync `3ea869a`), which read `partial` `95%` 7/8 with `host_parity_baseline` `stale` under Claude Code `2.1.232` / Codex `0.147.0` — the record whose owner-amended stop clause opened the named follow-up this loop closes. This supersedes the 0.90.1 record re-recorded on 2026-08-12Z (doctor-20260812T022356Z-252fb9 — the **Claude 2.1.227 host-parity baseline refresh** loop; implementation PR [#688](https://github.com/each4all/agentic-plugins/pull/688) squash `354a95d`, release PR [#689](https://github.com/each4all/agentic-plugins/pull/689) squash `8c9789a`, tag plugin-runtime-v0.90.1, marketplace sync `6ba5f21`, stage-doc sync `94de990`), the first baseline refresh to carry the ADR-0051 §Decision 2 release obligation instead of landing as a docs-only edit, which read `partial` `95%` 7/8 and `host_parity_baseline` `stale` under Claude Code `2.1.228` / Codex `0.147.0`: Claude published `2.1.228` 20.8h after `2.1.227`, 13.8h after that loop observed `2.1.227` and 7.8h **before** its PR merged, so that baseline was stale before it shipped. Against Claude’s median 24.0h inter-patch interval (11 intervals from `2.1.217` to `2.1.228`, npm publish times), its 22.1h loop — measured between the observation snapshot `compat-20260811T035848Z-d6c3df` and the post-install verification snapshot `compat-20260812T020732Z-295af8` — could not reliably close, which is the cost datum [ADR-0052](adr/0052-release-obligation-enforcement.md) used to re-judge ADR-0051 §Alternatives D and to leave it deferred. This supersedes the 0.90.0 record re-recorded on 2026-08-10Z (doctor-20260810T135637Z-d49983 — the **ADR-0051 host-parity baseline source** loop; ADR PR [#684](https://github.com/each4all/agentic-plugins/pull/684) squash `4bb25a5`, implementation PR [#685](https://github.com/each4all/agentic-plugins/pull/685) squash `20ebed7`, release PR [#686](https://github.com/each4all/agentic-plugins/pull/686) squash `4a23a5b`, tag plugin-runtime-v0.90.0, marketplace sync `926ec01`, stage-doc sync `d9b6e93`; all three execute proofs passed, `host_parity` `pass`, `host_parity_baseline` `current` under Claude Code `2.1.226` / Codex `0.147.0`, observed experience parity `ready` `100%` 8/8). The `95%` of the record **that** one superseded was likewise not a regression but a structural ceiling, and that release is what removed it: that record named the cause correctly — `runtime:compat` resolved the host baseline from the INSTALLED plugin root while `runtime:doctor` resolved it from the repo, so a baseline refreshed in-repo could not clear compat's drift — and [ADR-0051](adr/0051-host-parity-baseline-source.md) makes the packaged copy the sole authority for both. Measured immediately after this release: a fresh `runtime:compat` snapshot from the installed `0.90.0` read `drift: none` / `current` for the first time, against a baseline whose recorded provenance is `source: package`, `runtime_version: 0.90.0`, with a content hash — where the day before the same command re-raised the same drift on every run. Two same-day records show the work rather than hiding it: `doctor-20260808T063339Z-74647d` (2026-08-08Z) read `blocked` `78%` — the engineer peer-run ledger was blocked by a five-day-old non-terminal `plan-verify` handle, and the Codex hook attestation was version-stale because both the host (`0.145.0`->`0.147.0`) and a hook-bearing plugin (engineer `0.21.1`->`0.21.2`) had moved; after the handle was reconciled to `orphaned` and `runtime:settings --attest-codex-hook-review` recorded `settings-20260808T065145Z-c8409f` binding designer@0.3.1 / engineer@0.21.2 / founder@0.4.1 / orchestrator@0.13.1 against Codex `0.147.0`, `doctor-20260808T065825Z-2dfe23` (2026-08-08Z) read `blocked` `87%`, its one remaining block self-inflicted by an extra verification-only compat snapshot that became the latest run without ingested release notes. What went stale on the hook side was runtime's attestation ARTIFACT, not Codex's trust: `~/.codex/config.toml` `[hooks.state]` carried `enabled = true` plus a matching `trusted_hash` for all twelve entries throughout, because every hook command is the same `${PLUGIN_ROOT}`-relative text and the version bump did not edit it. This supersedes the 0.88.1 record `doctor-20260803T091403Z-7f2850` (2026-08-03Z) — the post-attestation record of the **presentation-and-persistence** loop, a FIVE-package patch release (release PR [#666](https://github.com/each4all/agentic-plugins/pull/666) squash `208545c`, tags `plugin-runtime-v0.88.1` / `plugin-engineer-v0.21.1` / `plugin-designer-v0.3.1` / `plugin-founder-v0.4.1` / `plugin-orchestrator-v0.13.1`, marketplace sync `4e6ee69`, stage-doc sync `0b98de6`). Three feature loops rode it: [#665](https://github.com/each4all/agentic-plugins/pull/665) `0d46223` spent the Stage-8 evidence budget PER REASON and gave `profile export`/`profile seed` the output they computed; [#667](https://github.com/each4all/agentic-plugins/pull/667) `d231aa2` accepted [ADR-0050](adr/0050-fragment-persistence-boundary.md) (immutable content-addressed fragments, one commit point); and [#668](https://github.com/each4all/agentic-plugins/pull/668) made the peer-run sweep preview state what `--apply` deletes — **rebase-merged, not squashed**, because its ten commits span five release-please package paths and a squash would have routed one commit to all five (ADR-0016), which is why this release bumps four persona packages independently rather than as one. All three execute proofs passed (`permission_proof`, `deep_peer_smoke`, `workflow_continuation_proof`) with `overall` `pass` under Claude Code `2.1.220` / Codex `0.145.0`. Observed experience parity is **`ready` `100%` 8/8** (115/115, zero manual follow-ups). The install proof taken earlier the same day, `doctor-20260803T033236Z-f56d25`, read **`partial` `91%` (105/115)** with one manual follow-up — `codex-hook-review` — because all four hook-bearing plugins (designer, engineer, founder, orchestrator) changed version at once, which invalidates runtime's version-bound hook-attestation claim (ADR-0030). The `codex-hook-review` follow-up was cleared the same day: the owner confirmed all twelve Codex hook entries (four plugins x three events) trusted, `runtime:settings --attest-codex-hook-review` recorded `settings-20260803T091332Z-ec30fe` binding designer@0.3.1 / engineer@0.21.1 / founder@0.4.1 / orchestrator@0.13.1 against Codex `0.145.0`, and the post-attestation record `doctor-20260803T091403Z-7f2850` restores observed parity to **`ready` `100%` 8/8** (115/115, zero manual follow-ups) with all three execute proofs passing again. What was stale was runtime's ATTESTATION ARTIFACT (the previous one dated 2026-07-22), not Codex's trust — runtime cannot query hook trust non-interactively, so the version bump correctly invalidated its claim while the host's own trust was intact throughout. This supersedes the 0.88.0-native record `doctor-20260802T132248Z-2b8a4c` (2026-08-02Z) — the **one machine snapshot per verb** loop, a MAJOR-bearing runtime release (feature PR [#663](https://github.com/each4all/agentic-plugins/pull/663) `383dc14` made every verb gather ONE machine snapshot — probe, raw host facts and user-global readers together — and rebuild everything it judges, persists and reports from that one gathering: `resume` had been reducing and persisting against a post-execution probe while its own `steps[]`, Stage 0 and fragment composition still derived from the pre-execution one, so a run stored a probe its rows had never been judged against; three cross-host review rounds then found that the first two repairs moved the problem rather than removed it, and each finding was reproduced before it was accepted — the first reconstruction introduced a false terminal completion, because §6.2 lets a satisfying observation clear a `declined` row and a host-scoped refusal lives only in that row, so the selection had to converge in the shared re-judgement rather than in `resume` alone; the snapshot trigger had to become the doctor SPAWN rather than 'evidence imported'; two user-global config files were each being opened twice; and the convergence closed the D0.1 receipt door until `attest` was made to judge the run as it was reduced. Five separate contract and comment sentences in that slice asserted behaviour the code did not have, every one caught by running the path rather than re-reading it, which is why the §7 text is now deliberately narrow and five reproduced gaps are recorded rather than claimed; feature PR [#662](https://github.com/each4all/agentic-plugins/pull/662) `5c8cf79` carries the release's BREAKING change, moving the JSON report identifier to `runtime-bootstrap-report-2.0` and removing the `completion` key from a legacy-historical exit-50 report in favour of `legacy_completion_summary`; feature PR [#661](https://github.com/each4all/agentic-plugins/pull/661) `358a75a` judged `tui.notifications` as the second half of `notify.codex.configured`; and `67bbc54` landed one grammar for answer applicability, asked at both boundaries, without a PR number of its own; release PR [#660](https://github.com/each4all/agentic-plugins/pull/660) `chore: release main` squash `6ea6208`, tag plugin-runtime-v0.88.0, marketplace sync `8243de5`; installs refreshed host-native — `claude plugin update` on Claude, `codex plugin marketplace upgrade` + `codex plugin add` on Codex — so no settings-executor artifact is claimed and the installed state is doctor-observed: both hosts report runtime `0.88.0` installed and enabled; Claude Code `2.1.220` / `codex-cli 0.145.0`, with `host_parity_baseline` reading `current` against the 2026-07-25 `2.1.220`/`0.145.0` baseline — neither host drifted this loop, so no compat cycle was required and the drift=none re-check `compat-20260725T020139Z-f0cc72` stands): `overall` `pass`, all three execute proofs (permission proof, deep-peer-smoke, workflow-continuation proof) executed and passed in **both** directions, experience parity **`ready` `100%` 8/8** with zero manual follow-ups and zero not-verified rows (runtime ships no hooks, so the four-plugin attestation `settings-20260722T021211Z-364c8f` stays current), and the doctor `entry_brief` and `session_capture` sections both read `ready`, with the workflow ledgers clean. One honest note belongs on this record: the egress provider-ack proof was NOT re-executed this loop and stays stale against `0.88.0`, because §4 requires the owner's explicit triple consent for a real send and this release did not carry one. This supersedes the 0.87.0-native record `doctor-20260731T051408Z-210043` (2026-07-31Z) — the **bootstrap effective-selection + model/effort posture** loop (feature PRs [#655](https://github.com/each4all/agentic-plugins/pull/655) `3b75915` and [#657](https://github.com/each4all/agentic-plugins/pull/657) `167fd4a`, release PR [#656](https://github.com/each4all/agentic-plugins/pull/656) squash `7fc7e63`, tag plugin-runtime-v0.87.0, marketplace sync `6d53cab`), which itself supersedes the 0.86.3-native **#645 Codex /hooks attestation import** record re-recorded on 2026-07-28Z as doctor-20260728T013239Z-2a6995 — that loop (bootstrap read the operator attestation from a top-level `doctorReport.codex_hook_review` key that doctor emits on no report, so the import never ran and the non-declinable `hooks.codex.attested` step could never be satisfied on any hook-bearing bundle, silently; the fix reads the `settings_runs.codex_hook_review` currency wrapper doctor actually publishes, judges the claim by the run own selection rather than doctor machine-wide verdict, refreshes a stale stored record instead of short-circuiting on its presence, and re-judges the step inside the importing resume against that resume own answered rows; feature PR [#652](https://github.com/each4all/agentic-plugins/pull/652) `b55ce53`, whose subject carries no PR number because the squash subject was passed explicitly on the CLI; release PR [#653](https://github.com/each4all/agentic-plugins/pull/653) `chore: release main` squash `9abddc6`, tag plugin-runtime-v0.86.3, marketplace sync `5bb88c1`; this loop Claude install ran host-native `claude plugin marketplace update agentic-plugins` + `claude plugin update runtime@agentic-plugins` while the Codex cache was already at 0.86.3 from its git marketplace source — so no settings-executor artifact is claimed and the installed state is doctor-observed: both hosts report runtime `0.86.3` installed and enabled; Claude Code `2.1.220` / `codex-cli 0.145.0`, with `host_parity_baseline` reading `current` against the 2026-07-25 `2.1.220`/`0.145.0` baseline — this loop drifted neither host, so no compat refresh was required): `overall` `pass`, all three execute proofs (permission proof, deep-peer-smoke, workflow-continuation proof) executed and passed in **both** directions, experience parity **`ready` `100%` 8/8** with zero manual follow-ups (runtime ships no hooks, so the four-plugin attestation `settings-20260722T021211Z-364c8f` stays current), and the doctor `entry_brief` / `session_capture` sections both read `ready` — with the honest pre-proof ledger note: the first 0.86.0 doctor read `warning` on one stale `running` engineer peer-run handle (`review-20260722T232336Z-146d22c0`, orphaned by a session end), reconciled to `orphaned` plus a 138-handle TTL prune via engineer `peer-runner.mjs sweep --apply` before the proof (the sweep dry-run's empty prune preview is a recorded follow-up). The same loop closed the **ADR-0048 §3 live-fire egress dogfood**: the run-schema-1.2 bootstrap run `bootstrap-20260723T124729Z-b5191c` (bundle `base`, 28 steps) took the operator's explicit `execute` answer on `proof.egress-provider-ack`; the operator exported the egress credential directly in their own shell (triple consent including `AGENTIC_EGRESS_REAL_SMOKE=1` — no token through agent or tool, ADR-0048 §4), and the doctor egress executor `doctor-20260723T133245Z-ac0499` dispatched ONE closed-vocabulary synthetic `response-needed` send through the pinned `notify.mjs` emitter — provider `acked`, `mirror_correlated=true`, write-ahead intent resolved `acked`, sanitized metadata only — after which the owner confirmed the message on their phone (subject `egress-proof-5f00461ebdc9`) and the receipt attestation was recorded (`surface=owner-phone`, matching `attempt_hash`, linked provider-proof artifact hash), deriving the run's **`delivery-attested`** completion label; this evidence was written into the docs only AFTER the real send and phone receipt, per the no-pre-recording discipline. Honest caveats (the first and third recorded as follow-up rows; the statusline decline is an owner decision, not a defect): the bootstrap run itself stays `open` on the undeclinable `config.model_effort` step (this machine deliberately runs host-default model/effort), both statusline steps were declined (the existing operator statusLine is kept; replace/manual-merge is a separate owner decision), and the run's `steps[]` row for the executed egress proof renders `pending` while the completion reducer reads `passed` (display-layer divergence). This supersedes the 0.85.0-native **ADR-0047 Release B** record `doctor-20260722T012908Z-472538` (2026-07-22Z; the loop shipped the §6 bounded expired-claim sweep `sweepExpiredClaims` behind the non-recursive `withReclaimLock` lock repair, and the §7 citation-aware artifact retention: the read-only `planRetention` planner, the `applyRetention` M1 deleting executor gated on a reviewed plan hash, and the `runtime:retention` CLI — §6 commits `1dc3bc4`/`949aa65`, §7 commits `123e9a0`/`e9d5a1c`/`1a3c5c6`/`0bdbdd5`; release PR [#616](https://github.com/each4all/agentic-plugins/pull/616) `chore: release main` merge `c2bc0f9`, tags plugin-attention-v0.9.0 + plugin-runtime-v0.85.0 (backticks deliberately omitted for the freshness gate; attention 0.9.0 = the ADR-0047 §1/§2 `response-needed` Stop-finality classifier + headline producers, the Claude-side producer beyond 0.8.0's vocabulary-only enum mirror), marketplace sync `36057ad`; installs refreshed host-native — `claude plugin update` for runtime+attention — so no settings-executor artifact is claimed and the installed state is doctor-observed: both hosts report runtime 0.85.0 and attention 0.9.0 installed and enabled; Claude Code `2.1.217` / `codex-cli 0.145.0`, with the same-day host-parity baseline refresh to `2.1.217`/`0.145.0` closing the dual drift the release exposed): `overall` `pass` with **zero warnings**, all three execute proofs (permission proof, deep-peer-smoke, workflow-continuation proof) executed and passed in **both** directions, `host_parity_baseline` **`current`** (the dual Claude `2.1.216`→`2.1.217` / Codex `0.144.6`→`0.145.0` drift the first proof run exposed was closed the same day by the compat cycle — content-backed CHANGELOG + Codex atom ingest in `compat-20260722T011840Z-a3fb14`, drift=none re-check in `compat-20260722T012840Z-915f54`; Claude additive/fix-only and Codex a no-plugin-CLI/hook-contract-break minor, the only Codex `codex features list` stage change being `multi_agent_v2` under-development→stable), the doctor `entry_brief` section reads `ready` (the operator's ADR-0045 user-scope `startup` opt-in with `empty=silent`, attention `0.9.0` enabled, the `floors.entry_brief=0.83.0` pin satisfied by `0.85.0`, entry executor present) alongside `session_capture` `ready`, Codex hook state holds `12/12` expected `enabled_trusted` with `unexpected_agentic_entries=0`, and **experience parity deliberately reads `partial` `91%` (manual follow-up `codex-hook-review`)** — the honest fail-closed half of the attestation discipline this cycle: the **Codex CLI `0.144.6`→`0.145.0` minor bump version-invalidated** the four-plugin `/hooks` attestation `settings-20260720T151554Z-3b543f` (`bound_versions.codex=0.144.6`), which the currency evaluation now reads `stale` (`currency_reason=codex_cli_version_changed`). The trusted Codex hook entries themselves are unchanged and healthy (12/12 `enabled_trusted`, `expected_untrusted=0`, `expected_disabled=0`) — neither released package changed a Codex hook-bearing plugin (runtime ships no hooks; attention `0.9.0` changed only the **Claude** hook surface and keeps zero Codex hook surface; the four attested plugins designer@0.3.0 / engineer@0.21.0 / founder@0.4.0 / orchestrator@0.13.0 are unchanged) — so a fresh operator `/hooks` confirmation plus `runtime:settings --attest-codex-hook-review` restores `ready` — and that restoration landed the same day: the operator's `/hooks` confirmation produced the fresh four-plugin attestation `settings-20260722T021211Z-364c8f` (`bound_versions.codex=0.145.0`), and the post-attestation record `doctor-20260722T021258Z-4c7514` restores observed parity to **`ready` `100%` 8/8** (zero manual follow-ups) with all three execute proofs re-passed in both directions and `host_parity_baseline` still `current`. This supersedes the 0.84.0-native **ADR-0047 Release A** record doctor-20260721T110155Z-928e38 (2026-07-21Z; runtime 0.84.0 + attention 0.8.0 on both hosts, Claude Code 2.1.216 / codex-cli 0.144.6; feature PRs #611 squash 560527f + #613 rebase-merged 54f39c0 / 952af14 / 73b88c1, release PR #612 squash d9a8a7d, tags plugin-attention-v0.8.0 + plugin-runtime-v0.84.0, marketplace sync d93d23e; overall pass with zero warnings, three execute proofs both directions, parity ready 100% 8/8 with the four-plugin attestation then current since attention 0.8.0 was vocabulary-only and codex-cli 0.144.6 was unchanged), and in turn the 0.83.1-native **S9 peer-follow-up hardening** record doctor-20260720T175310Z-a0fd88 (2026-07-20Z; runtime 0.83.1 + attention 0.7.1 on both hosts, overall pass with zero warnings, three execute proofs both directions, parity ready 100% — feature PR #606 rebase-merged eb480f3 / ba1c201 / fd381b4, release PR #607 squash 9a69944, tags plugin-attention-v0.7.1 + plugin-runtime-v0.83.1, marketplace sync f54d70c), and in turn the same-day attention-0.7.0 freshness record `doctor-20260720T105456Z-e1f9a9` — the **ADR-0045 S9 attention SessionStart entry-sensor** loop (feature PR [#602](https://github.com/each4all/agentic-plugins/pull/602), a rebase-merged cross-package pair: `5b840bd` recording the S9-gate SessionStart re-validation on Claude `2.1.215` ahead of registration, then `9460682` shipping the attention entry sensor + the `floors.entry_brief=0.83.0` pin; release PR [#603](https://github.com/each4all/agentic-plugins/pull/603) squash `0f8d84f`, tag `plugin-attention-v0.7.0`, marketplace sync `b0db587`; attention upgraded to `0.7.0` on **both** hosts; Claude Code `2.1.215` / `codex-cli 0.144.6`): `overall` `pass` with **zero warnings**, all three execute proofs (permission proof, deep-peer-smoke, workflow-continuation proof) executed and passed in **both** directions, `host_parity_baseline` **`current`** — the prior record's baseline-stale follow-up was closed inside the same recovery slice (compat run `compat-20260720T104815Z-9323ec`, content-backed release notes for BOTH drifted hosts — Claude `2.1.208`→`2.1.215`, Codex `0.144.1`→`0.144.6`, all additive/fix-only — then a re-snapshot `compat-20260720T105414Z-87af5e` reading `drift=none`) — and the doctor `entry_brief` / `session_capture` readiness sections both read `off` (the shipped defaults; the honest informational state before any operator opt-in). **Experience parity deliberately reads `partial` `91%` (manual follow-up `codex-hook-review`)**: the currency evaluation still refuses the pre-S8a4 four-plugin attestation `settings-20260713T234950Z-f08600` (`currency_reason=codex_cli_version_changed`); attention `0.7.0` changed only the **Claude** hook surface (the SessionStart entry sensor) and keeps zero Codex hook surface, so the trusted Codex hook entries are unchanged and a fresh operator `/hooks` confirmation plus `runtime:settings --attest-codex-hook-review` restores `ready`. **That restoration landed the same day**: the operator's `/hooks` confirmation produced the fresh four-plugin attestation `settings-20260720T151554Z-3b543f` (designer@0.3.0 / engineer@0.21.0 / founder@0.4.0 / orchestrator@0.13.0, `bound_versions.codex=0.144.6` — the version binding the pre-S8a4 artifact lacked), and the post-attestation record `doctor-20260720T151637Z-e2e061` restores observed parity to `ready` (`100%`, zero manual follow-ups, every criterion satisfied) with all three execute proofs re-passed in both directions and `host_parity_baseline` still `current`. This supersedes the same-day 0.83.0-native re-record `doctor-20260720T052332Z-a0d677` — the **ADR-0045 entry-time proposal surfaces** loop (feature PRs S6 [#597](https://github.com/each4all/agentic-plugins/pull/597) `932c135` extracting the shared host-localization leaf, S7a [#598](https://github.com/each4all/agentic-plugins/pull/598) `5c6dae8` the entry-brief bounded read layer, S7b [#599](https://github.com/each4all/agentic-plugins/pull/599) `45624bf` the §16 arbiter + pointer-only brief + user-scope-only session keys + context CLI, and S8 [#600](https://github.com/each4all/agentic-plugins/pull/600) `de20853` the snapshot-only dashboard entry advisory + §18 readiness diagnosis; release PR [#596](https://github.com/each4all/agentic-plugins/pull/596) squash `e249ac7`, tags `plugin-runtime-v0.83.0` + `plugin-attention-v0.6.0` (the ADR-0044 S5 attention Stop→publish-session sensor with the publisher floor 0.82.0 pin, [#595](https://github.com/each4all/agentic-plugins/pull/595) `3b8ed8d`), marketplace sync `360a71f`; both host plugin lists report runtime `0.83.0` enabled (claude=installed/0.83.0, codex=installed/0.83.0); Claude Code `2.1.215` / `codex-cli 0.144.1`): `overall` `pass`, Codex hook state `12/12` expected `enabled_trusted` with `unexpected_agentic_entries=2` (the retained pre-relocation attention rows, display-only), the new doctor `entry_brief` readiness section reads `off` against the installed binary (the shipped default — the honest informational state before any operator opt-in, mirroring the `session_capture` `off` precedent), and all three execute proofs (permission proof, deep-peer-smoke, workflow-continuation proof) executed and passed in **both** directions — with the honest `host_parity_baseline` `stale` caveat (Claude Code `2.1.208`→`2.1.215` drift after the 2026-07-14 baseline; tracked to a baseline-refresh follow-up slice per the 0.77.2 / attention-0.4.1 precedents). **Experience parity deliberately reads `partial` `91%` (manual follow-up `codex-hook-review`)**: the S8a4-hardened currency evaluation still refuses to prove the pre-S8a4 four-plugin attestation `settings-20260713T234950Z-f08600` current (`bound_versions.codex=null` → `currency_reason=codex_cli_version_changed`); neither released package changed a Codex hook-bearing plugin — runtime ships no hooks and attention exposes zero Codex hook surface — so the trusted hook entries are unchanged, and a fresh operator `/hooks` confirmation plus `runtime:settings --attest-codex-hook-review` restores `ready`. This supersedes the 0.82.0-native re-record `doctor-20260719T071752Z-6392f1` (2026-07-19Z; the 0.82.0 release itself shipped the ADR-0044 session-capture exit-side stack S2–S4, while that record's loop narrative below retained the machine-bootstrap headline) — the **ADR-0046 S8 machine-bootstrap stack** loop (feature commits S8a1 [#576](https://github.com/each4all/agentic-plugins/pull/576) `267cd33` hardening the existing doctor/settings production surfaces, S8a2 `5af386e` machine-bootstrap core + schemas, S8a4 `29bb5b1` repairing the §8.2 Codex `/hooks` attestation dead pipe end-to-end, S8a5 [#581](https://github.com/each4all/agentic-plugins/pull/581) `8712eb9` per-handler Codex hook-disabled evidence, and S8b [#582](https://github.com/each4all/agentic-plugins/pull/582) `960c8bc` wiring `runtime:bootstrap` as the tenth runtime command; release PR [#577](https://github.com/each4all/agentic-plugins/pull/577) squash `7adb9ef`, tag `plugin-runtime-v0.81.0`, marketplace sync `769b527`; the installed runtime was upgraded to `0.81.0` on **both** hosts, and both hosts report runtime `0.81.0` (claude=installed/0.81.0, codex=installed/0.81.0); Claude Code `2.1.214` / `codex-cli 0.144.1`): `overall` `pass`, Codex hook state `12/12` expected `enabled_trusted` with `unexpected_agentic_entries=2` (the retained pre-relocation attention rows, display-only), and all three execute proofs (permission proof, deep-peer-smoke, workflow-continuation proof) executed and passed in **both** directions. **Experience parity deliberately reads `partial` (6/8 satisfied; manual follow-up `codex-hook-review`)**: the S8a4-hardened currency evaluation refuses to prove the pre-S8a4 four-plugin attestation `settings-20260713T234950Z-f08600` (designer@0.3.0 / engineer@0.21.0 / founder@0.4.0 / orchestrator@0.13.0) current — the artifact predates the canonical `bound_versions` capture (`bound_versions.codex=null`), so it evaluates `stale` (`currency_reason=codex_cli_version_changed`) against `codex-cli 0.144.1` instead of being silently trusted — the fail-closed half of contract §11.2 #24 observed live: attestation currency binds the Codex CLI and the hook-bearing plugin versions, and the legacy null binding is refused rather than trusted (the hook-bearing-upgrade invalidation half is unit-pinned in S8a2 and demonstrated by the recorded designer/founder upgrade precedents); the trusted hook entries themselves are unchanged — this runtime-only release changed no hook-bearing plugin — and a fresh operator `/hooks` confirmation plus `runtime:settings --attest-codex-hook-review` restores `ready`. Stored-proof invalidation-by-design was equally observed live: before re-record, the prior record evaluated `not_reusable` with explicit reasons (runtime version/source/claude_cache/codex_installed `recorded=0.80.1` → `current=0.81.0`, plus Claude Code `recorded=2.1.209` → `current=2.1.214`). The S8c released-package acceptance additionally exercised the installed 0.81.0 from a consumer repo: `bootstrap.mjs` `status`/`verify` returned the contract §3.1 exit-30 `no-active-run` semantics, a full `plan --bundle base` → `status` → `abandon` lifecycle ran as `bootstrap-20260718T081054Z-9be0cf` (24 expected steps); a full installed `settings.mjs` probe run from the consumer repo planned zero source-tree catalog remediations — the S8a1 regression observed against the released package — while the probe-free `--skip-host-cli-probes` mode separately returned its discriminated `report_scope=local_plan` R0 report; and a consumer-root deep-peer-smoke passed in both directions selecting the **installed** companions `0.4.0` from both host caches (no workspace scripts on the discovery ladder) — with the honest `host_parity_baseline` `stale` caveat (Claude Code `2.1.208`→`2.1.214` drift after the 2026-07-14 baseline; tracked to a baseline-refresh follow-up slice per the 0.77.2 / attention-0.4.1 precedents). This supersedes the **S1 permission-advisor defect-class fix** record `doctor-20260714T235550Z-60336c` against the then-installed runtime `0.80.1`, whose loop (feature PR [#573](https://github.com/each4all/agentic-plugins/pull/573) squash-merged `89c16ad` — closing the permission-advisor defect class: secret leak, danger-rule bypass, cross-bucket governance, and their two mirrors; release PR [#574](https://github.com/each4all/agentic-plugins/pull/574) squash `6d82617`, tag `plugin-runtime-v0.80.1`, marketplace sync `160b3d8`; the installed runtime was upgraded to `0.80.1` on **both** hosts, and both hosts report runtime `0.80.1` (claude=installed/0.80.1, codex=installed/0.80.1); the runtime-only patch changed no hook-bearing plugin — runtime ships no hooks — so no `/hooks` re-attestation was required and the four-plugin attestation `settings-20260713T234950Z-f08600` (designer@0.3.0 / engineer@0.21.0 / founder@0.4.0 / orchestrator@0.13.0) stays current; Claude Code `2.1.209` / `codex-cli 0.144.1`): experience parity **`ready` `100%` 8/8** with zero manual follow-ups, `overall` `pass`, Codex hook state `12/12` expected `enabled_trusted` with `unexpected_agentic_entries=2` (the retained pre-relocation attention rows, display-only), and all three execute proofs (permission proof, deep-peer-smoke, workflow-continuation proof; `source=current-run`) passed in **both** directions — with the honest `host_parity_baseline` `stale` caveat (Claude Code `2.1.208`→`2.1.209` patch drift after the 2026-07-14 baseline; tracked to a baseline-refresh follow-up slice per the 0.77.2 / attention-0.4.1 precedents). This supersedes the **ADR-0043 §3 attention four-persona sensor** record `doctor-20260714T021309Z-0cd852` against the then-installed runtime `0.80.0`, whose loop (feature PR [#568](https://github.com/each4all/agentic-plugins/pull/568) rebase-merged as `4764811` — `SENSOR_PERSONAS` spanning all four personas with per-persona candidate homes (founder/designer canonical-only), the documented marker-shape table with an unknown-persona fail-close, the `marker.at` transition-anchored freshness gate with bidirectional bounds + `FUTURE_SKEW_MS` and ISO-UTC lexical validation, persona-aware headline omission for the manually-published personas' `blocked` state (usually publish-needed — never a wrong token), and the `emitTerminalEvents` monotonic-deadline full-slot batcher; the working-diff Codex review's two HIGH findings (future-timestamp freshness bypass; budget overrun + partial-slot dispatch kill) were applied with regressions before merge; release PR [#569](https://github.com/each4all/agentic-plugins/pull/569) squash `b72f28b`, tag `plugin-attention-v0.5.0`, marketplace sync `a7ca00f`; installed refresh via the settings executor `settings-20260714T021101Z-94f0f6`, Claude `update-plugin` + Codex `upgrade-marketplace` both exit 0, both hosts report attention `0.5.0`; attention exposes zero Codex hook surface, so no `/hooks` re-attestation was required and the four-plugin attestation `settings-20260713T234950Z-f08600` stayed current; Claude Code `2.1.208` / `codex-cli 0.144.1`): experience parity **`ready` `100%` 8/8** with zero manual follow-ups, `overall` `pass`, Codex hook state `12/12` expected `enabled_trusted` with `unexpected_agentic_entries=2` (the retained pre-relocation attention rows, display-only), and all three execute proofs passed in **both** directions — with its own honest `host_parity_baseline` `stale` caveat (Claude Code `2.1.207`→`2.1.208` patch drift after the 2026-07-11 baseline; tracked to a same-day baseline-refresh follow-up slice per the 0.77.2 / attention-0.4.1 precedents). Its **ADR-0043 §3 live-fire gate** was observed against the installed set: a real `claude -p` turn's Stop in a throwaway repo carrying REAL installed-founder (`0.4.0`) and installed-designer (`0.3.0`) terminal transitions — their sidecars rendered footers and upgraded the dated markers — emitted **two `workflow-terminal` events, one per persona, through the installed attention `0.5.0` sensor into the runtime `file-log` channel** with per-machine host-woven event ids and TTL dedupe claims, and no bare `turn-complete` flood (the enrichment suppressed it by design). This supersedes the **ADR-0043 S4 designer footer-onboarding** record `doctor-20260713T235039Z-a35cb7`, whose loop (feature PR [#565](https://github.com/each4all/agentic-plugins/pull/565) rebase-merged as `2df14d5` — the designer terminal-handoff sidecar + completion-footer onboarding mirroring the founder S3 reference under copy-not-import, with the Plan-verify peer's 7 applied findings (rename-artifact and stale pre-S4 wording fixes, runbook §Scope-honesty documentation of the cross-workflow LWW re-render window, command-carrying compact next-action defaults, a multiline-static-import hardening of the acceptance boundary scan, and extended runbook pins) and the marker/slot concurrency family adjudicated to the ADR-0043 §2 last-writer-wins acceptance per the S3 precedent, completing the four-persona acceptance matrix; release PR [#566](https://github.com/each4all/agentic-plugins/pull/566) squash `9e6c9e5`, tag `plugin-designer-v0.3.0`, marketplace sync `176478f`; installed refresh via the settings executor `settings-20260713T152632Z-faeb49`, Claude `update-plugin` + Codex `upgrade-marketplace` both exit 0, both hosts report designer `0.3.0`; the hook-bearing designer upgrade (four hook files changed) version-invalidated the prior four-plugin attestation `settings-20260713T092639Z-52bd42`, and the operator's fresh `/hooks` confirmation was recorded as `settings-20260713T234950Z-f08600` (designer@0.3.0 / engineer@0.21.0 / founder@0.4.0 / orchestrator@0.13.0) **before** this proof; Claude Code `2.1.207` / `codex-cli 0.144.1`): experience parity **`ready` `100%` 8/8** with zero manual follow-ups, `overall` `pass` with `host_parity_baseline` `current`, Codex hook state `12/12` expected `enabled_trusted` with `unexpected_agentic_entries=2` (the retained pre-relocation attention rows, display-only), and all three execute proofs passed in **both** directions. The **ADR-0043 §5 per-persona e2e gate** is now discharged for **all four personas**: unpinned installed-cache discovery on each host's designer `0.3.0` rendered a real completion footer on a `set-terminal` terminal transition — `completion state: publish-needed` with host-localized routing (`/designer:resume` on Claude, `$designer:resume` on Codex), generic-marker-free per the completion-output contract §3.2 — closing the designer half of the install + re-attest + observe gate (the founder half was discharged by the S3 record below). This supersedes the **ADR-0043 S3 founder footer-onboarding** record `doctor-20260713T092820Z-0efc74`, whose loop (feature PR [#562](https://github.com/each4all/agentic-plugins/pull/562) rebase-merged as `578936b`/`cdd246d` per ADR-0016 two-package routing — the founder terminal-handoff sidecar + completion-footer onboarding with the Plan-verify peer's 11 applied findings (rendered-tombstone + primary/backstop origin split, immutable snapshot render, publish-needed mapping under the completion-output contract §2, dual 0.79.0 footer / 0.71.0 notify discovery floors) plus the engineer `entry-routing-contract.md` four-persona fold; release PR [#563](https://github.com/each4all/agentic-plugins/pull/563) squash `cbb282e`, tag `plugin-founder-v0.4.0`, marketplace sync `6fa5adf`; installed refresh via the settings executor `settings-20260713T092325Z-8faf07`, Claude `update-plugin` + Codex `upgrade-marketplace` both exit 0, both hosts report founder `0.4.0`; the hook-bearing founder upgrade (four hook files changed) version-invalidated the prior four-plugin attestation `settings-20260713T030937Z-f50815`, and the operator's fresh `/hooks` confirmation was recorded as `settings-20260713T092639Z-52bd42` (designer@0.2.1 / engineer@0.21.0 / founder@0.4.0 / orchestrator@0.13.0) **before** this proof; Claude Code `2.1.207` / `codex-cli 0.144.1`): experience parity **`ready` `100%` 8/8** with zero manual follow-ups, `overall` `pass` with `host_parity_baseline` `current`, Codex hook state `12/12` expected `enabled_trusted` with `unexpected_agentic_entries=2` (the retained pre-relocation attention rows, display-only), and all three execute proofs passed in **both** directions. The **ADR-0043 §5 per-persona e2e gate** was observed against the same installed set: unpinned installed-cache discovery on each host's founder `0.4.0` rendered a real completion footer on a `set-terminal` terminal transition — `completion state: publish-needed` with host-localized routing (`/founder:resume` on Claude, `$founder:resume` on Codex), generic-marker-free per the completion-output contract §3.2 — discharging the founder half of the ADR-0043 §5 install + re-attest + observe gate (designer's half was discharged by the S4 record above). This supersedes the **S9 completion-output-contract** record `doctor-20260713T025136Z-7758d3`, whose loop (feature PR [#558](https://github.com/each4all/agentic-plugins/pull/558) rebase-merged as `00dbc80`/`85bbd3d`/`4c8e59f` per ADR-0016 three-package routing; release PR [#559](https://github.com/each4all/agentic-plugins/pull/559) squash `1d02390`, tags `plugin-runtime-v0.80.0` + plugin-engineer-v0.21.0 + plugin-orchestrator-v0.13.0, marketplace sync `ddb43d4`; installed refresh via the settings executor `settings-20260713T025027Z-8f625a`, 4 update commands executed / 0 failed, both hosts report runtime `0.80.0` with engineer `0.21.0` / orchestrator `0.13.0`; Claude Code `2.1.207` / `codex-cli 0.144.1`): `overall` `pass` with `host_parity_baseline` `current` against the 2026-07-11 baseline, Codex hook state `12/12` expected `enabled_trusted` with `unexpected_agentic_entries=2` (the retained pre-relocation attention rows, display-only), and all three execute proofs passed in **both** directions. **Experience parity deliberately reads `partial` `91%`** (manual follow-up `codex-hook-review`): the engineer/orchestrator hook-bearing upgrades version-invalidated the four-plugin `/hooks` attestation `settings-20260712T015100Z-312fbb` (attested at engineer 0.20.1 / orchestrator 0.12.1), so a fresh operator `/hooks` confirmation plus `runtime:settings --attest-codex-hook-review` is required before parity reads `ready` again — the trusted hook entries themselves are unchanged (no hook file changed in this release, so trust hashes still match). The operator completed that `/hooks` confirmation the same day: the fresh four-plugin attestation `settings-20260713T030937Z-f50815` (designer@0.2.1 / engineer@0.21.0 / founder@0.3.1 / orchestrator@0.13.0) restored observed parity in the post-attestation record `doctor-20260713T030956Z-20dcc3` — **`ready` `100%` 8/8** with zero manual follow-ups, `overall` `pass`, baseline `current`, and all three execute proofs re-passed in **both** directions against the same installed 0.80.0 set. This supersedes the 0.79.0 four-persona-seam record re-recorded on 2026-07-12Z as `doctor-20260712T080638Z-005af5` — that loop (feature PR [#555](https://github.com/each4all/agentic-plugins/pull/555) squash `cb720e7`, behind ADR authoring PR [#553](https://github.com/each4all/agentic-plugins/pull/553) and Accepted-flip PR [#554](https://github.com/each4all/agentic-plugins/pull/554); release PR [#556](https://github.com/each4all/agentic-plugins/pull/556) squash `558f78a`, tag `plugin-runtime-v0.79.0`, marketplace sync `8ca4651`; installed refresh via the settings executor `settings-20260712T080408Z-5b98f9`, Claude `update-plugin` + Codex `upgrade-marketplace` both exit 0, both hosts report runtime `0.79.0`; Claude Code `2.1.207` / `codex-cli 0.144.1`): experience parity **`ready` `100%` 8/8** and `overall` `pass` with `host_parity_baseline` `current` against the 2026-07-11 baseline (the #550 refresh), Codex hook state `12/12` expected `enabled_trusted` with `unexpected_agentic_entries=2` (the retained pre-relocation attention rows, display-only), the four-plugin `/hooks` attestation `settings-20260712T015100Z-312fbb` (attested 2026-07-12T01:51Z) still current — runtime ships no hooks, so this upgrade did not change the hook-bearing set — and all three execute proofs passed in **both** directions. This supersedes the attention `0.4.1` relocation record, re-recorded on 2026-07-11Z as `doctor-20260711T045954Z-731e34` against the then-installed runtime 0.78.1 — the relocation loop (fix PR [#546](https://github.com/each4all/agentic-plugins/pull/546) `ceb2fb9` + docs PR [#547](https://github.com/each4all/agentic-plugins/pull/547) `1d20f82` + release PR [#548](https://github.com/each4all/agentic-plugins/pull/548), tag `plugin-attention-v0.4.1`, sync `553ac79`; installed refresh via the settings executor `settings-20260711T045604Z-075b26`, both hosts report `0.4.1`; Claude Code `2.1.207` / `codex-cli 0.144.1`): experience parity **`ready` `100%` 8/8** and `overall` `pass` — the #543 command-portability gate cleared because the relocated attention supplies neither Codex discovery input (`effective.status=not_packaged` on the installed cache), Codex hook state reads **12/12** `enabled_trusted` with `unexpected_agentic_entries=2` (retained pre-relocation attention rows, display-only; host did not prune, runtime non-mutation hash-verified), the four-plugin `/hooks` attestation `settings-20260711T045915Z-5ca22a` reads current, the Claude-side manifest-declared registration is live-fire proven (file-log `turn-complete` from a `claude -p` turn on the installed 0.4.1), and all three execute proofs passed in **both** directions — with the honest `host_parity_baseline` `stale` caveat (Claude Code `2.1.206`→`2.1.207` patch drift after the 2026-07-10 baseline; closed the same day by the #550 refresh to the 2026-07-11 baseline, per the 0.77.2 precedent). That record in turn superseded the 0.78.1-native 2026-07-10Z record `doctor-20260710T153802Z-276226` (Claude Code `2.1.206` / `codex-cli 0.144.1`, runtime installed `0.78.1` on **both** hosts — the installed-state refresh ran through the `runtime:settings --execute-plugin-management` executor, artifact `settings-20260710T153653Z-a8e721`, Claude `update-plugin` + Codex `upgrade-marketplace` both exit 0; all three execute proofs `passed` in **both** directions; `host_parity_baseline` `current`). Codex hook state now reads `14/14` expected bundled hooks `enabled_trusted` with `unexpected_agentic_entries=0` — attention's trusted `stop`/`subagent_stop` entries sit **inside** doctor's expected set (the #543 fix working as designed) and Claude's `Notification`, which current Codex never materializes, is surfaced as `unmapped=1` instead of a permanently-missing expectation. The `/hooks` re-attestation `settings-20260710T153728Z-5796b6` covers all **five** bundled hook-bearing plugins (`attention` `0.4.0` / `designer` `0.2.0` / `engineer` `0.20.0` / `founder` `0.3.0` / `orchestrator` `0.12.0`) and reads current — the prior 4-plugin attestation `settings-20260709T141913Z-8b7122` was invalidated as `plugin_set_changed` by design. **Experience parity deliberately reads `partial` `95%` (7/8 satisfied, weight 109/115) and `overall` reads `warning`**: the same release added the command-portability parity gate, so `lifecycle_hook_continuity` holds `partial` with `command-warnings=attention` (bare `node` + Claude-adapter command shape) until the attention package resolves its Codex posture — portable wrappers, manifest declaration, or restructure, tracked in `plugins/runtime/docs/follow-ups.md` (resolved 2026-07-11 by **restructure**: the Claude registration relocated to a manifest-declared `adapters/claude/hooks/hooks.json` with no root default file; the install proof landed the same day as `doctor-20260711T045954Z-731e34` — see the newest record above). The criterion got stricter; the host did not regress — every surface scored before #543 still passes, and a fresh attestation can no longer launder the warning into a 100% score. Fix PR [#543](https://github.com/each4all/agentic-plugins/pull/543) folded attention into doctor's expected Codex hook sets after host truth disproved the claude-adapter-only exclusion premise (Codex 0.144.1 default-file discovery is command-shape-blind), and its Codex Refine-verify pass surfaced and fixed the cache-only versioned-path matching blocker plus attestation version currency; release PR [#544](https://github.com/each4all/agentic-plugins/pull/544) cut tag `plugin-runtime-v0.78.1` with marketplace sync commit `e8d8fdc`, and the proof above was taken against that released binary, not a patched working tree. This record supersedes the 0.78.0 record `doctor-20260710T135955Z-d752f5` (`ready` `100%` 8/8 under the pre-#543 criterion; its loop was feature PR [#540](https://github.com/each4all/agentic-plugins/pull/540) probe-free `--skip-host-cli-probes` settings with contract PR [#539](https://github.com/each4all/agentic-plugins/pull/539) + release PR [#541](https://github.com/each4all/agentic-plugins/pull/541), tag `plugin-runtime-v0.78.0`, sync `51db10f`; its honest inventory-lag note — 2 trusted attention entries outside the expected set — became fix #543), and before it the 0.77.2 record `doctor-20260710T044745Z-1a789e` — identical pass results whose honest `host_parity_baseline` `stale` caveat was closed the same day by the baseline-refresh slice (`compat-20260710T054356Z-34315e` ingest, post-refresh `drift: none` `compat-20260710T104459Z-67ece6`); that record's own loop was fix PR [#534](https://github.com/each4all/agentic-plugins/pull/534) + release PR [#535](https://github.com/each4all/agentic-plugins/pull/535), tag `plugin-runtime-v0.77.2`, sync `e351888`, preserved as-recorded — the 0.77.1 record `doctor-20260709T141930Z-515ebf`, the 0.76.0 loop `doctor-20260707T140348Z-5a8fb8`, and the intermediate 0.77.0 record `doctor-20260709T131625Z-33c54d`, which read `partial` `91%` because doctor mis-classified designer’s newly-trusted Codex hooks as disabled — an absent `enabled` key means enabled, `/hooks` exposes no enable toggle, and the attestation executor is fail-closed while any expected entry is disabled, so no hook-bearing plugin trusted after the ADR-0035 §6 writer removal could be attested. designer was the first such plugin; its `Stop` hook demonstrably fired and archived a terminal designer workflow during a `codex exec` turn while doctor called it disabled. Fix PR [#530](https://github.com/each4all/agentic-plugins/pull/530) corrected the classifier (only an explicit `enabled = false` disables) and recorded the observed Codex semantics in `plugins/runtime/docs/codex-capability-baseline.md`; release PR [#531](https://github.com/each4all/agentic-plugins/pull/531) cut tag `plugin-runtime-v0.77.1` with marketplace sync commit `73b2a75`, and that release's own `ready` `100%` proof was likewise taken against a released binary, not a patched working tree. The preceding ADR-0042 releases — feat PR [#528](https://github.com/each4all/agentic-plugins/pull/528) (designer in the doctor/settings inventory) and PR [#529](https://github.com/each4all/agentic-plugins/pull/529) (ADR-0042 `Accepted`, `plugin-designer-v0.2.0`) via release PR [#521](https://github.com/each4all/agentic-plugins/pull/521), sync commit `7dce7fe` — bumped the installed runtime and closed the prior proof’s reuse gate. Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. The explicit engineer dispatch proof satisfies ADR-0012 condition 2; ADR-0013 remains future command-surface integration work, not a blocker for this condition.** |
