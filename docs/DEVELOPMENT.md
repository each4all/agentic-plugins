...
> `plugin-runtime` v0.27.2, runtime ships `doctor`, `settings` with
> explicit plugin-management execution artifacts, semantic failure
> classification for unavailable host plugin surfaces, Claude plugin
> command-surface preflight/blocking, narrow Codex `plugin_hooks`
> host-config apply, consensus artifacts
