...
- **Runtime/operator control plane (ADR-0024, Accepted)** is the immediate Stage 3+ dogfood target. The first implementation PR introduces `plugins/runtime` as an L1 framework primitive with read-only `runtime:doctor` diagnostics for host/plugin/auth/model/effort/sandbox readiness and workflow/peer-run ledger health. `runtime:settings` (dry-run-first install/config assistance), dynamic peer fanout and consensus loops, context-hygiene policy, and completion footers remain follow-up PRs.
