...
> `plugin-runtime` v0.93.0 (the ADR-0048 bootstrap-observability
...
| 2 | engineer guarantees bidirectional companion round-trip | satisfied | Claude direction on engineer's own code: ✓ (D Phase 5 dispatch-peer parallel-review). Codex direction was historically pending on a Codex CLI commands auto-trigger path, with ADR-0013 retained as the future host-native command integration candidate. **2026-05-06 Stage 2.5+ exit audit Q4 PASS adds substantial evidence (128 unit + 4 smoke 양방향 실 LLM round-trip + JSON envelope wire validation per `companions/contract.md` §4.2). [ADR-0018](adr/0018-stage3-architecture-orchestrator-and-branch-context.md) §sub-5 ✅ PR [#54](https://github.com/each4all/agentic-plugins/pull/54) ships the cross-host integration test contract (claude→codex / codex→claude in-process state.mjs round-trip + stop-archive subprocess + standalone `.github/workflows/cross-host-tests.yml` CI matrix slot). 2026-05-16 satisfied transition. Latest installed proof: `plugin-runtime` `0.93.0` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded on 2026-08-25Z as `doctor-20260825T070411Z-d6ecb2` — the post-release record of the **portable-profile session-config** loop, which shipped one runtime slice: the machine profile's 1.1 → 1.2 carriage of the session-config family (`session_capture`, `entry_brief`, `entry_brief_empty`) as trailing bare scalars, with a `projectSession` user-global reader and seed proposals carrying a runtime-derived `user_scope_only` label `a973d05` (PR [#734](https://github.com/each4all/agentic-plugins/pull/734)), released as PR [#735](https://github.com/each4all/agentic-plugins/pull/735) squash `9be7501`, tag `plugin-runtime-v0.93.0`, marketplace sync `c526efe`, stage-doc sync `4ac05c9`. All three execute proofs pass in both directions (`permission_proof`, `deep_peer_smoke`, `workflow_continuation_proof`) with `host_parity` `pass` under Claude Code `2.1.243` / Codex `0.149.1`. `overall` reads **`warning`**: `hard_failures` is empty and the one warning is "latest compatibility check needs follow-up" — no hard failures is not the same claim as `pass`. Observed experience parity is `partial` `81%` (105/130, 5 satisfied / 3 partial / 1 not-verified) with **zero blocked** criteria and one manual follow-up, `codex-hook-review`: this release touched no Codex hook-bearing plugin (runtime ships no hooks), so the trusted entries are unchanged and the follow-up is the owner-gated `/hooks` re-attestation the Codex CLI offers no non-interactive query for. `host_parity_baseline` reads `stale` with the 2026-08-16 `2.1.233` / `0.147.0` baseline against an observed `2.1.243` / `0.149.1` — refreshing it is protected-asset work this recovery is forbidden to do, so it stays a named follow-up rather than a silently-carried claim.
