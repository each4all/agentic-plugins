...
> `plugin-runtime` v0.71.0 (ADR-0040 operator observability: the
> notify-schema contract lib, `notify_*` settings keys, the `notify.mjs`
> emitter, `--notification-plan` Codex fragments, and the
> `runtime:dashboard` Tier 1+2 aggregate view), runtime ships `doctor`, `settings` with
...
| 2 | engineer guarantees bidirectional companion round-trip | satisfied | Claude direction on engineer's own code: ✓ (D Phase 5 dispatch-peer parallel-review). Codex direction was historically pending on a Codex CLI commands auto-trigger path, with ADR-0013 retained as the future host-native command integration candidate. **2026-05-06 Stage 2.5+ exit audit Q4 PASS adds substantial evidence (128 unit + 4 smoke 양방향 실 LLM round-trip + JSON envelope wire validation per `companions/contract.md` §4.2). [ADR-0018](adr/0018-stage3-architecture-orchestrator-and-branch-context.md) §sub-5 ✅ PR [#54](https://github.com/each4all/agentic-plugins/pull/54) ships the cross-host integration test contract (claude→codex / codex→claude in-process state.mjs round-trip + stop-archive subprocess + standalone `.github/workflows/cross-host-tests.yml` CI matrix slot). 2026-05-16 satisfied transition. Latest installed proof: `plugin-runtime` `0.71.0` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded under the 0.71.0 install on 2026-07-04 as `doctor-20260704T071736Z-ead466` (Claude Code `2.1.201` / `codex-cli 0.142.5`; engineer `0.19.0` / orchestrator `0.11.0` / founder `0.2.0` / image `0.2.0`; experience parity `ready` `100%`, 8/8 satisfied — the 2026-07-02 `/hooks` review attestation `settings-20260702T060856Z-c8cfdd` remains current for the unchanged installed hook-bearing set engineer `0.19.0` / orchestrator `0.11.0` / founder `0.2.0`, doctor hook-state reports all 9 bundled hook entries configured+enabled with `untrusted=0`, and `image`/`runtime` ship no hooks), superseding the 0.70.1 record `doctor-20260702T060924Z-a61f92` (Claude Code `2.1.198` / `codex-cli 0.142.5`, parity `ready` `100%`) after the 0.71.0 release train (ADR-0040 operator-observability slices [#473](https://github.com/each4all/agentic-plugins/pull/473) notify-schema / [#475](https://github.com/each4all/agentic-plugins/pull/475) `notify_*` settings keys / [#476](https://github.com/each4all/agentic-plugins/pull/476) emitter / [#477](https://github.com/each4all/agentic-plugins/pull/477) reader-lib extraction / [#478](https://github.com/each4all/agentic-plugins/pull/478) `--notification-plan` / [#479](https://github.com/each4all/agentic-plugins/pull/479) `runtime:dashboard`, release PR [#474](https://github.com/each4all/agentic-plugins/pull/474)) bumped the installed runtime and the Claude Code `2.1.198` → `2.1.201` host drift closed that proof's reuse gate. Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. The explicit engineer dispatch proof satisfies ADR-0012 condition 2; ADR-0013 remains future command-surface integration work, not a blocker for this condition.** |
