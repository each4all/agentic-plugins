...
> `plugin-runtime` v0.40.0, runtime ships `doctor`, `settings` with
...
> planning with operator-explicit URL fetch via `--fetch-release-notes-url`,
...
> auditing with explicit gate and unresolved-row details, runtime artifact
> inventory, and an
