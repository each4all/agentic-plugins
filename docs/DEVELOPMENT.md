...
> `plugin-runtime` v0.28.0, runtime ships `doctor`, `settings` with
> observed experience-parity scoring, explicit plugin-management execution artifacts, semantic failure
...
> manual follow-up checklists for host-native `/plugin ...`
> install/update/cleanup commands surfaced from both settings and doctor,
> Codex `/hooks` review/trust manual follow-ups when packaged hooks are ready,
> narrow Codex `plugin_hooks` host-config apply, consensus artifacts
