...
> `plugin-runtime` v0.72.0 (ADR-0040 operator observability: the
...
| 2 | engineer guarantees bidirectional companion round-trip | satisfied | Claude direction on engineer's own code: ✓ (D Phase 5 dispatch-peer parallel-review). Codex direction was historically pending on a Codex CLI commands auto-trigger path, with ADR-0013 retained as the future host-native command integration candidate. **2026-05-06 Stage 2.5+ exit audit Q4 PASS adds substantial evidence (128 unit + 4 smoke 양방향 실 LLM round-trip + JSON envelope wire validation per `companions/contract.md` §4.2). [ADR-0018](adr/0018-stage3-architecture-orchestrator-and-branch-context.md) §sub-5 ✅ PR [#54](https://github.com/each4all/agentic-plugins/pull/54) ships the cross-host integration test contract (claude→codex / codex→claude in-process state.mjs round-trip + stop-archive subprocess + standalone `.github/workflows/cross-host-tests.yml` CI matrix slot). 2026-05-16 satisfied transition. Latest installed proof: `plugin-runtime` `0.72.0` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded under the 0.72.0 install on 2026-07-04 as `doctor-20260704T171153Z-aea7a1` (Claude Code `2.1.201` / `codex-cli 0.142.5`; engineer `0.20.0` / orchestrator `0.12.0` / founder `0.3.0` / attention `0.2.0` / image `0.2.0`; experience parity `ready` `100%`, 8/8 satisfied — the `/hooks` review attestation was refreshed for the bumped hook-bearing set as `settings-20260704T170801Z-b66656`, doctor hook-state reports all 9 bundled Codex hook entries [engineer `0.20.0` / orchestrator `0.12.0` / founder `0.3.0`] configured+enabled+trusted with `untrusted=0`, and `attention` `0.2.0` is recognized as a deliberately Claude-hook-only plugin [ADR-0040 §3] — this change fixes the 0.72.0 doctor so a Claude-only hook plugin is classified `claude_adapter_only` rather than a false Codex packaging gap, restoring parity `ready` `100%`), superseding the 0.71.0 record `doctor-20260704T071736Z-ead466` (Claude Code `2.1.201` / `codex-cli 0.142.5`, parity `ready` `100%`) after the 0.72.0 release train (ADR-0040 §3 hook-only `plugins/attention` [#482](https://github.com/each4all/agentic-plugins/pull/482) + §5 peer-run terminal self-sensor ×3 persona [#484](https://github.com/each4all/agentic-plugins/pull/484) + runtime attention recognition, release PR [#483](https://github.com/each4all/agentic-plugins/pull/483)) bumped the installed runtime and closed that proof's reuse gate. Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. The explicit engineer dispatch proof satisfies ADR-0012 condition 2; ADR-0013 remains future command-surface integration work, not a blocker for this condition.** |
