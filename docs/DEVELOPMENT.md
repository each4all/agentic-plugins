...
> `plugin-runtime` v0.5.0, runtime ships `doctor`, `settings`,
> artifact-only consensus, context-hygiene artifacts with explicit
> budget checks and latest-handoff stale metadata, and an advisory
> completion footer helper. PR [#105](https://github.com/each4all/agentic-plugins/pull/105)
> plus release PR [#106](https://github.com/each4all/agentic-plugins/pull/106)
> is recorded as an ADR-0024 dogfood datapoint, but it does not by
...
- **Runtime/operator control plane (ADR-0024, Accepted)** is the immediate Stage 3+ dogfood target. As of `plugin-runtime` v0.5.0, `plugins/runtime` is an L1 framework primitive with shipped `runtime:doctor`, first `runtime:settings`, artifact-only `runtime:consensus`, runtime-owned `runtime:context` artifacts and explicit budget checks, and the advisory completion footer helper with latest-handoff stale metadata. Deferred boundaries remain explicit: no automatic plugin install/update apply mode, no automatic peer execution, no automatic host-session context mutation or compaction, and no raw peer/consensus output in the main session.
