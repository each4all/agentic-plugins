...
> `plugin-runtime` v0.12.0, runtime ships `doctor`, `settings` with
> explicit plugin-management execution artifacts, consensus artifacts
> plus an explicit `execute --execute` companion boundary,
> context-hygiene artifacts with explicit budget checks and
> latest-handoff stale metadata, an explicit workflow-storage migration
> surface, and an advisory completion footer helper. PR
> [#105](https://github.com/each4all/agentic-plugins/pull/105) plus
> release PR [#106](https://github.com/each4all/agentic-plugins/pull/106)
> is recorded as an early ADR-0024 dogfood datapoint; the later consensus
> executor PR [#129](https://github.com/each4all/agentic-plugins/pull/129)
> plus release PR [#130](https://github.com/each4all/agentic-plugins/pull/130)
> records the `plugin-runtime` v0.12.0 state, but these do not by
> themselves satisfy ADR-0012 condition 3. `plugins/designer` remains
...
- **Runtime/operator control plane (ADR-0024, Accepted)** is the immediate Stage 3+ dogfood target. As of `plugin-runtime` v0.12.0, `plugins/runtime` is an L1 framework primitive with shipped `runtime:doctor`, `runtime:settings` with explicit plugin-management execution artifacts, `runtime:consensus` artifacts plus an explicit `execute --execute` companion boundary, runtime-owned `runtime:context` artifacts and explicit budget checks, explicit workflow-storage migration, and the advisory completion footer helper with latest-handoff stale metadata. Deferred boundaries remain explicit: no automatic unbounded consensus loops, no broad host-native config apply mode, no automatic host-session context mutation or compaction, and no raw peer/consensus output in the main session.
