...
> `plugin-runtime` v0.94.0 (the ADR-0048 bootstrap-observability
...
| 2 | engineer guarantees bidirectional companion round-trip | satisfied | Claude direction on engineer's own code: ✓ (D Phase 5 dispatch-peer parallel-review). Codex direction was historically pending on a Codex CLI commands auto-trigger path, with ADR-0013 retained as the future host-native command integration candidate. **2026-05-06 Stage 2.5+ exit audit Q4 PASS adds substantial evidence (128 unit + 4 smoke 양방향 실 LLM round-trip + JSON envelope wire validation per `companions/contract.md` §4.2). [ADR-0018](adr/0018-stage3-architecture-orchestrator-and-branch-context.md) §sub-5 ✅ PR [#54](https://github.com/each4all/agentic-plugins/pull/54) ships the cross-host integration test contract (claude→codex / codex→claude in-process state.mjs round-trip + stop-archive subprocess + standalone `.github/workflows/cross-host-tests.yml` CI matrix slot). 2026-05-16 satisfied transition. Latest installed proof: `plugin-runtime` `0.94.0` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded on 2026-08-27Z as `doctor-20260827T013118Z-e8d064` — the post-release record of the **value-carrying interview** loop, which shipped one runtime slice: the bootstrap interview's `set:<key>=<value-or-unset>` answer form with two Stage-4 value steps (`config.session`, `config.notify_kinds`), the run schema 1.2 → 1.3 semantic bump that arms the future-minor mutator fence, and a `runtime:settings --unset` config-removal operation — the standing decision folded from the `choices[]` ledger rather than held in `steps[].desired`, which version drift clears `f795085` (PR [#739](https://github.com/each4all/agentic-plugins/pull/739)), released as PR [#738](https://github.com/each4all/agentic-plugins/pull/738) squash `479693e`, tag `plugin-runtime-v0.94.0`, marketplace sync `6d53574`, stage-doc sync `443ad73`. All three execute proofs pass in both directions (`permission_proof`, `deep_peer_smoke`, `workflow_continuation_proof`) with `host_parity` `pass` under Claude Code `2.1.247` / Codex `0.150.0`. `overall` reads **`warning`**: `hard_failures` is empty and the one warning is "latest compatibility check needs follow-up" — no hard failures is not the same claim as `pass`. Observed experience parity is `partial` `81%` (105/130, 5 satisfied / 3 partial / 1 not-verified) with **zero blocked** criteria and one manual follow-up, `codex-hook-review`: this release touched no Codex hook-bearing plugin (runtime ships no hooks), so the trusted entries are unchanged and the follow-up is the owner-gated `/hooks` re-attestation the Codex CLI offers no non-interactive query for. `host_parity_baseline` reads `stale` with the 2026-08-16 `2.1.233` / `0.147.0` baseline against an observed `2.1.247` / `0.150.0` — refreshing it is protected-asset work this recovery is forbidden to do, so it stays a named follow-up rather than a silently-carried claim.
