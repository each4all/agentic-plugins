...
**value-carrying interview** loop, a single runtime change carried by one
squash: the bootstrap interview's `set:<key>=<value-or-unset>` answer form
with two Stage-4 value steps (`config.session`, `config.notify_kinds`), the
run schema 1.2 → 1.3 semantic bump that arms the future-minor mutator fence,
and a `runtime:settings --unset` config-removal operation `f795085`
(PR #739); release PR
...
verdict, and ordered so a `--record` failure at 40 does not hide findings
underneath it `16d5c41`
...
match: `diff -r` over the two 0.95.0 caches, excluding Claude's
`.orphaned_at` supersede marker, reports no difference. Both hold 128 files,
the same file set as 0.94.0, added and removed alike none, because this
...
**The whole-tree digest below is a new measurement, not a continuation of the
old one.** The digests carried by earlier records were operator measurements
whose recipe was never written down, and no script in this repository
reproduces them. Five candidate recipes were run against the 0.94.0 cache —
whose published digest is known, and whose 128-file count reproduces exactly,
so the tree was the right one — and none matched. The digest recorded here is
therefore taken under a recipe stated beside it, and it is NOT comparable with
the digests in records before this one. The recipe is: list every file under
the cache root except the `.orphaned_at` marker, strip the leading `./`, sort
under `LC_ALL=C`, hash each with `shasum -a 256`, and hash the resulting
listing again. It yields
`f106374fcb5819e69c7837db2f5deb94e2bd9d8efea21288468125dc233bdc3c` on both
caches. Carrying the old number forward, or quietly publishing a new one under
an unstated method, would have preserved the appearance of continuity over the
