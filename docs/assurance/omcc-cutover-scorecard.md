...
plugin-management config writes planned after the `plugin-runtime` `0.65.1`
release and installed-state refresh. Current runtime execution evidence is
carried forward by the installed `plugin-runtime` `0.65.1` loop: permission
...
recorded under the 0.61.0 install; the 0.62→0.65 bumps are runtime-only — 0.65.1
routes the cross-script installed-state consumers (settings/cutover/proof-reuse)
through doctor's list-authoritative `resolveCodexInstallState` decision per
ADR-0034 §Scope — and does not touch the peer/dispatch path). The Codex `/hooks` review/trust
...
The latest `plugin-runtime` `0.65.1` release/install proof loop is carried by
feature PR #397 (the ADR-0034 §Scope cross-script installed-state consumer migration) and
release PR #398 (runtime-only; no co-released hook-bearing plugins),
release tag `plugin-runtime-v0.65.1`, marketplace sync commit `f988ade`, and
