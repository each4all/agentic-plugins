...
plugin-management config writes planned after the `plugin-runtime` `0.64.0`
release and installed-state refresh. Current runtime execution evidence is
carried forward by the installed `plugin-runtime` `0.64.0` loop: permission
...
recorded under the 0.61.0 install; the 0.62→0.64 bumps are runtime-only — 0.64.0
adds the ADR-0032 Codex per-plugin command surface recognition (`doctor.mjs`
`codex plugin --help` probe + precise detection, `settings.mjs` recommendation
threading) — and does not touch the peer/dispatch path). The Codex `/hooks` review/trust
...
The latest `plugin-runtime` `0.64.0` release/install proof loop is carried by
feature PR #390 (the ADR-0032 Codex per-plugin command surface recognition) and
release PR #391 (runtime-only; no co-released hook-bearing plugins),
release tag `plugin-runtime-v0.64.0`, marketplace sync commit `fc3b592`, and
