...
plugin-management config writes planned after the `plugin-runtime` `0.68.0`
release and installed-state refresh (Claude `claude plugin update` and Codex
`codex plugin marketplace upgrade` cache materialization both report 0.68.0, 2026-06-22). Current runtime execution evidence is
native to the installed `plugin-runtime` `0.68.0`: permission proof, deep peer
smoke, and workflow continuation proof passed in both directions, re-recorded
on 2026-06-22 as `doctor-20260622T091748Z-4f8cf8` (Claude Code `2.1.185` /
`codex-cli 0.141.0`; engineer `0.17.0` / orchestrator `0.9.0` / founder `0.2.0`) after
the founder `PLUGIN_NAMES` inventory addition (ADR-0036, runtime 0.68.0)
invalidated the prior 0.67.0-install record's carry-forward reuse. The Codex `/hooks` review/trust
...
The latest `plugin-runtime` `0.68.0` release/install proof loop is carried by
feature commit `a526583` (ADR-0036 — recognize founder in the doctor/settings
plugin inventory), release PR #421, release tag `plugin-runtime-v0.68.0`,
marketplace sync commit `0b2019a`, and the 0.68.0-native proof doctor artifact
`doctor-20260622T091748Z-4f8cf8` (re-recorded 2026-06-22). The 0.68.0 founder-inventory change
expanded `PLUGIN_NAMES`, invalidating the prior 0.67.0 proof's carry-forward
...
| R3 | Switching development tools must work in both directions: Claude Code to Codex and Codex to Claude. | Cross-host tests cover resume and stop-archive behavior for engineer/orchestrator; companions exist in both directions. Installed `plugin-runtime` `0.68.0` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded under the 0.68.0 install on 2026-06-22 KST (`doctor-20260622T091748Z-4f8cf8`). Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. | satisfied | Keep the explicit installed-runtime proof in the release/install continuity loop whenever the peer path changes. |
| R4 | Claude Code and Codex user experience must be equivalent where possible; non-portable host-specific features must not become hard dependencies. | Architecture documents host-specific adapter boundaries; runtime baseline documents command/hook/subagent differences; Codex macro/meta skill mirrors exist. The native `runtime:doctor` execute proofs re-recorded under the 0.68.0 install on 2026-06-22 (`doctor-20260622T091748Z-4f8cf8`) confirmed bidirectional peer execution and engineer workflow continuation passed both directions, and `runtime:settings` reports source/cache version parity with zero plugin-management config writes planned. The Codex `/hooks` review/trust attestation recorded on 2026-06-03 (`settings-20260603T072929Z-88b458`; `~/.codex/config.toml` hook-state `untrusted=0`/`disabled=0`) covered the then-bundled `engineer@0.14.0`/`orchestrator@0.8.0` hooks; the hook-bearing set has since bumped to `engineer@0.17.0`/`orchestrator@0.9.0`, so observed experience parity reads `partial` (score `86%`, 1 manual follow-up) until a fresh active-session `/hooks` review plus `runtime:settings --attest-codex-hook-review` lands per this row's gate. | satisfied | Refresh the active-session `/hooks` review/trust attestation after hook-bearing plugin upgrades or hook packaging changes; do not claim observed parity `ready` until the post-attestation doctor/cutover evidence is recorded. |
