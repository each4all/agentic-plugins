...
2026-09-18 Amendment), one package per commit so release-please routed each to
its own release (ADR-0016). A preparation PR moved nothing: it made the shared
...
rather than as one batch. Every release has its catalog-sync commit, so none
needed the `workflow_dispatch` retry path the plan named for a skipped sync.
...
plugin's command files — for ~3,311 always-on tokens (583 + 504 + 551 + 234 +
450 + 989), against 112 / ~15,431 on `2.1.276`; attention and companions read
...
first: it renders no mention, not even of a system skill. This is also the
first observation of runtime follow-up R13 — whether packaged skills stay
invokable under Codex's replaced loader — answered for codex-cli `0.156.1` at
that scope: 55 resolved, one invoked end to end.

⚠ **Resolution was checked everywhere a runbook or registry reads a path.**
All 118 concrete `$CLAUDE_PLUGIN_ROOT` references in the 57 installed command
files (60 distinct paths) resolve inside their caches; resolved against the
new roots, the
...
observed the real current pair, `2.1.276` / `0.155.0`, and reports
...
compatibility run is still `compat-20260828T090343Z-1ee918`, whose `status
current` / `drift none` was taken against `2.1.250` / `0.150.1` and so does
not describe the installed hosts. Doctor also attaches to that run a next step
saying its recorded state `current` "is not one this runtime recognises":
`compatNextSteps` in `plugins/runtime/scripts/lib/state-readers.mjs` has no
branch for `current`, and the 2026-09-10 control emitted the same line under
0.97.1, so it is a standing reader defect rather than a 0.97.2 regression —
recorded here, not fixed by a documentation-only recovery.
...
against `2.1.250` / `0.150.1`. It is still the newest compat run, and it does
not describe the `2.1.268` / `0.154.0` pair the 0.97.2 proof observes.
