...
Current local dry-run evidence: a fresh 2026-07-21Z `runtime:settings` dry-run
(`runtime-settings-1.23`) reports all agentic-plugins surfaces available with
source/cache versions matching the current repo manifest (runtime source
`0.84.0` = installed `0.84.0` on both hosts), and zero plugin-management
config writes planned against the installed `plugin-runtime` `0.84.0` state
(both the Claude plugin cache and `codex plugin list --json` report the
runtime plugin installed and enabled at 0.84.0 — claude=installed/0.84.0,
codex=installed/0.84.0 — with attention 0.8.0 alongside engineer 0.21.0 and
orchestrator 0.13.0, per the 2026-07-21Z `doctor-20260721T110155Z-928e38`
read; the Stage-0-form installs again ran host-native — Claude `claude plugin
update runtime@agentic-plugins` + `claude plugin update
attention@agentic-plugins`, Codex `codex plugin marketplace upgrade`
propagating the refreshed marketplace snapshot followed by `codex plugin add`
for each plugin — so this upgrade too left no `runtime:settings
...
`plugin-runtime` `0.84.0`: permission proof, deep peer smoke, and workflow
continuation proof executed and passed in both directions, recorded on
2026-07-21Z as `doctor-20260721T110155Z-928e38` — the **ADR-0047 Release A**
loop (feature PR #611 squash `560527f`, the §5 seeded standing notification
watch; feature PR #613, a rebase-merged three-commit package split per
ADR-0016 — `54f39c0` the runtime `response-needed` kind contract + Codex
shuttle remap, `952af14` the attention copy-not-import enum mirror,
`73b88c1` the chain-aware notification-plan re-render + shuttle semver
hardening; release PR #612 squash `d9a8a7d`, tags `plugin-attention-v0.8.0`
+ `plugin-runtime-v0.84.0`, marketplace sync `d93d23e`; Claude Code `2.1.216`
/ `codex-cli 0.144.6`; the doctor `entry_brief` readiness section reads
`ready` — the operator's ADR-0045 user-scope `startup` opt-in with
`empty=silent`, the `floors.entry_brief=0.83.0` pin satisfied by `0.84.0`,
the entry executor present — alongside `session_capture` `ready`). `overall`
reads `pass` with zero warnings; Codex hook state reads **12/12** expected
`enabled_trusted` with `unexpected_agentic_entries=0` (the previously
retained pre-relocation attention rows are gone); **experience parity reads
`ready` `100%` (8/8, zero manual follow-ups) with no re-attestation
required** — attention `0.8.0` is vocabulary-only (the ADR-0047 §1
copy-not-import enum mirror; no producer, no hook change) and keeps zero
Codex hook surface, runtime ships no hooks, and the four attested plugins
(designer@0.3.0 / engineer@0.21.0 / founder@0.4.0 / orchestrator@0.13.0)
plus `codex-cli 0.144.6` are unchanged, so the CLI-version-bound four-plugin
attestation `settings-20260720T151554Z-3b543f` evaluates **current** under
the contract §11.2 #24 currency binding — the no-invalidation half of the
discipline observed live. **`host_parity_baseline` reads `current`** — the
Claude `2.1.215`→`2.1.216` drift this loop's first proof run exposed was
closed the same day by the compat cycle (content-backed `CHANGELOG.md`
ingest in run `compat-20260721T105600Z-71cea8`, final drift=none re-check in
`compat-20260721T105910Z-31010c`; additive/fix-only, no
plugin-CLI or hook-contract change); the ingest run was also the first live
exercise of the 0.84.0 standing notification watch (the `claude-notification-agent-types`
row signalled on the changelog's recorded 2.1.198 entry and injected its
required review step — a known, already-recorded gap; the row stays open by
design). This supersedes the 0.83.1-native S9 record recorded on 2026-07-20Z
as doctor-20260720T175310Z-a0fd88 — the **S9 peer-follow-up hardening** loop
(feature PR #606, rebase-merged eb480f3 / ba1c201 / fd381b4; release PR #607
squash 9a69944, tags plugin-attention-v0.7.1 + plugin-runtime-v0.83.1 —
backticks deliberately omitted for the freshness gate — marketplace sync
f54d70c; Claude Code 2.1.215 / codex-cli 0.144.6; that record read `overall`
`pass` with zero warnings, three execute proofs passed both directions, hook
state 12/12 `enabled_trusted` with the two retained pre-relocation attention
rows still present, and parity `ready` `100%`).
...
The latest `plugin-runtime` `0.84.0` release/install proof loop is the
**ADR-0047 Release A** slice — feature PR #611 squash `560527f` (the §5
seeded standing notification watch in `runtime:compat plan`, schema 1.1 with
the `actionable` flag) and feature PR #613, a rebase-merged three-commit
package split per ADR-0016: `54f39c0` (the runtime `response-needed` kind
contract + Codex shuttle remap), `952af14` (the attention copy-not-import
enum mirror), and `73b88c1` (the chain-aware notification-plan re-render +
shuttle private-semver hardening); release PR #612 squash `d9a8a7d` cut tags
`plugin-attention-v0.8.0` and `plugin-runtime-v0.84.0` with marketplace sync
commit `d93d23e`, and the 0.84.0-native proof
doctor-20260721T110155Z-928e38 (2026-07-21Z; `overall` `pass` with zero
warnings, three execute proofs executed and passed in both directions, Codex
hook state `12/12` expected `enabled_trusted` with
`unexpected_agentic_entries=0` — the previously retained pre-relocation
attention rows are gone; the doctor `entry_brief` section reads `ready` — the
operator's ADR-0045 user-scope `startup` opt-in, the
`floors.entry_brief=0.83.0` pin satisfied by `0.84.0` — alongside
`session_capture` `ready`; experience parity `ready` `100%` 8/8 with the
four-plugin attestation `settings-20260720T151554Z-3b543f` still current,
since attention 0.8.0 is vocabulary-only with zero Codex hook surface,
runtime ships no hooks, and the attested set plus `codex-cli 0.144.6` are
unchanged — no fresh `/hooks` review required; `host_parity_baseline`
`current` after the same-day 2.1.216 compat cycle — ingest + first live
standing-watch exercise in compat-20260721T105600Z-71cea8, drift=none
re-check in compat-20260721T105910Z-31010c) was taken against that
released binary, not a patched working tree. It supersedes the 0.83.1
