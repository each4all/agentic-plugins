...
`plugin-runtime` `0.94.0` state is claude=installed/0.94.0,
codex=installed/0.94.0, per the 2026-08-27Z
`doctor-20260827T013118Z-e8d064` read; the superseded 0.93.0 state was read by
...
installed `plugin-runtime` `0.94.0`: permission proof, deep peer smoke, and
workflow continuation proof executed and passed in both directions, recorded
on 2026-08-27Z as
`doctor-20260827T013118Z-e8d064` — the post-release record of the
**value-carrying interview** loop. The prior record was the
**portable-profile session-config** loop, a single runtime change: the machine
profile's 1.1 → 1.2 carriage of the session-config family (`session_capture`,
`entry_brief`, `entry_brief_empty`) as trailing bare scalars, a
`projectSession` user-global reader, and seed proposals carrying a
runtime-derived `user_scope_only` label `a973d05` (PR #734); release PR
[#735](https://github.com/each4all/agentic-plugins/pull/735) squash `9be7501`,
tag plugin-runtime-v0.93.0, marketplace sync `c526efe`, stage-doc sync
`4ac05c9`. The CURRENT loop is the **value-carrying interview** slice, a single
runtime change carried by one squash: the bootstrap interview's
`set:<key>=<value-or-unset>` answer form with two Stage-4 value steps
(`config.session`, `config.notify_kinds`), the run schema 1.2 → 1.3 semantic
bump that arms the future-minor mutator fence, and a `runtime:settings --unset`
config-removal operation — the first destructive write on that path, which is
why its apply became temp+rename atomic `f795085`
(PR [#739](https://github.com/each4all/agentic-plugins/pull/739)); release PR
...
`443ad73`, with `host_parity` `pass` under Claude Code `2.1.247` / Codex
`0.150.0`. `overall` reads **`warning`** — `hard_failures` is empty, and the
one warning is "latest compatibility check needs follow-up"; no hard failures
is not the same claim as `pass`. An install WAS performed by this recovery on
both hosts, and the evidence is the byte comparison rather than the version
string — both caches hold an identical 128-file 0.94.0 package (whole-tree
digest
`9b66e6de4277776b99e75cfc4ffccaacba553bcb966edb71a531eab5b6a1edfb`), and the
packaged baseline hashes
`86fdf17e4418884f01b67785eeaaacc74923a8a01efa238f514bf270da600843` identically
across both caches, the `plugin-runtime-v0.94.0` tag and the repository at
`443ad73`, which is also the value doctor independently records as
`host_parity_baseline.provenance.content_sha256` — unchanged from 0.93.0,
because this release did not touch the packaged baseline. The file count moved
127 → 128 for a reason worth naming: the run schema was RENAMED 1.2 → 1.3 and
`lib/answer-values.mjs` was added, so one file joined the package and none
left.
...
The latest `plugin-runtime` `0.94.0` release/install proof loop is the
**value-carrying interview** slice, whose 0.94.0-native proof
`doctor-20260827T013118Z-e8d064` (2026-08-27Z) reads `partial` `81%` 5/9
...
`2.1.247` / `0.150.0`, refreshing which is protected-asset work this recovery
...
`/hooks` re-attestation the only way to clear it. The superseded 0.93.0
**portable-profile session-config** proof doctor-20260825T070411Z-d6ecb2
(2026-08-25Z) read the same shape, as did the 0.92.0
