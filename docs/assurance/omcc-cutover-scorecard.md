...
plugin-management config writes planned after the `plugin-runtime` `0.66.0`
release and installed-state refresh (Claude `claude plugin update` and Codex
`codex plugin add` cache materialization both report 0.66.0, 2026-06-10). Current runtime execution evidence is
carried forward by the installed `plugin-runtime` `0.66.0` loop: permission
...
recorded under the 0.61.0 install; the 0.62→0.66 bumps are runtime-only — 0.66.0
...
`--apply-codex-plugin-hooks` host-config write — and does not touch the
...
The latest `plugin-runtime` `0.66.0` release/install proof loop is carried by
feature PRs #403 (ADR-0035 §5/§6 explicit `codex plugin add` install executor) and
#405 (ADR-0035 §6 `--apply-codex-plugin-hooks` hard-removal; satellite docs #406) and
release PR #404 (runtime-only; no co-released hook-bearing plugins),
release tag `plugin-runtime-v0.66.0`, marketplace sync commit `0281d86`, and
