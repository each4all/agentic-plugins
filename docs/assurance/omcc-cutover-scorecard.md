...
`0.75.0` release and installed-state refresh (`claude plugin list` and `codex
plugin list` both report the runtime plugin installed and enabled at 0.75.0,
2026-07-07). Current runtime execution evidence is native to the installed
`plugin-runtime` `0.75.0`: permission proof, deep peer smoke, and workflow
continuation proof passed in both directions, re-recorded on 2026-07-07 as
`doctor-20260707T071802Z-482de4`
(Claude Code `2.1.202` / `codex-cli 0.142.5`; engineer `0.20.0` /
orchestrator `0.12.0` / founder `0.3.0` / attention `0.4.0` / image `0.2.0`;
experience parity `ready` `100%`, 8/8 satisfied). This supersedes the prior
0.74.1 loop (`doctor-20260706T143221Z-c32cdf`, parity `ready` `100%`), the
immediately-preceding ADR-0041 §2d E1 egress transport (IPv6) fix release.
The Codex `/hooks` review/trust attestation for the hook-bearing set
(`engineer@0.20.0` / `orchestrator@0.12.0` / `founder@0.3.0`) is **unchanged by
the 0.75.0 release** (runtime ships no Codex hooks; the ADR-0041 egress channel
is `notify.mjs`, not a hook), so the 2026-07-04 attestation
`settings-20260704T170801Z-b66656` still covers it; doctor hook-state reports all
9 expected bundled Codex hook entries `enabled` and `trusted` (`untrusted=0`,
`disabled=0`). The `plugins/attention` `0.4.0` sensor plugin is recognized as a
deliberately Claude-hook-only plugin (ADR-0040 §3 hook-only L1, no Codex hooks):
doctor classifies such a plugin as `claude_adapter_only` instead of a false Codex
packaging gap, which keeps observed experience parity `ready` `100%`. Runtime still records the
...
The latest `plugin-runtime` `0.75.0` release/install proof loop is the ADR-0041
§3a **opt-in closed-vocabulary headline egress token** — release PR #512 cut
`plugin-attention` `0.4.0` (the `deriveHeadlineToken` producer) and
`plugin-runtime` `0.75.0` (the `loadEgressHeadlineOptIn` / `HEADLINE_VOCAB` egress
opt-in), plus the acceptance gate PR #514, release tag `plugin-runtime-v0.75.0`,
marketplace sync commit `ac5539f` (release-please Action follow-up), and the
0.75.0-native proof doctor artifact `doctor-20260707T071802Z-482de4` (recorded
2026-07-07 under Claude Code `2.1.202` / Codex `0.142.5`, parity `ready` `100%`).
Real owner Telegram delivery of the §3a headline was proven 2026-07-07
(byte-for-byte `approval · complete · @mba · …`, K2 real-network smoke; see
ADR-0041 §3a and `docs/release-proofs/adr0041-headline-egress.md`). Earlier release/install proof loops
(the 0.74.0 ADR-0041 §2d E1 egress transport fix loop, the 0.73.0 ADR-0041 E1 cross-machine notification egress loop, the 0.72.0 ADR-0040 §3/§5 loop, the 0.71.0 emitter/dashboard slices, the 0.70.1
...
| R3 | Switching development tools must work in both directions: Claude Code to Codex and Codex to Claude. | Cross-host tests cover resume and stop-archive behavior for engineer/orchestrator; companions exist in both directions. Installed `plugin-runtime` `0.75.0` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded under the 0.74.1 install on 2026-07-07 KST (`doctor-20260707T071802Z-482de4`, Claude Code `2.1.202` / Codex `0.142.5`, runtime installed `0.75.0` on both hosts, experience parity `ready` `100%` with the `/hooks` attestation `settings-20260704T170801Z-b66656` still covering the hook-bearing set unchanged by this release). Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. | satisfied | Keep the explicit installed-runtime proof in the release/install continuity loop whenever the peer path changes. |
| R4 | Claude Code and Codex user experience must be equivalent where possible; non-portable host-specific features must not become hard dependencies. | Architecture documents host-specific adapter boundaries; runtime baseline documents command/hook/subagent differences; Codex macro/meta skill mirrors exist. The native `runtime:doctor` execute proofs re-recorded under the 0.74.1 install on 2026-07-07 (`doctor-20260707T071802Z-482de4`, Claude Code `2.1.202` / Codex `0.142.5`, runtime `0.75.0` on both hosts) confirmed bidirectional peer execution and engineer workflow continuation passed both directions, and `runtime:settings` reports source/cache version parity with zero plugin-management config writes planned. The Codex `/hooks` review/trust attestation from 2026-07-04 (`settings-20260704T170801Z-b66656`; `~/.codex/config.toml` hook-state `untrusted=0`/`disabled=0`) still covers the hook-bearing set `engineer@0.20.0`/`orchestrator@0.12.0`/`founder@0.3.0` — unchanged by the 0.75.0 release (runtime ships no Codex hooks; the ADR-0041 egress channel is `notify.mjs`, not a hook); `attention@0.4.0` is a deliberately Claude-hook-only plugin (ADR-0040 §3, recognized `claude_adapter_only`, not a Codex hook), and `image` ships no Codex hooks manifest, so observed experience parity reads `ready` (score `100%`, 8/8 satisfied) per this row's gate. | satisfied | Refresh the active-session `/hooks` review/trust attestation after hook-bearing plugin upgrades or hook packaging changes; do not claim observed parity `ready` until the post-attestation doctor/cutover evidence is recorded. |
