...
`0.79.0` release and installed-state refresh (the refresh itself was executed
through the `runtime:settings --execute-plugin-management` executor, artifact
`settings-20260712T080408Z-5b98f9` — Claude `update-plugin` and Codex
`upgrade-marketplace` both exit 0; the Claude plugin cache and `codex plugin
list --json` both report the runtime plugin installed and enabled at 0.79.0,
2026-07-12Z). Current runtime execution evidence is native to the installed
`plugin-runtime` `0.79.0`: permission proof, deep peer smoke, and workflow
continuation proof passed in both directions, recorded on 2026-07-12Z as
`doctor-20260712T080638Z-005af5` — the **ADR-0043 S2 four-persona
workflow-projection seam loop** (feature PR #555 squash `cb720e7`, behind ADR
authoring PR #553 and Accepted-flip PR #554; release PR #556 squash `558f78a`,
tag `plugin-runtime-v0.79.0`, marketplace sync `8ca4651`; Claude Code `2.1.207`
/ `codex-cli 0.144.1`). Experience parity reads `ready` `100%` 8/8 and
`overall` `pass`; `host_parity_baseline` reads `current` against the
2026-07-11 baseline (the #550 refresh); Codex hook state reads **12/12**
expected `enabled_trusted` with `unexpected_agentic_entries=2` (the retained
pre-relocation attention rows, display-only), and the four-plugin `/hooks`
attestation `settings-20260712T015100Z-312fbb` (attested 2026-07-12T01:51Z)
remains current — runtime ships no hooks, so the upgrade left the hook-bearing
set unchanged. This supersedes the 2026-07-11Z **attention 0.4.1 relocation
...
The latest `plugin-runtime` `0.79.0` release/install proof loop is the
ADR-0043 S2 **four-persona workflow-projection seam** slice — feature PR #555
...
`plugin-runtime-v0.79.0` with marketplace sync commit `8ca4651`, and the
0.79.0-native proof `doctor-20260712T080638Z-005af5` (2026-07-12Z; parity
`ready` `100%` 8/8, `overall` `pass`, `host_parity_baseline` `current`,
three execute proofs passed both directions) was taken against that released
binary — it is the record of reference. The preceding 0.78.1 loop was an
