...
⚠ **Two of the three fixes are not exercised by this installed run, and are
not claimed from it.** C9's `current` branch and C13's informational-plan
branch are reachable only from a `current` compat run, which a stale baseline
cannot produce; C11 is not exercised unless the list probe fails. The report
carries zero occurrences of C9's defect text, and that proves nothing here:
`7a77bef` adds exactly `if (status === 'current') return [];` and touches no
other branch, so the text is absent because this run is
`release_notes_required`. By owner decision on 2026-09-18 all three are
evidenced by their tests plus the per-file delivery measurement recorded in
R3, on the footing the C11 topic already required of itself.
