...
`plugin-runtime` `0.82.0` state (both the Claude plugin cache and `codex plugin
list --json` report the runtime plugin installed and enabled at 0.82.0 —
claude=installed/0.82.0, codex=installed/0.82.0 — alongside engineer 0.21.0 and
orchestrator 0.13.0, per the 2026-07-19Z doctor read; the Stage-0-form installs
...
`plugin-runtime` `0.82.0`: permission proof, deep peer smoke, and workflow
continuation proof executed and passed in both directions, recorded on
2026-07-19Z as `doctor-20260719T071752Z-6392f1` — the **ADR-0044
session-capture exit-side stack** loop (feature PRs S2 #588 `9dc3eff`, S3a
#590 `8b7d887`, S3b #591 `7f5710a`, S4 #592 `417bee6`; release PR #589 squash
`717c4c3`, tag `plugin-runtime-v0.82.0`, marketplace sync `31a01ba`; Claude
Code `2.1.215` / `codex-cli 0.144.1`; the new doctor `session_capture`
readiness section reads `off` against the installed binary — the shipped
default, the honest informational state before any operator opt-in). `overall`
reads `pass`;
...
supersedes the 0.81.0-native **ADR-0046 S8 machine-bootstrap stack loop**
...
The latest `plugin-runtime` `0.82.0` release/install proof loop is the
**ADR-0044 session-capture exit-side** slice — feature PRs S2 #588 `9dc3eff`
...
`717c4c3` cut tag `plugin-runtime-v0.82.0` and marketplace sync commit
`31a01ba`, and the 0.82.0-native proof `doctor-20260719T071752Z-6392f1`
(2026-07-19Z; `overall` `pass`, three execute proofs executed and passed in
both directions, Codex hook state `12/12` expected `enabled_trusted` with
`unexpected_agentic_entries=2` retained pre-relocation attention rows,
display-only; the new doctor `session_capture` readiness section reads `off`,
the shipped default) was taken against that
released binary, not a patched working tree. It supersedes the 0.81.0
