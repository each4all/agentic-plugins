...
plugin-management recommendations after the `plugin-runtime` `0.57.0` release
and installed-state refresh. Current runtime execution evidence on 2026-05-17 KST
from installed `plugin-runtime` `0.57.0` reports permission proof, deep peer
...
parity reached 100%. The `plugin-runtime` `0.57.0` release/install proof loop is
carried by PR #327 and release PR #328, release tag
`plugin-runtime-v0.57.0`, marketplace sync commit `d9f01c4`, settings artifact
`settings-20260517T043011Z-5189e9`, hook attestation artifact
`settings-20260517T043034Z-84dfec`, proof doctor artifact
`doctor-20260517T043046Z-7745e9`, and cutover record
`cutover-20260517T043144Z-c1d3c3`.
...
| R3 | Switching development tools must work in both directions: Claude Code to Codex and Codex to Claude. | Cross-host tests cover resume and stop-archive behavior for engineer/orchestrator; companions exist in both directions. Installed `plugin-runtime` `0.57.0` executed `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` on 2026-05-17 KST. Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. | satisfied | Keep the explicit installed-runtime proof in the release/install continuity loop whenever the peer path changes. |
| R4 | Claude Code and Codex user experience must be equivalent where possible; non-portable host-specific features must not become hard dependencies. | Architecture documents host-specific adapter boundaries; runtime baseline documents command/hook/subagent differences; Codex macro/meta skill mirrors exist. `runtime:settings` reports source/cache version parity and zero plugin-management recommendations on 2026-05-17 KST after the `0.57.0` installed-state refresh; the operator opened Codex `/hooks` and confirmed trusted PreCompact, SessionStart, and Stop hooks for `engineer@0.12.0` and `orchestrator@0.7.2`; `runtime:settings --attest-codex-hook-review` recorded `settings-20260517T043034Z-84dfec`; `runtime:doctor --record` then reported observed experience parity `ready`, score 100%, and zero manual follow-ups in `doctor-20260517T043046Z-7745e9`. Runtime still does not mutate or non-interactively prove Codex trust state; it records the operator claim as bounded evidence. | satisfied | Refresh the active-session `/hooks` review/trust attestation after hook-bearing plugin upgrades or hook packaging changes. |
