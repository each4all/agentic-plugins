...
closed the same day by the compat-cycle refresh (content-backed `CHANGELOG.md`
ingest, run `compat-20260721T105910Z-31010c`; additive/fix-only, no
plugin-CLI or hook-contract change), which was also the first live exercise
of the 0.84.0 standing notification watch (the `claude-notification-agent-types`
...
`current` after the same-day 2.1.216 compat-cycle refresh, the loop that also
first exercised the standing notification watch live) was taken against that
