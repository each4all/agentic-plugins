...
git clone that tracks `main`, so a Codex start after `main` moves re-clones it
and force-reinstalls every configured plugin under an unchanged version, and
cross-plugin discovery on Codex also read that clone. S1–S3 make each sibling
lookup resolve from the caller's own install cache, never the clone, and make
doctor report each Codex install against its catalog pin, keeping the catalog
...
mechanism ADR-0061 measured, the next package-path commit on `main` reaches
Codex under these same versions until S4 pins the catalog. The packaged
...
clone timeout while GitHub pack transfer to this machine ran at 65–100 KB/s
(another registry served 1.5 MB/s, and `git ls-remote` and incremental fetches
stayed fast). A short `codex exec` started the background re-clone but exited
before it finished, leaving a staging directory with no `HEAD`. The upgrade
then ran once with a process-scoped git `insteadOf`, set through `GIT_CONFIG_*`
in that command's environment only, pointing the clone at a bare local mirror
of `main` at `807f76a`, the commit GitHub served. It reinstalled all eight
plugins in about a second, and the snapshot's recorded source and `origin`
remain the GitHub URL. No persistent git or Codex configuration changed.
