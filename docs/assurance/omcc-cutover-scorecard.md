...
doctor-20260810T135637Z-d49983 on 2026-08-10Z, and the 0.88.1 state before it by
doctor-20260803T091403Z-7f2850. (An earlier revision of this paragraph
...
record of the **Claude 2.1.227 host-parity baseline refresh** loop
(implementation PR #688 squash `354a95d`; release PR #689 squash `8c9789a`;
tag `plugin-runtime-v0.90.1`; marketplace sync `6ba5f21`, stage-doc sync
`94de990`), the first baseline refresh to carry the ADR-0051 §Decision 2
release obligation rather than landing as a docs-only edit, with `host_parity`
`pass` under Claude Code `2.1.228` / Codex `0.147.0`. Observed experience parity
is **`partial` `95%`** (7 satisfied, 1 partial, zero blocked) and
`host_parity_baseline` reads **`stale`** — both honest rather than a code
regression. Claude published `2.1.228` 20.8h after `2.1.227`: 13.8h after this
...
The latest `plugin-runtime` `0.90.1` release/install proof loop is the
**Claude 2.1.227 host-parity baseline refresh** slice, whose 0.90.1-native proof
`doctor-20260812T022356Z-252fb9` (2026-08-12Z) reads `partial` `95%` 7/8 with zero
blocked criteria and `host_parity` `pass`, and honestly reads
`host_parity_baseline` `stale` — Claude moved to `2.1.228` 7.8h before this
loop's own PR merged, so the refreshed baseline was overtaken before it shipped.
It supersedes the ADR-0051 host-parity baseline source slice, whose 0.90.0-native
