...
plugin-management config writes planned after the `plugin-runtime` `0.67.0`
release and installed-state refresh (Claude `claude plugin update` and Codex
`codex plugin marketplace upgrade` cache materialization both report 0.67.0, 2026-06-11). Current runtime execution evidence is
carried forward by the installed `plugin-runtime` `0.67.0` loop: permission
proof, deep peer smoke, and workflow continuation proof passed in both
directions. The execution proof is `doctor-20260603T044000Z-4532fd` (reusable,
recorded under the 0.61.0 install; the 0.62→0.67.0 bumps are runtime-only — 0.66.0
wires the explicit policy-gated `codex plugin add` install executor through the
ADR-0035 §5/§6 gates and hard-removes the non-conformant
`--apply-codex-plugin-hooks` host-config write, 0.66.1 makes the Codex
peer-direction hook-gate readiness warning stage-aware (ADR-0030), and 0.67.0
adds the `runtime:consensus ratify` converged-run owner ratification surface — and none touch the
peer/dispatch path). The Codex `/hooks` review/trust
...
trust_level=trusted). Observed experience parity is now `ready`, score `100%`,
with zero manual follow-ups. Runtime still records the operator claim and does
not mutate or independently prove Codex trust state; the attestation is valid
while the hook-bearing plugin set and source versions still match.
The latest `plugin-runtime` `0.67.0` release/install proof loop is carried by
feature PRs #403 (ADR-0035 §5/§6 explicit `codex plugin add` install executor),
#405 (ADR-0035 §6 `--apply-codex-plugin-hooks` hard-removal; satellite docs #406),
#410 (ADR-0030 stage-aware Codex peer-direction hook-gate readiness warning), and
#414 (`runtime:consensus ratify` converged-run owner ratification) and
release PRs #404, #411, and #415 (runtime-only; no co-released hook-bearing plugins),
release tag `plugin-runtime-v0.67.0`, marketplace sync commit `fd78ab9`, and
reusable proof doctor artifact `doctor-20260602T020728Z-a9b47e`. The Codex `/hooks`
review/trust attestation for the hook-bearing `engineer@0.14.0` and
`orchestrator@0.8.0` bundled hooks was recorded on 2026-06-03
(`settings-20260603T072929Z-88b458`), and a confirming `runtime:doctor` run
(`doctor-20260603T044000Z-4532fd`, reusable) reports observed experience parity
`ready` at `100%` with zero manual follow-ups. The prior fully re-attested
...
| R3 | Switching development tools must work in both directions: Claude Code to Codex and Codex to Claude. | Cross-host tests cover resume and stop-archive behavior for engineer/orchestrator; companions exist in both directions. Installed `plugin-runtime` `0.67.0` carries the reusable `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof recorded under the 0.61.0 install on 2026-06-02 KST (`doctor-20260602T020728Z-a9b47e`); the 0.62→0.67 bumps are runtime-only and do not touch the peer/dispatch path. Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. | satisfied | Keep the explicit installed-runtime proof in the release/install continuity loop whenever the peer path changes. |
| R4 | Claude Code and Codex user experience must be equivalent where possible; non-portable host-specific features must not become hard dependencies. | Architecture documents host-specific adapter boundaries; runtime baseline documents command/hook/subagent differences; Codex macro/meta skill mirrors exist. After the `plugin-runtime` `0.67.0` release (runtime-only — the hook-bearing `engineer`/`orchestrator` set is unchanged, so the 2026-06-03 attestation for the unchanged hook set carries forward), the reusable `runtime:doctor` execute proofs from 2026-06-03 (`doctor-20260603T044000Z-4532fd`, recorded under the 0.61.0 install) confirmed bidirectional peer execution and engineer workflow continuation passed both directions, and `runtime:settings` reports source/cache version parity with zero plugin-management config writes planned. The Codex `/hooks` review/trust attestation for the bundled `engineer@0.14.0`/`orchestrator@0.8.0` hooks was recorded on 2026-06-03 (`settings-20260603T072929Z-88b458`; `~/.codex/config.toml` hook-state `untrusted=0`/`disabled=0`), and a confirming `runtime:doctor` run reports observed experience parity `ready` (score `100%`, 0 manual follow-ups). The attestation stays valid while the hook-bearing plugin set and source versions still match. | satisfied | Refresh the active-session `/hooks` review/trust attestation after hook-bearing plugin upgrades or hook packaging changes; do not claim observed parity `ready` until the post-attestation doctor/cutover evidence is recorded. |
