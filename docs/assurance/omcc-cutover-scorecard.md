...
plugin-management config writes planned after the `plugin-runtime` `0.62.0`
release and installed-state refresh. Current runtime execution evidence is
carried forward by the installed `plugin-runtime` `0.62.0` loop: permission
...
recorded under the 0.61.0 install; the 0.62.0 bump is runtime-only — it adds
the `runtime:doctor` host-parity-baseline freshness check and Codex
plugin_hooks stage-aware readiness — and does not touch the peer/dispatch
path). The Codex `/hooks` review/trust
...
The latest `plugin-runtime` `0.62.0` release/install proof loop is carried by
feature PRs #374 (`runtime:doctor` host-parity-baseline freshness) and #370
(Codex plugin_hooks stage-aware readiness, ADR-0030) and release PR #371,
release tag `plugin-runtime-v0.62.0`, marketplace sync commit `40bef33`, and
