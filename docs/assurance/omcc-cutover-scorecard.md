...
historical rather than current evidence; the current manifest is 0.97.1. The
...
`plugin-runtime` `0.97.1` state is claude=installed/0.97.1,
codex=installed/0.97.1, per the 2026-08-28Z
`doctor-20260828T090207Z-ce202d` read; the superseded 0.97.0 state was read by
doctor-20260828T035114Z-44ccdf on 2026-08-28Z, the 0.96.0 state before it by
doctor-20260827T235051Z-c11408 on 2026-08-27Z, the 0.95.0 state before it by
...
installed `plugin-runtime` `0.97.1`: permission proof, deep peer smoke, and
workflow continuation proof executed and passed in both directions, recorded
on 2026-08-28Z as
`doctor-20260828T090207Z-ce202d` — the post-release record of the
**host-parity baseline refresh** loop. The prior record was the
...
(named CURRENT here until the 0.96.0, 0.97.0, and 0.97.1 loops superseded it;
the current loop is named once, below):
...
The CURRENT loop is the **host-parity baseline refresh** slice, and it is the
loop whose deliverable is a *protected asset*. Both hosts
had drifted — Claude Code `2.1.233`→`2.1.250` across 17 patch positions, Codex
...
2026-08-18 proof. Across the retained window the census is 10 `stale`, one
`unknown`, and this loop's `current`: the exception,
`doctor-20260820T021739Z-5ab1c9`, reads `unknown` because neither host CLI
returned a usable version in that run, so it assessed nothing rather than
...
is protected-asset work that recovery was forbidden to do. This loop performed
it the only way the architecture allows — by shipping a release — and
the same field now reads **`current`** with `direction` `exact` on both hosts,
resolved with `provenance.source` `package` from the 0.97.1 cache copy,
`content_sha256`
...
⚠ **The parity and `overall` figures inside this loop's artifact are mid-loop.**
The proof was recorded before the confirming compatibility run existed, so it
reads `partial` `86%` (99/115, 5 satisfied / 3 partial / 0 blocked) and `overall`
...
satisfied. A plain `runtime:doctor` after it reads `overall` **`pass`** and
parity **`91%`** with **6 satisfied / 2 partial / 0 blocked**. Both survivors are
the operator-only Codex `/hooks` re-attestation. A reader who quotes the 86% out
of the artifact without this paragraph will understate the loop by five points
and misreport `overall`.

⚠ **Unlike its predecessor, this recovery performed an install**, and the
distinction matters for what the byte comparison below proves: both hosts were
moved 0.97.0 → 0.97.1 (`claude plugin marketplace update` then `claude plugin
...
and ADR-0057 editing the file; this one is the deliverable.

Byte identity was re-run rather than restated: `diff -r` between Claude's cache
and Codex's served marketplace tree exits `0` with no output, **120** files each
— unchanged from 0.97.0, because this release edits documents only — whole-tree
digest
`3f338a25cfb82f4b0204827beb366ae0147d23a856484a8d04ab9f125bcc6644` under the
path-first recipe below, control-verified this loop by reproducing 0.97.0's
...
after the scan; 81 are on disk now. `malformed > 0` never clears once set, which
is why this is worth measuring on a real machine rather than reasoning about.
...
The latest `plugin-runtime` `0.97.1` release/installed-state proof loop is the
**host-parity baseline refresh** slice, whose 0.97.1-native proof
`doctor-20260828T090207Z-ce202d` (2026-08-28Z) reads `partial` `86%` 5/8
(99/115) with **zero blocked criteria**, `host_parity` `pass`, `overall`
`warning` (zero hard failures, one warning: latest compatibility check needs
follow-up), and `host_parity_baseline` **`current`** with `direction` `exact` on
both hosts — the 2026-08-28 `2.1.250` / `0.150.1` baseline matching the observed
pair — a match produced by exactly the protected-asset refresh the superseded
0.97.0 record said that recovery was forbidden to perform. ⚠ Those `86%` and `warning` readings are
**mid-loop**: the confirming `runtime:compat` run
`compat-20260828T090343Z-1ee918` came after the proof and returned `status
current` / `drift none`, after which a plain `runtime:doctor` reads `overall`
**`pass`** and parity **`91%`** 6/8 with both survivors being the operator-only
Codex `/hooks` re-attestation. The superseded 0.97.0 proof
doctor-20260828T035114Z-44ccdf read the same `86%` 5/8 with
`host_parity_baseline` `stale`/`ahead`. Its one manual follow-up is `codex-hook-review`: this
release touched no Codex hook-bearing plugin — runtime ships no hooks — so the
trusted entries are unchanged, and the follow-up exists because the observed
Codex CLI offers no non-interactive hook-trust query, making the owner's
`/hooks` re-attestation the only way to clear it.

⚠ **For the superseded 0.97.0 proof, THE SCORE WAS UNCHANGED, AND THAT WAS THE
READING** (the current loop's 91% is a separate figure, reached only after its
confirming compatibility run). `5/8 (99/115)` is
...
its `current` under this era's weaker meaning. That is no longer the live state:
the 0.97.1 loop above replaced it with `compat-20260828T090343Z-1ee918`, which
reads `status current` / `drift none`.
