...
plugin-management recommendations after the `plugin-runtime` `0.51.2` release
and installed-state refresh. Current runtime execution evidence on 2026-05-17 KST
from installed `plugin-runtime` `0.51.2` reports permission proof, deep peer
smoke, and workflow continuation proof passed in both directions; experience
parity is 91%. The `plugin-runtime` `0.51.2` release/install proof loop is
carried by PR #299 and release PR #300, release tag
`plugin-runtime-v0.51.2`, marketplace sync commit `0425b7a`, settings artifact
`settings-20260517T001655Z-71b359`, doctor artifact
`doctor-20260517T001716Z-7a7c64`, and cutover record
`cutover-20260517T001811Z-c83755`.
...
| R3 | Switching development tools must work in both directions: Claude Code to Codex and Codex to Claude. | Cross-host tests cover resume and stop-archive behavior for engineer/orchestrator; companions exist in both directions. Installed `plugin-runtime` `0.51.2` executed `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` on 2026-05-17 KST. Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. | satisfied | Keep the explicit installed-runtime proof in the release/install continuity loop whenever the peer path changes. |
| R4 | Claude Code and Codex user experience must be equivalent where possible; non-portable host-specific features must not become hard dependencies. | Architecture documents host-specific adapter boundaries; runtime baseline documents command/hook/subagent differences; Codex macro/meta skill mirrors exist. `runtime:settings` reports source/cache version parity and zero plugin-management recommendations on 2026-05-17 KST after the `0.51.2` installed-state refresh; `runtime:doctor` reports 91% experience parity with installed `plugin-runtime` `0.51.2`. `runtime:cutover` now surfaces the remaining active-session Codex `/hooks` review/trust follow-up as `host=codex; commands=/hooks`, and runtime deliberately does not infer that state non-interactively. | partial | Clear the Codex active-session hook review/trust follow-up and rerun doctor until the observed parity score reaches 100% without hiding host-specific syntax differences. |
