...
against the installed `plugin-runtime` `0.90.0` state (`claude plugin list`
...
claude=installed/0.89.0, codex=installed/0.89.0 — per the 2026-08-10Z
`doctor-20260810T135637Z-d49983` read; the superseded 0.88.1 state was read by
...
native to the installed `plugin-runtime` `0.90.0`: permission proof, deep peer
smoke, and workflow continuation proof executed and passed in both directions,
recorded on 2026-08-10Z as `doctor-20260810T135637Z-d49983` — the post-release
record of the **ADR-0051 host-parity baseline source** loop (ADR PR #684 squash
...
sync `d9b6e93`), with `host_parity` `pass` and `host_parity_baseline` `current`
under Claude Code `2.1.226` / Codex `0.147.0`. Observed experience parity is
**`ready` `100%`** (8 satisfied, zero partial, zero blocked). The preceding
record's `95%` was a structural ceiling, not a regression, and this loop is what
removed it: that record named the cause correctly — `runtime:compat` resolved
...
The latest `plugin-runtime` `0.90.0` release/install proof loop is the
**ADR-0013 trigger re-evaluation** slice, whose 0.89.0-native proof
`doctor-20260810T135637Z-d49983` (2026-08-10Z) reads `ready` `100%` 8/8 with zero
blocked criteria, `host_parity` `pass` and `host_parity_baseline` `current` — the
compat baseline-source asymmetry that held the preceding record at `95%` is
resolved by ADR-0051, whose release this proof records. It supersedes the
