...
historical rather than current evidence; the current manifest is 0.90.2. The
newest hook-review attestation, which the 0.90.2 proof selects and reports
current, is `settings-20260809T122130Z-c26a0c`. The current installed
`plugin-runtime` `0.90.2` state is claude=installed/0.90.2,
codex=installed/0.90.2, per the 2026-08-14Z
`doctor-20260814T083045Z-bc4fc6` read; the superseded 0.90.1 state was read by
...
than restated.)
This loop refreshed only Claude host-
...
native to the installed `plugin-runtime` `0.90.2`: permission proof, deep peer
smoke, and workflow continuation proof executed and passed in both directions,
recorded on 2026-08-14Z as `doctor-20260814T083045Z-bc4fc6` — the post-release
record of the **ADR-0051 P2 review hardening** loop
(implementation PR #694 squash `aaf4744`; release PR #695 squash `271c5ae`;
tag `plugin-runtime-v0.90.2`; marketplace sync `dbcb983`, stage-doc sync
`3ea869a`), with `host_parity` `pass` (zero issues) and `overall.hard_failures`
empty under Claude Code `2.1.232` / Codex `0.147.0`. Observed experience parity
is **`partial` `95%`** (7 satisfied, 1 partial, zero blocked) and
`host_parity_baseline` reads **`stale`** — both honest rather than a code
regression. This release did not touch the baseline, so the staleness is
inherited: the packaged copy still carries `354a95d`'s 2026-08-11 observation of
`2.1.227` while the host has reached `2.1.232`, five patches on. The macro's own
stop clause said not to record a current proof against a stale baseline; the
owner amended it on 2026-08-14 to record this proof and open a **named**
baseline follow-up, because halting would have left the proof-coupled
assertions red with no owner for the staleness either. The one partial is
`runtime_handoff_artifacts`, whose compat leg reads `needs_attention` after this
loop's own snapshot `compat-20260814T080135Z-26a825` recorded the drift and
ingested content-backed release notes (`gap_analysis_ready` → `planned`,
`refresh-baseline` required). That drift is not a repeat of the previous one:
the 2.1.228–2.1.232 range changes surfaces this baseline documents — plugin
marketplace sources (GitLab, `command`), `/plugin install` refreshing the
marketplace first, server-supplied hooks for self-hosted runners, subagent
forking on by default, and a plugin-cache fix for symlinked development
checkouts that touches the very control case the P2 hardening pinned. The byte
comparison is clean: both host caches hold an identical 0.90.2 package (117
files, whole-tree digest `e19a62f0319a84fd099b157e004a4ed958dde5d8eebc9c04d47ef98f9a9804af`) and the packaged baseline hashes
`a7fe6fa1412f7ab59c123c86fd27e7ec9fce92328fc61a1127722cf5d02b3379` in the Claude cache, the Codex cache and the repository alike — the
same value doctor now records as
`host_parity_baseline.provenance.content_sha256`, taken over the file's bytes
per the ADR-0051 §Decision 5 extension, with `provenance.status` `resolved` and
`source` `package`. It supersedes the 0.90.1 record recorded on 2026-08-12Z as
...
The latest `plugin-runtime` `0.90.2` release/install proof loop is the
**ADR-0051 P2 review hardening** slice, whose 0.90.2-native proof
`doctor-20260814T083045Z-bc4fc6` (2026-08-14Z) reads `partial` `95%` 7/8 with zero
blocked criteria, `host_parity` `pass` and `overall.hard_failures` empty, and
honestly reads `host_parity_baseline` `stale` — this release did not touch the
baseline, so it still carries the 2026-08-11 observation of `2.1.227` while the
host has reached `2.1.232`. Two readings in this loop are worth separating from
