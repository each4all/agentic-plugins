...
Current local dry-run evidence: a fresh 2026-07-26Z `runtime:settings` dry-run
...
`0.86.2` = installed `0.86.2` on both hosts), and zero plugin-management
config writes planned against the installed `plugin-runtime` `0.86.2` state
(both the Claude plugin cache and `codex plugin list --json` report the
runtime plugin installed and enabled at 0.86.2 — claude=installed/0.86.2,
codex=installed/0.86.2 — per the 2026-07-26Z `doctor-20260726T014023Z-1b377b`
...
`plugin-runtime` `0.86.2`: permission proof, deep peer smoke, and workflow
continuation proof executed and passed in both directions, recorded on
2026-07-26Z as `doctor-20260726T014023Z-1b377b` — the **ADR-0048
...
#629 3615dcc, then the 0.86.1 egress-ack intent-WAL hardening #637 b984dc8 and
the 0.86.2 Stage-8 proof-rendering pair #641 af620df (egress-proof opt-in read
from run provenance, not the derived row) / #639 c549ed2 (each Stage-8 proof
rendered once from the completion reducer, control rows kept distinct from
evidence rows); release PR #642 `chore: release main` squash `9e2af7d`, tag
`plugin-runtime-v0.86.2`, marketplace sync `668c325`; Claude Code `2.1.220` /
...
The latest `plugin-runtime` `0.86.2` release/install proof loop is the
...
#641 af620df / #639 c549ed2; release PR #642 (`chore: release
main`, squash `9e2af7d`) cut tag `plugin-runtime-v0.86.2` with marketplace
sync commit `668c325`, and the 0.86.2-native proof
doctor-20260726T014023Z-1b377b (2026-07-26Z; `overall` `pass` with zero
