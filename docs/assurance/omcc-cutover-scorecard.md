...
`0.77.1` release and installed-state refresh (`claude plugin list` and `codex
plugin list` both report the runtime plugin installed and enabled at 0.77.1,
2026-07-09). Current runtime execution evidence is native to the installed
`plugin-runtime` `0.77.1`: permission proof, deep peer smoke, and workflow
continuation proof passed in both directions, re-recorded on 2026-07-09 as
`doctor-20260709T141930Z-515ebf`
(Claude Code `2.1.205` / `codex-cli 0.143.0`; designer `0.2.0` / engineer `0.20.0` /
orchestrator `0.12.0` / founder `0.3.0` / attention `0.4.0` / image `0.2.0`;
`host_parity` `pass`, 0 issues; experience parity `ready` `100%` 8/8; `overall` `pass`). This supersedes the prior
...
The latest `plugin-runtime` `0.77.1` release/install proof loop is the ADR-0042
**designer persona acceptance** — feat PR #528 taught doctor/settings to recognize
designer in the plugin inventory and cut `plugin-runtime` `0.77.1`, alongside
PR #529 (ADR-0042 `Accepted`, `plugin-designer-v0.2.0`), release PR #521, release
tag `plugin-runtime-v0.77.1`, marketplace sync
commit `7dce7fe` (release-please Action follow-up). The 0.77.0-native proof
`doctor-20260709T131625Z-33c54d` read `partial` `91%` on the hook-state defect
above; fix PR #530 and release PR #531 cut `plugin-runtime-v0.77.1` (marketplace
sync commit `73b2a75`), and the current 0.77.1-native proof
`doctor-20260709T141930Z-515ebf` (recorded 2026-07-09 under Claude Code `2.1.205`
/ Codex `0.143.0`; three execute proofs passed both directions; parity `ready`
`100%` 8/8) is the record of reference.
...
| R3 | Switching development tools must work in both directions: Claude Code to Codex and Codex to Claude. | Cross-host tests cover resume and stop-archive behavior for engineer/orchestrator; companions exist in both directions. Installed `plugin-runtime` `0.77.1` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded under the 0.77.1 install on 2026-07-09 KST (`doctor-20260709T141930Z-515ebf`, Claude Code `2.1.205` / Codex `0.143.0`, runtime installed `0.77.1` on both hosts; release tag `plugin-runtime-v0.77.1`; experience parity `ready` `100%` 8/8). The intermediate 0.77.0 record `doctor-20260709T131625Z-33c54d` read `partial` `91%` because doctor mis-read the newly-trusted designer hooks as disabled; the classifier fix released as 0.77.1 restores `ready` `100%`. Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. | satisfied | Keep the explicit installed-runtime proof in the release/install continuity loop whenever the peer path changes. |
