...
Current local dry-run evidence: a fresh 2026-08-02Z `runtime:settings` dry-run
reports all agentic-plugins surfaces available with source/cache versions
matching the current repo manifest (runtime source `0.88.0` = installed
`0.88.0` on both hosts), and zero plugin-management config writes planned
against the installed `plugin-runtime` `0.88.1` state (`claude plugin list`
reports the runtime plugin enabled at 0.88.1 with its Claude cache at 0.88.1,
and `codex plugin list` reports it installed and enabled at 0.88.1 —
claude=installed/0.88.1, codex=installed/0.88.1 — per the 2026-08-03Z
`doctor-20260803T091403Z-7f2850` read (the post-attestation record; the
same-day install proof doctor-20260803T033236Z-f56d25 read the same installed
state); this loop refreshed both hosts host-
...
native to the installed `plugin-runtime` `0.88.1`: permission proof, deep peer
smoke, and workflow continuation proof executed and passed in both directions,
recorded on 2026-08-03Z as `doctor-20260803T091403Z-7f2850` — the
post-attestation record of the **presentation-and-persistence** loop, a five-package patch release (release PR
...
marketplace sync `4e6ee69`, stage-doc sync `0b98de6`). Observed experience parity
is **`ready` `100%` 8/8** (115/115, zero manual follow-ups). The install proof
taken earlier the same day, `doctor-20260803T033236Z-f56d25`, read `partial`
...
The latest `plugin-runtime` `0.88.1` release/install proof loop is the
**presentation-and-persistence** slice, whose 0.88.1-native post-attestation proof
`doctor-20260803T091403Z-7f2850` (2026-08-03Z) reads `ready` `100%` 8/8, after the
