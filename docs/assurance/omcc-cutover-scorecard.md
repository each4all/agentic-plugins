...
historical rather than current evidence; the current manifest is 0.90.3. The
newest hook-review attestation is `settings-20260809T122130Z-c26a0c`, and the
0.90.3 proof no longer reports it current: all four hook-bearing persona
packages bumped in that release and `~/.codex/config.toml` `[hooks.state]` is
now empty, so the attestation is stale until an operator completes a Codex
`/hooks` review and re-runs `runtime:settings --attest-codex-hook-review`. The
current installed
`plugin-runtime` `0.90.3` state is claude=installed/0.90.3,
codex=installed/0.90.3, per the 2026-08-16Z
`doctor-20260816T144610Z-154506` read; the superseded 0.90.2 state was read by
...
The latest `plugin-runtime` `0.90.3` release/install proof loop is the
**Claude 2.1.233 host-parity baseline refresh and withdrawn-todo-tool
adaptation** slice, whose 0.90.3-native proof
`doctor-20260816T144610Z-154506` (2026-08-16Z) reads `partial` `91%` 6/8 with zero
blocked criteria, `host_parity` `pass`, `overall.hard_failures` empty, and
`host_parity_baseline` **`current`**. This slice is also the clearest live case
of the rule stated just above: it bumped all four hook-bearing persona packages,
`~/.codex/config.toml` `[hooks.state]` is now empty, and both of its partials
