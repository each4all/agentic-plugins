...
historical rather than current evidence; the current manifest is 0.97.0. The
...
`plugin-runtime` `0.97.0` state is claude=installed/0.97.0,
codex=installed/0.97.0, per the 2026-08-28Z
`doctor-20260828T035114Z-44ccdf` read; the superseded 0.96.0 state was read by
...
installed `plugin-runtime` `0.97.0`: permission proof, deep peer smoke, and
workflow continuation proof executed and passed in both directions, recorded
on 2026-08-28Z as
`doctor-20260828T035114Z-44ccdf` — the post-release record of the
**permission advisor removal** loop. The prior record was the
...
(named CURRENT here until the 0.96.0 and 0.97.0 loops superseded it; the current
loop is named once, below):
...
The CURRENT loop is the **permission advisor removal** slice: the
...
**The reading this removal most needed is the corpus one, and it is the second
consecutive test of the same machinery.** ADR-0056 deleted a section and shipped
...
The latest `plugin-runtime` `0.97.0` release/installed-state proof loop is the
**permission advisor removal** slice, whose 0.97.0-native proof
`doctor-20260828T035114Z-44ccdf` (2026-08-28Z) reads `partial` `86%` 5/8
...
follow-up), and `host_parity_baseline` **`stale`** with `baseline-direction`
`ahead` — the 2026-08-16 `2.1.233` / `0.147.0` baseline against an observed
`2.1.250` / `0.150.1`, refreshing which is protected-asset work this recovery
is forbidden to do. Its one manual follow-up is `codex-hook-review`: this
...
⚠ **THE SCORE IS UNCHANGED, AND THAT IS THE READING.** `5/8 (99/115)` is
...
quietly retiring the one live assertion that runtime relaxes nothing. The newest
compat run still reads `legacy_era`, the era gate refusing an assurance-era
verdict as history rather than silently re-interpreting its `current` under this
era's weaker meaning.
