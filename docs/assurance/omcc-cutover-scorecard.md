...
`plugin-runtime` `0.95.0` state is claude=installed/0.95.0,
codex=installed/0.95.0, per the 2026-08-27Z
`doctor-20260827T055919Z-cc5c4b` read; the superseded 0.94.0 state was read by
...
installed `plugin-runtime` `0.95.0`: permission proof, deep peer smoke, and
workflow continuation proof executed and passed in both directions, recorded
on 2026-08-27Z as
`doctor-20260827T055919Z-cc5c4b` — the post-release record of the
**verdicts that never left the run** loop. The prior record was the
...
`e6c1b26`, with `host_parity` `pass` under Claude Code `2.1.247` / Codex
`0.150.1`. `overall` reads **`warning`** — `hard_failures` is empty, and the
one warning is "latest compatibility check needs follow-up"; no hard failures
is not the same claim as `pass`. The process exit code is `0`, which is a
reading here rather than a constant: this is the first proof recorded under
the ladder the same release shipped.

**No install was performed by this recovery** — both hosts already served
0.95.0 when it began — so what follows is a verification rather than an
install proof, and it was taken as a direct comparison rather than a digest
match: `diff -r` over Claude's cache and **Codex's served tree**, excluding
Claude's `.orphaned_at` supersede marker, reports no difference. Which tree
Codex serves is not a guess, and this recovery got it wrong once: it is the
marketplace checkout `codex plugin list --json` reports, under
`~/.codex/.tmp/marketplaces/`, not the `~/.codex/plugins/cache` path first
compared here. Both hold 128 files, the same file set as 0.94.0, because this
release only modified existing files. The packaged baseline hashes
`86fdf17e4418884f01b67785eeaaacc74923a8a01efa238f514bf270da600843` identically
across both caches, the `plugin-runtime-v0.95.0` tag and the repository, which
is also the value doctor independently records as
`host_parity_baseline.provenance.content_sha256` — unchanged from 0.93.0 and
0.94.0, because none of those releases touched the packaged baseline.
...
default collation, NOT `LC_ALL=C`; hash the listing. For 0.95.0 that is
`2a237c3ff0bcc7e3161e7694c2ca1f3a8ef3d67e4a3d67acf8929e75324d4028` on both
trees. Pinning it in a script, so the locale stops being ambient, is a named
...
The latest `plugin-runtime` `0.95.0` release/install proof loop is the
**verdicts that never left the run** slice, whose 0.95.0-native proof
`doctor-20260827T055919Z-cc5c4b` (2026-08-27Z) reads `partial` `81%` 5/9
(105/130) with **zero blocked criteria**, `host_parity` `pass`, `overall`
...
`2.1.247` / `0.150.1`, refreshing which is protected-asset work this recovery
...
`/hooks` re-attestation the only way to clear it. The superseded 0.94.0
