...
plugin-management config writes planned after the `plugin-runtime` `0.66.1`
release and installed-state refresh (Claude `claude plugin update` and Codex
`codex plugin marketplace upgrade` cache materialization both report 0.66.1, 2026-06-11). Current runtime execution evidence is
carried forward by the installed `plugin-runtime` `0.66.1` loop: permission
...
recorded under the 0.61.0 install; the 0.62→0.66.1 bumps are runtime-only — 0.66.0
...
`--apply-codex-plugin-hooks` host-config write, and 0.66.1 makes the Codex
peer-direction hook-gate readiness warning stage-aware (ADR-0030) — and none touch the
...
The latest `plugin-runtime` `0.66.1` release/install proof loop is carried by
feature PRs #403 (ADR-0035 §5/§6 explicit `codex plugin add` install executor),
#405 (ADR-0035 §6 `--apply-codex-plugin-hooks` hard-removal; satellite docs #406), and
#410 (ADR-0030 stage-aware Codex peer-direction hook-gate readiness warning) and
release PRs #404 and #411 (runtime-only; no co-released hook-bearing plugins),
release tag `plugin-runtime-v0.66.1`, marketplace sync in this 0.66.1 proof-sync PR, and
