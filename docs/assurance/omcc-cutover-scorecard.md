...
Current local dry-run evidence: a fresh 2026-07-22Z `runtime:settings` dry-run
...
`0.85.0` = installed `0.85.0` on both hosts), and zero plugin-management
config writes planned against the installed `plugin-runtime` `0.85.0` state
(both the Claude plugin cache and `codex plugin list --json` report the
runtime plugin installed and enabled at 0.85.0 — claude=installed/0.85.0,
codex=installed/0.85.0 — with attention 0.9.0 alongside engineer 0.21.0 and
orchestrator 0.13.0, per the 2026-07-22Z `doctor-20260722T012908Z-472538`
read; the Release B installs again ran host-native — Claude `claude plugin
update runtime@agentic-plugins` + `claude plugin update
attention@agentic-plugins` and the Codex cache refreshed through `codex plugin
marketplace upgrade` — doctor-observed on both hosts, so this upgrade too
left no `runtime:settings --execute-plugin-management` executor artifact of
its own, the newest being the 2026-07-14 `settings-20260714T021101Z-94f0f6`). Current runtime
execution evidence is native to the installed
`plugin-runtime` `0.85.0`: permission proof, deep peer smoke, and workflow
continuation proof executed and passed in both directions, recorded on
2026-07-22Z as `doctor-20260722T012908Z-472538` — the **ADR-0047 Release B**
loop (the §6 bounded expired-claim sweep `sweepExpiredClaims` behind the
non-recursive `withReclaimLock` lock repair, and the §7 citation-aware
artifact retention: the read-only `planRetention` planner, the
`applyRetention` M1 deleting executor gated on a reviewed plan hash, and the
`runtime:retention` CLI — §6 commits `1dc3bc4`/`949aa65`, §7 commits
`123e9a0`/`e9d5a1c`/`1a3c5c6`/`0bdbdd5`; release PR #616 `chore: release
main` merge `c2bc0f9`, tags `plugin-attention-v0.9.0` +
`plugin-runtime-v0.85.0` (attention 0.9.0 = the ADR-0047 §1/§2
`response-needed` Stop-finality classifier + headline producers, the
Claude-side producer beyond 0.8.0's vocabulary-only enum mirror), marketplace
sync `36057ad`; Claude Code `2.1.217` / `codex-cli 0.145.0`; the doctor
`entry_brief` readiness section reads `ready` — the operator's ADR-0045
user-scope `startup` opt-in with `empty=silent`, the
`floors.entry_brief=0.83.0` pin satisfied by `0.85.0`, the entry executor
present — alongside `session_capture` `ready`). `overall` reads `pass` with
zero warnings; Codex hook state reads **12/12** expected `enabled_trusted`
with `unexpected_agentic_entries=0`; **`host_parity_baseline` reads
`current`** — the dual Claude `2.1.216`→`2.1.217` / Codex `0.144.6`→`0.145.0`
drift this loop's first proof run exposed was closed the same day by the
compat cycle (content-backed `CHANGELOG.md` + Codex releases-atom ingest in
run `compat-20260722T011840Z-a3fb14`, final drift=none re-check in
`compat-20260722T012840Z-915f54`; Claude additive/fix-only and Codex a
no-plugin-CLI/hook-contract-break minor, the notable `codex features list`
stage change being `multi_agent_v2` under-development→stable). **Experience
parity deliberately reads `partial` `91%` (6/8 satisfied; manual follow-up
`codex-hook-review`)** — the honest fail-closed half of the attestation
discipline this cycle: the **Codex CLI `0.144.6`→`0.145.0` minor bump
version-invalidated** the four-plugin `/hooks` attestation
`settings-20260720T151554Z-3b543f` (`bound_versions.codex=0.144.6`), which
the currency evaluation now reads `stale`
(`currency_reason=codex_cli_version_changed`) under the contract §11.2 #24
currency binding. The trusted Codex hook entries themselves are unchanged and
healthy (12/12 `enabled_trusted`, `expected_untrusted=0`,
`expected_disabled=0`); neither released package changed a Codex hook-bearing
plugin — runtime ships no hooks, and attention `0.9.0` changed only the
**Claude** hook surface while keeping zero Codex hook surface, with the four
attested plugins (designer@0.3.0 / engineer@0.21.0 / founder@0.4.0 /
orchestrator@0.13.0) unchanged — so a fresh operator `/hooks` confirmation
plus `runtime:settings --attest-codex-hook-review` restores `ready`, but that
is an **owner-gated active-Codex-session step not performed this cycle**, so
parity is honestly recorded `partial` rather than claimed `ready`. This
...
The latest `plugin-runtime` `0.85.0` release/install proof loop is the
**ADR-0047 Release B** slice — the §6 bounded expired-claim sweep
`sweepExpiredClaims` (behind the non-recursive `withReclaimLock` lock repair)
and the §7 citation-aware artifact retention (`planRetention` read-only
planner, `applyRetention` M1 deleting executor gated on a reviewed plan hash,
`runtime:retention` CLI), §6 commits `1dc3bc4`/`949aa65` and §7 commits
`123e9a0`/`e9d5a1c`/`1a3c5c6`/`0bdbdd5`; release PR #616 (`chore: release
main`, merge `c2bc0f9`) cut tags `plugin-attention-v0.9.0` and
`plugin-runtime-v0.85.0` with marketplace sync commit `36057ad`, and the
0.85.0-native proof doctor-20260722T012908Z-472538 (2026-07-22Z; `overall`
`pass` with zero warnings, three execute proofs executed and passed in both
directions, Codex hook state `12/12` expected `enabled_trusted` with
`unexpected_agentic_entries=0`; the doctor `entry_brief` section reads
`ready` — the operator's ADR-0045 user-scope `startup` opt-in, the
`floors.entry_brief=0.83.0` pin satisfied by `0.85.0` — alongside
`session_capture` `ready`; `host_parity_baseline` `current` after the
same-day dual-drift compat cycle — content-backed ingest of the Claude
`CHANGELOG.md` + Codex releases-atom in compat-20260722T011840Z-a3fb14,
drift=none re-check in compat-20260722T012840Z-915f54; experience parity
**deliberately `partial` `91%`** with the manual `codex-hook-review`
follow-up — the Codex CLI `0.144.6`→`0.145.0` minor bump version-invalidated
the four-plugin attestation `settings-20260720T151554Z-3b543f`
(`currency_reason=codex_cli_version_changed`), with the trusted hook entries
themselves unchanged and healthy (`12/12` `enabled_trusted`) and neither
released package touching a Codex hook-bearing plugin, so a fresh
owner-gated `/hooks` review plus `runtime:settings --attest-codex-hook-review`
— not performed this cycle — restores `ready`) was taken against that
released binary, not a patched working tree. It supersedes the 0.84.0-native
