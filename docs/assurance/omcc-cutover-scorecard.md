...
`plugin-runtime` `0.96.0` state is claude=installed/0.96.0,
codex=installed/0.96.0, per the 2026-08-27Z
`doctor-20260827T235051Z-c11408` read; the superseded 0.95.0 state was read by
...
installed `plugin-runtime` `0.96.0`: permission proof, deep peer smoke, and
workflow continuation proof executed and passed in both directions, recorded
on 2026-08-27Z as
`doctor-20260827T235051Z-c11408` — the post-release record of the
**assurance matcher removal** loop. The prior record was the
...
`443ad73`. The CURRENT loop is the **verdicts that never left the run** slice:
...
The CURRENT loop is the **assurance matcher removal** slice: the
...
`0a73715`, tag `plugin-runtime-v0.96.0`, marketplace sync `06ec398`, stage-doc
sync `375f5ad`, with `host_parity` `pass` under Claude Code `2.1.248` / Codex
`0.150.1`. `overall` reads **`warning`** on the same single warning.

⚠ **`plugin-runtime-v0.96.0` is the first release in four to move the packaged
baseline's hash** (`86fdf17e…` → `bdb6eeb7…`, the same truncated-digest form this
record already uses for whole-tree digests), because it is the release that cut
the assurance section out of that protected file. It did **not** refresh the
dated header, and that is correct rather than an omission: refreshing the header
asserts a new reviewed host observation, and this loop reviewed no host.

**An install WAS performed by this recovery**, along the host-specific verbs the
asymmetry note below records: `claude plugin update runtime@agentic-plugins`
reported 0.95.0 → 0.96.0 at user scope, and `codex plugin marketplace upgrade`
refreshed the Git snapshot and moved the served tree in place. Both were
confirmed by re-reading the installed manifests rather than by trusting the
command output.

⚠ **One documented precondition went unexercised, and it is recorded rather than
quietly dropped.** The note below says Claude's `directory` source needs
`claude plugin marketplace update` BEFORE `claude plugin update`, or the cached
catalog reports nothing to do. This recovery ran the update alone and it
resolved 0.96.0 correctly. That is a single observation with no control — the
catalog may simply have been fresh — so it is **not** evidence the trap is gone,
and the note stands. What it does establish is that the trap is not
unconditional, which the note's wording implies.

The byte comparison is then a direct one rather than a digest match: `diff -r`
over Claude's cache and **Codex's served tree**, excluding Claude's
`.orphaned_at` supersede marker, exits 0 with no output. Which tree Codex serves
is not a guess, and an earlier recovery got it wrong once: it is the marketplace
checkout `codex plugin list` reports, under `~/.codex/.tmp/marketplaces/`, not
`~/.codex/plugins/cache`. Both hold **126** files — down from 0.95.0's 128,
which is the removal — and the whole-tree digest is `725ad396…` on both under
the path-first recipe below.

⚠ **The packaged baseline hash MOVED this loop**, to
`bdb6eeb7e5b343641905de877ac562057a7841f736c6cafd0ffa65f56fbc3285`, identical
across both caches and the repository and independently recorded by doctor as
`host_parity_baseline.provenance.content_sha256`. It had held at
`86fdf17e…` through 0.93.0, 0.94.0 and 0.95.0 precisely because none of those
releases touched the packaged baseline; this one cut a 200-line section out of
it. A hash that had been evidence of stability is, this once, evidence of the
edit — and reading its change as drift would invert what it says.
...
stays comparable with every record before it. For each file under the cache
root except the `.orphaned_at` supersede marker, emit the root-relative path,
a space, then its `shasum -a 256`; sort that listing under the login shell's
default collation, NOT `LC_ALL=C`; hash the listing. For 0.96.0 that is
`725ad3967057a8cfae3b285eb25e75d47567e0498e31dd8da90c848272e84fbe` on both
trees, over 126 files; the superseded 0.95.0 value was
...
The latest `plugin-runtime` `0.96.0` release/install proof loop is the
**assurance matcher removal** slice, whose 0.96.0-native proof
`doctor-20260827T235051Z-c11408` (2026-08-27Z) reads `partial` `86%` 5/8
...
`2.1.248` / `0.150.1`, refreshing which is protected-asset work this recovery
...
⚠ **THE DENOMINATOR MOVED, AND THE SCORE IS NOT COMPARABLE TO THE ONE ABOVE IT.**
`5/8 (99/115)` against the prior `5/9 (105/130)` is not a regression and not an
improvement: [ADR-0056](../adr/0056-assurance-matcher-removal.md) §Decision 8
removed `host_compatibility_assurance`, a weight-15 criterion that could never be
satisfied, so both the numerator and the denominator lost it. The three partial
criteria are the same three, unchanged. Reading `81% → 86%` as progress would be
reading an arithmetic identity as work.

**What this proof measures that no earlier one could.** It is the first recorded
proof taken against a runtime with no assurance layer, and three of its readings
are the removal's own acceptance evidence. `doctor_runs` reads `available
malformed=0` across **79** retained artifacts — the dual reader carries the whole
corpus, which is the failure ADR-0056 §Consequences predicted (`blocked
malformed=70`) for a producer that bumped without one. `experience_parity`
reports **8** criteria against a **115** total, the denominator §Decision 8 named
in advance. And the newest compat run reads `legacy_era`, which is the era gate
refusing an assurance-era verdict as history rather than silently re-interpreting
its `current` under this era's weaker meaning. The superseded 0.94.0
...
**receiver-delegation** proof doctor-20260824T112523Z-679846 (2026-08-24Z). It is the first recorded proof taken against a shipped, non-empty
grant set, and it still reads **`unassured`** — but for a new reason: the
released grant names claude `2.1.234`/`2.1.235` with codex `0.147.0`, while
the machine reads claude `2.1.237` / codex `0.148.0`. This is the
**cohort-miss** refusal, distinct from the empty-set refusal R1 exercised, and
`record_status` `resolved` confirms every other layer worked. The refusal is
