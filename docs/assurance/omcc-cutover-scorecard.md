...
`0.80.0` release and installed-state refresh (the refresh itself was executed
through the `runtime:settings --execute-plugin-management` executor, artifact
`settings-20260713T025027Z-8f625a` — 4 update commands executed, 0 failed; the
Claude plugin cache and `codex plugin list --json` both report the runtime
plugin installed and enabled at 0.80.0 with engineer 0.21.0 and orchestrator
0.13.0, 2026-07-13Z). Current runtime execution evidence is native to the
installed `plugin-runtime` `0.80.0`: permission proof, deep peer smoke, and
workflow continuation proof passed in both directions, recorded on 2026-07-13Z
as `doctor-20260713T025136Z-7758d3` — the **S9 completion-output-contract
loop** (feature PR #558 rebase-merged as `00dbc80`/`85bbd3d`/`4c8e59f` per the
ADR-0016 three-package split; release PR #559 squash `1d02390`, tag
`plugin-runtime-v0.80.0` alongside plugin-engineer-v0.21.0 and
plugin-orchestrator-v0.13.0, marketplace sync `ddb43d4`; Claude Code `2.1.207`
/ `codex-cli 0.144.1`). `overall` reads `pass`; `host_parity_baseline` reads
`current` against the 2026-07-11 baseline (the #550 refresh); Codex hook state
reads **12/12** expected `enabled_trusted` with `unexpected_agentic_entries=2`
(the retained pre-relocation attention rows, display-only). **Experience
parity deliberately reads `partial` `91%`** (manual follow-up
`codex-hook-review`): the engineer `0.21.0` / orchestrator `0.13.0`
hook-bearing upgrades version-invalidated the four-plugin `/hooks` attestation
`settings-20260712T015100Z-312fbb` (attested at engineer 0.20.1 / orchestrator
0.12.1), so a fresh operator `/hooks` confirmation plus `runtime:settings
--attest-codex-hook-review` is required before parity reads `ready` again —
the trusted hook entries themselves are unchanged (no hook file changed in
this release, so trust hashes still match). This supersedes the 2026-07-12Z
...
The latest `plugin-runtime` `0.80.0` release/install proof loop is the
**S9 completion-output-contract** slice — feature PR #558 (rebase-merged
`00dbc80`/`85bbd3d`/`4c8e59f`) added the runtime-owned
...
sanitized workflow-checkpoint footer line, and the engineer/orchestrator
sidecar gate-naming floor; release PR #559 cut tag `plugin-runtime-v0.80.0`
(with plugin-engineer-v0.21.0 / plugin-orchestrator-v0.13.0) and marketplace
sync commit `ddb43d4`, and the 0.80.0-native proof
`doctor-20260713T025136Z-7758d3` (2026-07-13Z; `overall` `pass`,
`host_parity_baseline` `current`, three execute proofs passed both
...
orchestrator 0.13.0) was taken against that released binary — it is the
record of reference. The preceding 0.79.0 loop was the ADR-0043 S2
