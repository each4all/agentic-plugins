...
0.97.4. The newest settings artifact, `settings-20260923T055128Z-284df5`, is
the 2026-09-23Z hook-review attestation recorded under 0.97.4 after the owner
re-reviewed Codex `/hooks`: the Codex CLI move `0.155.0` → `0.156.1` had
staled the previous one (`currency_reason` `codex_cli_version_changed`), and
the relocation releases had bumped all four hook-bearing packages since it was
recorded. It binds designer@0.3.8 / engineer@0.21.10 / founder@0.4.8 /
orchestrator@0.13.7 against Codex `0.156.1`, the 0.97.4 proof reports it
current with `manual-followups=0`, and its persisted artifact carries dry-run
plugin-management and cleanup plans with zero planned actions but no
surface-availability or source/cache version readings. It supersedes
...
rather than a live gap. The current installed `plugin-runtime` `0.97.4` state
is claude=installed/0.97.4, codex=installed/0.97.4 (Codex's reading from a
successful `codex plugin list` rather than the cache fallback), per the
2026-09-23Z `doctor-20260923T055158Z-4fd9af` read; the superseded 0.97.3 state
was read by doctor-20260918T040805Z-cd9948 (2026-09-18Z), the 0.97.2 state
before it by
...
installed `plugin-runtime` `0.97.4`: permission proof, deep peer smoke, and
workflow continuation proof executed and passed in both directions, recorded
on 2026-09-23Z as `doctor-20260923T055158Z-4fd9af` — the post-release record
of the **capabilities Claude registered twice** loop. The prior record was the
**three readers that answered for a state they never measured** loop, recorded
on 2026-09-18Z as doctor-20260918T040805Z-cd9948; before it the **cutover
...
The CURRENT loop is the **capabilities Claude registered twice** slice —
...
`compat-20260918T040728Z-6f1daa`, which this proof selected again because no
compat run has been taken since. The live host pair has moved to
`2.1.280` / `0.156.1`, and `host_parity_baseline` reads it directly — `stale`,
...
before that commit, still matches the tags. Neither difference is executable,
but one version label currently names two trees across the hosts for those two
packages. The mechanism, inferred from the two install models rather than
...
`0.155.0`; the CLI is now `0.156.1`, and the relocation releases had bumped
...
`doctor-20260923T055158Z-4fd9af` (2026-09-23Z) reads `blocked` `87%` 7/8
(100/115) with zero manual follow-ups, `host_parity` `pass`, `overall`
`warning` (zero hard failures, one warning — `latest compatibility check
requires release notes`), and `host_parity_baseline` **`stale`** with
`direction` `ahead` — the 2026-08-28 `2.1.250` / `0.150.1` baseline against an
observed `2.1.280` / `0.156.1`. None of those readings is this loop's effect:
the superseded 0.97.3 proof doctor-20260918T040805Z-cd9948 (2026-09-18Z),
taken before any of the six relocation releases, read the same `blocked` `87%`
on the same criterion over the same compat run, against an observed
`2.1.276` / `0.155.0`. That proof's loop was the **three readers that answered
...
enabled, 0 untrusted, 0 missing. This loop opened that gate twice over — the
...
read again at 12/12 enabled, 0 untrusted, 0 missing. The
