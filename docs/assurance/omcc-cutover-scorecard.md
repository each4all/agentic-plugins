...
plugin-management config writes planned after the `plugin-runtime` `0.71.0`
release and installed-state refresh (`claude plugin list` reports the runtime
plugin installed and enabled at 0.71.0, 2026-07-04). Current runtime execution evidence is
native to the installed `plugin-runtime` `0.71.0`: permission proof, deep peer
smoke, and workflow continuation proof passed in both directions, re-recorded
on 2026-07-04 as `doctor-20260704T071736Z-ead466` (Claude Code `2.1.201` /
`codex-cli 0.142.5`; engineer `0.19.0` / orchestrator `0.11.0` / founder `0.2.0` /
image `0.2.0`; experience parity `ready` `100%`, 8/8 satisfied). The prior
0.70.1 record `doctor-20260702T060924Z-a61f92` (Claude Code `2.1.198` /
`codex-cli 0.142.5`, parity `ready` `100%`) was superseded when the 0.71.0
release train bumped the installed runtime and the Claude Code `2.1.198` →
`2.1.201` host drift closed its reuse gate, requiring this fresh re-record.
The Codex `/hooks` review/trust attestation recorded on 2026-07-02
(`settings-20260702T060856Z-c8cfdd`) for the installed hook-bearing set
`engineer@0.19.0` / `orchestrator@0.11.0` / `founder@0.2.0` remains current —
the 0.71.0 release changes no hook-bearing plugin (`image`/`runtime` ship no
Codex hooks manifest) — keeping observed experience parity `ready`.
Doctor hook-state
...
The latest `plugin-runtime` `0.71.0` release/install proof loop is carried by
the ADR-0040 operator-observability slices (#473 notify-schema contract lib,
#475 `notify_*` settings keys, #476 `notify.mjs` emitter, #477 reader-lib
extraction, #478 `--notification-plan` Codex fragments, #479
`runtime:dashboard` Tier 1+2), release PR #474, release tag `plugin-runtime-v0.71.0`,
marketplace sync commit `e051a90` (release-please Action follow-up), and the 0.71.0-native proof doctor artifact
`doctor-20260704T071736Z-ead466` (recorded 2026-07-04 under Claude Code `2.1.201` /
Codex `0.142.5`, parity `ready` `100%`). The proof was freshly re-recorded with all six plugins
(companions / engineer / founder / image / orchestrator / runtime) installed. The 2026-07-02
`/hooks` review/trust attestation (`settings-20260702T060856Z-c8cfdd`)
...
8/8 satisfied) with no pending manual follow-up. The prior parity-`ready` state was the
0.70.1 loop (ADR-0039 follow-up #469, release PR #465, doctor artifact
`doctor-20260702T060924Z-a61f92`, settings attestation
`settings-20260702T060856Z-c8cfdd`), whose runtime release predates
...
| R3 | Switching development tools must work in both directions: Claude Code to Codex and Codex to Claude. | Cross-host tests cover resume and stop-archive behavior for engineer/orchestrator; companions exist in both directions. Installed `plugin-runtime` `0.71.0` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded under the 0.71.0 install on 2026-07-04 KST (`doctor-20260704T071736Z-ead466`, Claude Code `2.1.201` / Codex `0.142.5`, experience parity `ready` `100%` with the still-current 2026-07-02 `/hooks` attestation `settings-20260702T060856Z-c8cfdd`). Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. | satisfied | Keep the explicit installed-runtime proof in the release/install continuity loop whenever the peer path changes. |
