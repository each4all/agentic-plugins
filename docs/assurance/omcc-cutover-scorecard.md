...
plugin-management config writes planned after the `plugin-runtime` `0.70.0`
release and installed-state refresh (`claude plugin list` reports the runtime
plugin installed and enabled at 0.70.0, 2026-06-30). Current runtime execution evidence is
native to the installed `plugin-runtime` `0.70.0`: permission proof, deep peer
smoke, and workflow continuation proof passed in both directions, re-recorded
on 2026-06-30 as `doctor-20260630T230435Z-21cb07` (Claude Code `2.1.197` /
`codex-cli 0.142.3`; engineer `0.18.0` / orchestrator `0.10.0` / founder `0.2.0` /
image `0.2.0`; experience parity `ready` `100%`, 8/8 criteria). The 0.70.0 release
shipped the ADR-0038 permission advisor (`runtime:doctor --permission-diagnosis`,
`runtime:settings --permission-plan`), which bumped the runtime manifest version and
invalidated the prior 0.69.0 proof's reuse gate, requiring this fresh re-record. The Codex `/hooks` review/trust attestation refreshed on
2026-06-23 (`settings-20260623T125701Z-c24057`) for the hook-bearing set
`engineer@0.18.0` / `orchestrator@0.10.0` / `founder@0.2.0` still covers the
current state unchanged (`image` ships no Codex hooks manifest); doctor hook-state
...
The latest `plugin-runtime` `0.70.0` release/install proof loop is carried by
the nine-slice ADR-0038 permission-advisor macro (final acceptance slice #460
`fbe21b1`), release PR #452, release tag `plugin-runtime-v0.70.0`,
marketplace sync commit `e6367f0` (release-please Action follow-up), and the 0.70.0-native proof doctor artifact
`doctor-20260630T230435Z-21cb07` (recorded 2026-06-30 under Claude Code `2.1.197` /
Codex `0.142.3`, parity `ready` `100%`). The proof was freshly re-recorded with all six plugins
...
for the `engineer@0.18.0`/`orchestrator@0.10.0`/`founder@0.2.0` hooks (all 9 bundled hook
entries enabled and trusted) still covers the unchanged hook-bearing set, so the
current `runtime:doctor` run reports observed experience parity `ready` (`100%`,
8/8 criteria). The prior fully re-attested parity-`ready` state was the `0.68.1`
loop (release PR #430, doctor artifact `doctor-20260623T131948Z-13a7b3`, settings
...
| R3 | Switching development tools must work in both directions: Claude Code to Codex and Codex to Claude. | Cross-host tests cover resume and stop-archive behavior for engineer/orchestrator; companions exist in both directions. Installed `plugin-runtime` `0.70.0` carries the native `runtime:doctor --permission-proof --execute-permission-proof --deep-peer-smoke --execute-deep-peer-smoke --workflow-continuation-proof --execute-workflow-continuation-proof` proof re-recorded under the 0.70.0 install on 2026-06-30 KST (`doctor-20260630T230435Z-21cb07`, Claude Code `2.1.197` / Codex `0.142.3`, experience parity `ready` `100%`). Both `claude -> codex` and `codex -> claude` passed the permission proof, deep peer smoke, and engineer workflow continuation proof through `state.mjs create`, `dispatch-peer.mjs`, `pending_ensemble`, and `ensemble_results` in an ephemeral temp repo. | satisfied | Keep the explicit installed-runtime proof in the release/install continuity loop whenever the peer path changes. |
| R4 | Claude Code and Codex user experience must be equivalent where possible; non-portable host-specific features must not become hard dependencies. | Architecture documents host-specific adapter boundaries; runtime baseline documents command/hook/subagent differences; Codex macro/meta skill mirrors exist. The native `runtime:doctor` execute proofs re-recorded under the 0.70.0 install on 2026-06-30 (`doctor-20260630T230435Z-21cb07`, Claude Code `2.1.197` / Codex `0.142.3`) confirmed bidirectional peer execution and engineer workflow continuation passed both directions, and `runtime:settings` reports source/cache version parity with zero plugin-management config writes planned. The Codex `/hooks` review/trust attestation from 2026-06-23 (`settings-20260623T125701Z-c24057`; `~/.codex/config.toml` hook-state `untrusted=0`/`disabled=0`) for the `engineer@0.18.0`/`orchestrator@0.10.0`/`founder@0.2.0` hooks (all 9 bundled hook entries enabled and trusted) still covers the unchanged hook-bearing set (`image` ships no Codex hooks manifest), so observed experience parity reads `ready` (score `100%`, 8/8 criteria). | satisfied | Refresh the active-session `/hooks` review/trust attestation after hook-bearing plugin upgrades or hook packaging changes; do not claim observed parity `ready` until the post-attestation doctor/cutover evidence is recorded. |
