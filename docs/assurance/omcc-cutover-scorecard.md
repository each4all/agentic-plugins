...
`plugin-runtime` `0.93.0` state is claude=installed/0.93.0,
codex=installed/0.93.0, per the 2026-08-25Z
`doctor-20260825T070411Z-d6ecb2` read; the superseded 0.92.0 state was read by
...
installed `plugin-runtime` `0.93.0`: permission proof, deep peer smoke, and
workflow continuation proof executed and passed in both directions, recorded
on 2026-08-25Z as
`doctor-20260825T070411Z-d6ecb2` — the post-release record of the
**portable-profile session-config** loop. The prior record was the
**receiver-delegation** loop, cut early by owner decision so the receiver
change would be observed rather than believed: the context-state provenance
fix `c87dc81` (PR #730), the packaged receiver API `e6e53fb` (PR #731) and the
installed-receiver classifier `c94c5dd` (PR #732); release PR
[#723](https://github.com/each4all/agentic-plugins/pull/723) squash `70ba210`,
tag plugin-runtime-v0.92.0, marketplace sync `e54b7fd`, stage-doc sync
`d577677`. The CURRENT loop is the **portable-profile session-config** slice,
a single runtime change: the machine profile's 1.1 → 1.2 carriage of the
session-config family (`session_capture`, `entry_brief`, `entry_brief_empty`)
as trailing bare scalars, a `projectSession` user-global reader, and seed
proposals carrying a runtime-derived `user_scope_only` label `a973d05`
(PR #734); release PR
...
`4ac05c9`, with `host_parity` `pass` under Claude Code `2.1.243` / Codex
`0.149.1`. `overall` reads **`warning`** — `hard_failures` is empty, and the
...
string — both caches hold an identical 127-file 0.93.0 package (whole-tree
digest
`92f449d9ee4c83f24a3879663194553964d69322359950aa46f13f81e8e91d0c`), and the
packaged baseline hashes
`86fdf17e4418884f01b67785eeaaacc74923a8a01efa238f514bf270da600843` identically
across both caches, the `plugin-runtime-v0.93.0` tag and the repository at
`4ac05c9`, which is also the value doctor independently records as
`host_parity_baseline.provenance.content_sha256` — unchanged from 0.92.0,
because this release did not touch the packaged baseline.
...
The latest `plugin-runtime` `0.93.0` release/install proof loop is the
**portable-profile session-config** slice, whose 0.93.0-native proof
`doctor-20260825T070411Z-d6ecb2` (2026-08-25Z) reads `partial` `81%` 5/9
...
`2.1.243` / `0.149.1`, refreshing which is protected-asset work this recovery
...
`/hooks` re-attestation the only way to clear it. The superseded 0.92.0
**receiver-delegation** proof `doctor-20260824T112523Z-679846` (2026-08-24Z)
read the same shape. It is the first recorded proof taken against a shipped, non-empty
