...
plugin-management config writes planned after the `plugin-runtime` `0.65.0`
release and installed-state refresh. Current runtime execution evidence is
carried forward by the installed `plugin-runtime` `0.65.0` loop: permission
...
recorded under the 0.61.0 install; the 0.62→0.65 bumps are runtime-only — 0.65.0
reads Codex installed-state from `codex plugin list --json` (ADR-0034) via the
shared `resolveCodexInstallState` resolver in `doctor.mjs` — and does not touch
the peer/dispatch path). The Codex `/hooks` review/trust
...
The latest `plugin-runtime` `0.65.0` release/install proof loop is carried by
feature PR #394 (the ADR-0034 Codex installed-state read from `codex plugin list`) and
release PR #395 (runtime-only; no co-released hook-bearing plugins),
release tag `plugin-runtime-v0.65.0`, marketplace sync commit `f65ab31`, and
