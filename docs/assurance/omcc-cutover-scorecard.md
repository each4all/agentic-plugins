...
Current local dry-run evidence: a fresh 2026-07-28Z `runtime:settings` dry-run
...
`0.86.3` = installed `0.86.3` on both hosts), and zero plugin-management
config writes planned against the installed `plugin-runtime` `0.86.3` state
(both the Claude plugin cache and `codex plugin list --json` report the
runtime plugin installed and enabled at 0.86.3 — claude=installed/0.86.3,
codex=installed/0.86.3 — per the 2026-07-28Z `doctor-20260728T013239Z-2a6995`
read; this loop's Claude install ran host-native `claude plugin marketplace
update agentic-plugins` + `claude plugin update runtime@agentic-plugins`,
while the Codex cache was already at 0.86.3 without an explicit command — its
marketplace source is the git remote, so the release landed there on refresh —
doctor-observed on both hosts, so this upgrade too
...
`plugin-runtime` `0.86.3`: permission proof, deep peer smoke, and workflow
continuation proof executed and passed in both directions, recorded on
2026-07-28Z as `doctor-20260728T013239Z-2a6995` — the **#645 Codex /hooks
...
`plugin-runtime-v0.86.3`, marketplace sync `5bb88c1`; Claude Code `2.1.220` /
...
The latest `plugin-runtime` `0.86.3` release/install proof loop is the
**#645 Codex /hooks attestation import** slice — bootstrap read the operator
...
main`, squash `9abddc6`) cut tag `plugin-runtime-v0.86.3` with marketplace
