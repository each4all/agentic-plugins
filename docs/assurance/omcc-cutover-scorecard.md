...
Current local dry-run evidence: a fresh 2026-07-25Z `runtime:settings` dry-run
...
`0.86.1` = installed `0.86.1` on both hosts), and zero plugin-management
config writes planned against the installed `plugin-runtime` `0.86.1` state
(both the Claude plugin cache and `codex plugin list --json` report the
runtime plugin installed and enabled at 0.86.1 — claude=installed/0.86.1,
codex=installed/0.86.1 — per the 2026-07-25Z `doctor-20260725T020157Z-61e7d6`
read; the ADR-0048 installs again ran host-native — Claude `claude plugin
update runtime@agentic-plugins` and the Codex cache refreshed through
`codex plugin marketplace upgrade` + `codex plugin add
runtime@agentic-plugins` — doctor-observed on both hosts, so this upgrade too
...
`plugin-runtime` `0.86.1`: permission proof, deep peer smoke, and workflow
continuation proof executed and passed in both directions, recorded on
2026-07-25Z as `doctor-20260725T020157Z-61e7d6` — the **ADR-0048
bootstrap-observability** loop (run schema 1.2 + evidence-contract vnext with
kind-discriminated fail-closed proof validators and the `notify.configured` /
`notify.codex.configured` split, the mode-bound exact Codex notify wiring
probe with per-OS receiver argv, the agentic-6 statusline adapter with
per-host exact probes and a render-only credential-free shim, the
triple-consent egress-ack-proof doctor executor, and the single-merge-owner
integration; feature PRs #631 f9e7d07 / #632 cd27d2d / #633 84b2c86 /
#634 ac5803a / #635 73127e3 atop the pre-macro activation-semantics fix
#629 3615dcc, then the 0.86.1 egress-ack intent-WAL hardening #637 b984dc8;
release PR #638 `chore: release main` squash `82cf981`, tag
`plugin-runtime-v0.86.1`, marketplace sync `0eb8807`; Claude Code `2.1.220` /
...
current); **`host_parity_baseline` reads `current`** — the Claude
`2.1.217`→`2.1.218` patch drift this loop's first doctor read exposed was
closed the same day by the compat cycle (content-backed `CHANGELOG.md` ingest
via explicit `--fetch-release-notes-url` in run
`compat-20260723T124245Z-8aabdc`, baseline refresh, drift=none re-check
`compat-20260723T124641Z-738a00`; Claude 2.1.218 additive/fix-only with no
plugin-CLI or hook-contract change, Codex `0.145.0` unchanged) — with the
...
The latest `plugin-runtime` `0.86.1` release/install proof loop is the
**ADR-0048 bootstrap-observability** slice — run schema 1.2 +
evidence-contract vnext (kind-discriminated fail-closed proof validators;
the `notify.configured` / `notify.codex.configured` split), the mode-bound
exact Codex notify wiring probe with per-OS receiver argv, the agentic-6
statusline adapter (per-host exact probes; render-only credential-free shim),
the triple-consent egress-ack-proof doctor executor, and the
single-merge-owner integration — feature PRs #631 f9e7d07 / #632 cd27d2d /
#633 84b2c86 / #634 ac5803a / #635 73127e3 atop the pre-macro
activation-semantics fix #629 3615dcc; release PR #630 (`chore: release
main`, squash `82cf981`) cut tag `plugin-runtime-v0.86.1` with marketplace
sync commit `0eb8807`, and the 0.86.1-native proof
doctor-20260725T020157Z-61e7d6 (2026-07-25Z; `overall` `pass`, three execute
...
`current` after the same-day Claude `2.1.217`→`2.1.218` patch-drift compat
cycle — content-backed ingest in compat-20260723T124245Z-8aabdc, drift=none
re-check compat-20260723T124641Z-738a00; the honest pre-proof ledger note:
one stale `running` engineer peer-run handle reconciled to `orphaned` plus a
138-handle TTL prune via engineer peer-runner sweep --apply) was taken
against that released binary, not a patched working tree — and the same loop
