...
0.97.2. The newest settings artifact, `settings-20260910T015634Z-9d4337`, is
the 2026-09-10Z hook-review attestation recorded under 0.97.1 after the owner
completed the Codex `/hooks` review that a Codex CLI move off `0.147.0` had
staled (first read `stale` at `0.148.0` in the 2026-08-20 proof,
`currency_reason` `codex_cli_version_changed`); it binds designer@0.3.7 /
engineer@0.21.8 / founder@0.4.7 / orchestrator@0.13.5 against Codex `0.154.0`,
the 0.97.2 proof reports it current with `manual-followups=0`, and its
persisted artifact carries dry-run plugin-management and cleanup plans with
zero planned actions but no surface-availability or source/cache version
readings. It supersedes `settings-20260816T233407Z-4f23d9`, recorded on
...
rather than a live gap. The current installed `plugin-runtime` `0.97.3` state
is claude=installed/0.97.3, codex=installed/0.97.3, per the 2026-09-18Z
`doctor-20260918T040805Z-cd9948` read; the superseded 0.97.2 state was read by
...
installed `plugin-runtime` `0.97.3`: permission proof, deep peer smoke, and
workflow continuation proof executed and passed in both directions, recorded
on 2026-09-18Z as `doctor-20260918T040805Z-cd9948` — the post-release record
of the **three readers that answered for a state they never measured** loop.
The prior record was the **cutover invocation-policy** loop, recorded on
...
The CURRENT loop is the **three readers that answered for a state they never
...
accumulated. This loop's fresh snapshot `compat-20260918T040728Z-6f1daa`
...
⚠ **C11 is evidenced by this run; C9 and C13 are not observable in it and are
...
no other branch. C13 carries a stronger and more direct limitation: this run
...
⚠ **The install was already in place before this recovery began, and the
...
runtime version. No session in this loop ran a Codex install or upgrade
command, and no settings plugin-management execution artifact backs it. The
owner re-reviewed Codex `/hooks` and this loop recorded
...
The preceding loop was the **cutover invocation-policy** slice, and its
...
The latest `plugin-runtime` `0.97.3` release/installed-state proof loop is the
**three readers that answered for a state they never measured** slice, whose
0.97.3-native proof `doctor-20260918T040805Z-cd9948` (2026-09-18Z) reads
...
baseline against an observed `2.1.276` / `0.155.0`. The one blocked criterion
is `runtime_handoff_artifacts`, on `compat=release_notes_required`, and the
drop from `ready` `100%` is a reveal rather than a regression: the superseded
...
fifteen-day-old artifact, and this loop's fresh snapshot
`compat-20260918T040728Z-6f1daa` merely observed the pair that had drifted
since. Refreshing the baseline was subtask BL of this loop's macro and the
...
re-reviewed `/hooks` and this loop recorded
`settings-20260918T040649Z-9365d7` binding `0.155.0` against the unchanged
designer 0.3.7 / engineer 0.21.8 / founder 0.4.7 / orchestrator 0.13.5, with
runtime's own read agreeing at 12/12 enabled, 0 untrusted, 0 missing. The
