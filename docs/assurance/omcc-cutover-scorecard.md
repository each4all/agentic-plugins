...
Current local dry-run evidence: a fresh 2026-07-31Z `runtime:settings`
dry-run (`runtime-settings-1.23`) reports all agentic-plugins surfaces
available with source/cache versions matching the current repo manifest
(runtime source `0.87.0` = installed `0.87.0` on both hosts), and zero
plugin-management config writes planned against the installed `plugin-runtime` `0.87.0` state
(`claude plugin list` reports the runtime plugin enabled at 0.87.0 with its
Claude cache at 0.87.0, and `codex plugin list --json` reports it installed
and enabled at 0.87.0 — claude=installed/0.87.0, codex=installed/0.87.0 —
per the 2026-07-31Z `doctor-20260731T051408Z-210043` read; this loop
refreshed both hosts host-native, `claude plugin update` on Claude and
`codex plugin marketplace upgrade` + `codex plugin add` on Codex, with the
installed state doctor-observed, so this upgrade too left no
`runtime:settings --execute-plugin-management` executor artifact of its own,
the newest being the 2026-07-14 `settings-20260714T021101Z-94f0f6`). Current runtime execution evidence is native to the installed `plugin-runtime` `0.87.0`:
permission proof, deep peer smoke, and workflow continuation proof executed
and passed in both directions, recorded on 2026-07-31Z as
`doctor-20260731T051408Z-210043` — the **bootstrap effective-selection +
model/effort posture** loop, two runtime slices released together (feature
PR #655 squash `3b75915` made a declined plugin actually narrow the run's
effective selection, which the contract already specified but the
implementation derived only to validate and then discarded, leaving a
declined plugin simultaneously excluded and required and every dependent
proof permanently stale; feature PR #657 squash `167fd4a` let a machine
declare `model_effort_fallback = "host-native"` as a posture, closing a
Stage-4 step that asked only whether a coordinate key was set and so left
every machine deliberately running host defaults `pending` forever; neither
feature squash subject carries a PR number, both having been passed
explicitly on the CLI; release PR #656 `chore: release main` squash
`7fc7e63`, tag `plugin-runtime-v0.87.0`, marketplace sync `6d53cab`; Claude Code `2.1.220` /
`codex-cli 0.145.0`; the doctor `entry_brief` and `session_capture`
readiness sections both read `ready`). `overall` reads `pass`; experience
parity reads **`ready` `100%` 8/8** with zero manual follow-ups and zero
not-verified rows (runtime ships no hooks, so the four-plugin attestation
`settings-20260722T021211Z-364c8f` stays current); **`host_parity_baseline`
reads `current`** — neither host drifted this loop, so the 2026-07-25 `2.1.220` / `0.145.0`
baseline stands unchanged and the drift=none re-check
`compat-20260725T020139Z-f0cc72` still reads both hosts `matches`; and the
pre-proof workflow ledgers read clean, with no stale peer-run handle to
reconcile this time. Two honest notes belong on this record. The same
install read parity `84%` before these executors ran, both not-verified rows
naming the unexecuted proofs rather than any defect — the executors are what
the score measures, which is why closing this row takes a proof re-record
and not a token edit. And the posture this release shipped was exercised on
the spot: the machine recorded `model_effort_fallback = "host-native"` in
user-global config through `runtime:settings --apply --target user`
immediately after the install, so model/effort now resolves `<host-default>
(host-native default)` in both directions as a *declared* posture rather
than an unexplained absence, while the ADR-0048 §3 dogfood run
`bootstrap-20260723T124729Z-b5191c` re-judges its Stage-4 step on its next
verb and its own run document stays unrewritten until a resume runs. This
supersedes the 0.86.3-native record `doctor-20260728T013239Z-2a6995`
...
The latest `plugin-runtime` `0.87.0` release/install proof loop is the **bootstrap
effective-selection + model/effort posture** slice, whose 0.87.0-native
proof `doctor-20260731T051408Z-210043` (2026-07-31Z; `overall` `pass`, three
execute proofs passed in both directions, experience parity `ready` `100%`
8/8 with zero manual follow-ups, `host_parity_baseline` `current`) was taken
against the released binary rather than a patched working tree. Two runtime
slices shipped in it: feature PR #655 squash `3b75915` made a declined
plugin narrow the run's effective selection, so a decline stops leaving the
plugin excluded and required at once with every dependent proof permanently
stale; and feature PR #657 squash `167fd4a` let a machine declare
`model_effort_fallback = "host-native"` as a posture, closing a Stage-4 step
that judged only whether a coordinate key was set. Release PR #656 (`chore:
release main`, squash `7fc7e63`) cut tag `plugin-runtime-v0.87.0` with marketplace sync commit
`6d53cab`. In turn the 0.86.3 loop was the **#645 Codex /hooks attestation
