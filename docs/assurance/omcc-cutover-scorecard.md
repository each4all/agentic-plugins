...
Current local dry-run evidence: a fresh 2026-07-20Z `runtime:settings` dry-run
reports all agentic-plugins surfaces available with source/cache versions
matching the current repo manifest (runtime source `0.83.1` = installed
`0.83.1` on both hosts), and zero plugin-management config writes planned
against the installed `plugin-runtime` `0.83.1` state (both the Claude plugin
cache and `codex plugin list --json` report the runtime plugin installed and
enabled at 0.83.1 — claude=installed/0.83.1, codex=installed/0.83.1 — with
attention 0.7.1 alongside engineer 0.21.0 and orchestrator 0.13.0, per the
2026-07-20Z `doctor-20260720T175310Z-a0fd88` read; the Stage-0-form installs
again ran host-native — Claude `claude plugin update attention@agentic-plugins`
+ `claude plugin update runtime@agentic-plugins`, Codex `codex plugin
marketplace upgrade` propagating the refreshed marketplace snapshot — so this
upgrade too left no `runtime:settings --execute-plugin-management` executor
artifact of its own, the newest being the 2026-07-14
`settings-20260714T021101Z-94f0f6`). Current runtime
execution evidence is native to the installed
`plugin-runtime` `0.83.1`: permission proof, deep peer smoke, and workflow
...
hardening** loop (feature PR #606, a rebase-merged three-commit package split
per ADR-0016 — `eb480f3` the attention Stop-seam SIGKILL kill bound +
build-metadata-safe semver copy, `ba1c201` the runtime shared-`semverCompare`
prerelease tie-break + build-metadata strip, `fd381b4` the release-exempt
hardening test suite; release PR #607 squash `9a69944`, tags
`plugin-attention-v0.7.1` + `plugin-runtime-v0.83.1`, marketplace sync
`f54d70c`; Claude Code `2.1.215` / `codex-cli 0.144.6`; the doctor
`entry_brief` readiness section now reads `ready` — the operator's ADR-0045
user-scope `startup` opt-in with `empty=silent`, the `floors.entry_brief=0.83.0`
pin satisfied by `0.83.1`, the entry executor present — alongside
`session_capture` `off`). `overall` reads `pass` with zero warnings;
...
display-only); **experience parity reads `ready` `100%` (8/8, zero manual
follow-ups) with no re-attestation required** — attention `0.7.1` changed only
the Claude hook surface and keeps zero Codex hook surface, runtime ships no
hooks, and the four attested plugins (designer@0.3.0 / engineer@0.21.0 /
founder@0.4.0 / orchestrator@0.13.0) plus `codex-cli 0.144.6` are unchanged,
so the CLI-version-bound four-plugin attestation
`settings-20260720T151554Z-3b543f` evaluates **current** under the contract
§11.2 #24 currency binding — the no-invalidation half of the discipline
observed live. **`host_parity_baseline` reads `current`** — no host CLI drift
since the same-day 2026-07-20 dual-host refresh. This supersedes the same-day
0.83.0-native current-evidence record recorded on 2026-07-20Z as
doctor-20260720T052332Z-a0d677 — the **ADR-0045 entry-time proposal surfaces**
loop (feature PRs S6 #597 `932c135`, S7a #598 `5c6dae8`, S7b #599 `45624bf`,
S8 #600 `de20853`; release PR #596 squash `e249ac7`, tag plugin-runtime-v0.83.0
— backticks deliberately omitted for the freshness gate — alongside
`plugin-attention-v0.6.0`, the ADR-0044 S5 Stop→publish-session sensor +
publisher floor 0.82.0 pin, #595 `3b8ed8d`, marketplace sync `360a71f`; Claude
Code `2.1.215` / `codex-cli 0.144.1`; that record read `overall` `pass`, three
execute proofs passed both directions, hook state `12/12` `enabled_trusted`,
the then-new `entry_brief` section `off` — the shipped default before any
operator opt-in — and parity deliberately `partial` `91%` with the
`codex-hook-review` follow-up: the pre-S8a4 attestation
settings-20260713T234950Z-f08600 was refused as `stale` under the §11.2 #24
fail-closed currency binding, `bound_versions.codex=null` against `codex-cli
0.144.1`, rather than silently trusted — the fail-closed half of the
discipline, its invalidation half unit-pinned in S8a2 and demonstrated by the
recorded designer/founder upgrade precedents — until the same-day fresh
attestation `settings-20260720T151554Z-3b543f` and post-attestation record
`doctor-20260720T151637Z-e2e061` restored `ready` `100%`; its
`host_parity_baseline` read honestly `stale` — 2026-07-14 baseline Claude Code
`2.1.208` vs observed `2.1.215` — until the same-day dual-host refresh closed
it).
...
The latest `plugin-runtime` `0.83.1` release/install proof loop is the
**S9 peer-follow-up hardening** slice — feature PR #606, a rebase-merged
three-commit package split per ADR-0016: `eb480f3` (the attention Stop-seam
SIGKILL kill bound + build-metadata-safe semver copy), `ba1c201` (the runtime
shared-`semverCompare` prerelease tie-break + build-metadata strip), and
`fd381b4` (the release-exempt hardening test suite); release PR #607 squash
`9a69944` cut tags `plugin-attention-v0.7.1` and `plugin-runtime-v0.83.1` with
marketplace sync commit `f54d70c`, and the 0.83.1-native proof
doctor-20260720T175310Z-a0fd88 (2026-07-20Z; `overall` `pass` with zero
...
display-only; the doctor `entry_brief` section reads `ready` — the operator's
ADR-0045 user-scope `startup` opt-in, the `floors.entry_brief=0.83.0` pin
satisfied by `0.83.1` — alongside `session_capture` `off`; experience parity
`ready` `100%` 8/8 with the four-plugin attestation
`settings-20260720T151554Z-3b543f` still current, since attention 0.7.1
touched only the Claude hook surface, runtime ships no hooks, and the attested
set plus `codex-cli 0.144.6` are unchanged — no fresh `/hooks` review
required) was taken against that released binary, not a patched working tree.
It supersedes the 0.83.0 **ADR-0045 entry-time proposal surfaces** loop —
