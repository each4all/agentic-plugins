...
plugin-management config writes planned after the `plugin-runtime` `0.63.0`
release and installed-state refresh. Current runtime execution evidence is
carried forward by the installed `plugin-runtime` `0.63.0` loop: permission
...
recorded under the 0.61.0 install; the 0.63.0 bump is runtime-only — it adds
the ADR-0031 session-level handoff seam (`context.mjs` `normalizeProjection` /
`buildHandoffGuidance` consuming a bounded `--workflow-projection-file`) — and
does not touch the peer/dispatch path). The Codex `/hooks` review/trust
...
The latest `plugin-runtime` `0.63.0` release/install proof loop is carried by
feature PR #378 (the ADR-0031 runtime session-handoff seam) and release PR #379
(which co-released `engineer@0.15.0` and `orchestrator@0.9.0`, ADR-0031
session-handoff wiring with no packaged-hook change),
release tag `plugin-runtime-v0.63.0`, marketplace sync commit `e9dd726`, and
