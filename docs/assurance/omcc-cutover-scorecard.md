...
parity is 91%. The `plugin-runtime` `0.55.0` release/install proof loop is
carried by PR #321 and release PR #322, release tag
`plugin-runtime-v0.55.0`, marketplace sync commit `492d966`, settings artifact
`settings-20260517T034535Z-fe4a5b`, doctor artifact
`doctor-20260517T034554Z-ef2584`, and cutover record
`cutover-20260517T034641Z-d68147`.
...
| R4 | Claude Code and Codex user experience must be equivalent where possible; non-portable host-specific features must not become hard dependencies. | Architecture documents host-specific adapter boundaries; runtime baseline documents command/hook/subagent differences; Codex macro/meta skill mirrors exist. `runtime:settings` reports source/cache version parity and zero plugin-management recommendations on 2026-05-17 KST after the `0.55.0` installed-state refresh; `runtime:doctor` reports 91% experience parity with installed `plugin-runtime` `0.55.0`. `runtime:cutover` surfaces the remaining active-session Codex `/hooks` review/trust follow-up as `host=codex; commands=/hooks`, prints unresolved R4 requirement/gate detail, prints the latest footer reason, and runtime deliberately does not infer that state non-interactively. | partial | Clear the Codex active-session hook review/trust follow-up and rerun doctor until the observed parity score reaches 100% without hiding host-specific syntax differences. |
