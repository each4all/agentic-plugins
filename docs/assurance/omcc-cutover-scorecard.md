...
`plugin-runtime` `0.92.0` state is claude=installed/0.92.0,
codex=installed/0.92.0, per the 2026-08-24Z
`doctor-20260824T112523Z-679846` read; the superseded 0.91.2 state was read by
...
installed `plugin-runtime` `0.92.0`: permission proof, deep peer smoke, and
workflow continuation proof executed and passed in both directions, recorded
on 2026-08-24Z as
`doctor-20260824T112523Z-679846` — the post-release record of the
**receiver-delegation** loop, cut early by owner decision so the receiver
change would be observed rather than believed. The prior record was the **first
assurance grant** loop (ADR-0054 rollout R2), which shipped as a single
release: the grant review brief `f9af711` (PR #713), its adversarial
verification `1205258` (PR #714) which re-ran the brief's controls and
corrected four of its negative claims, the owner ratification `da7b8d9`
(PR #715) retargeted after the host moved mid-review, and the grant itself
together with the R1 assertions it replaces `592fa1a` (PR #716); release PR
[#717](https://github.com/each4all/agentic-plugins/pull/717) squash `95e5b04`,
tag plugin-runtime-v0.91.2, marketplace sync `b92cc1d`, stage-doc sync
`da60fd3`. The CURRENT loop is the **receiver-delegation** slice: the
context-state provenance fix `c87dc81` (PR #730), the packaged receiver API
`e6e53fb` (PR #731) and the installed-receiver classifier `c94c5dd`
(PR #732); release PR
...
`d577677`, with `host_parity` `pass` under Claude Code `2.1.241` / Codex
...
is not the same claim as `pass`. No install WAS performed by this recovery, on both
hosts, and the evidence is the byte comparison rather than the version string —
both caches hold an identical 127-file 0.92.0 package (whole-tree digest
`76a7caf942ceee09a4a877cb2da6173d4ba50ff6768946bf324537b28fe8b8da`), and the
packaged baseline hashes
`86fdf17e4418884f01b67785eeaaacc74923a8a01efa238f514bf270da600843` identically
across both caches, the `plugin-runtime-v0.92.0` tag and the repository at
`d577677`, which is also the value doctor independently records as
`host_parity_baseline.provenance.content_sha256`. The Claude install needed
`claude plugin marketplace update` first: a `directory`-source marketplace
serves a cached catalog, so `claude plugin update` reported nothing to do while
`claude plugin list` still showed the pre-release versions.
...
The latest `plugin-runtime` `0.92.0` release/install proof loop is the
**receiver-delegation** slice, whose 0.92.0-native proof
`doctor-20260824T112523Z-679846` (2026-08-24Z) reads `partial` `81%` 5/9
...
`ahead`. It is the first recorded proof taken against a shipped, non-empty
