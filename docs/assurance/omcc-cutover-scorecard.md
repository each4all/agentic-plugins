...
against the installed `plugin-runtime` `0.89.0` state (`claude plugin list`
reports the runtime plugin enabled at 0.89.0 with its Claude cache at 0.89.0,
and `codex plugin list` reports it installed and enabled at 0.89.0 —
claude=installed/0.89.0, codex=installed/0.89.0 — per the 2026-08-08Z
`doctor-20260808T070055Z-fa01c6` read; the superseded 0.88.1 state was read by
doctor-20260803T091403Z-7f2850, the post-attestation record of that loop);
this loop refreshed both hosts host-
native, `claude plugin update` on Claude and `codex plugin marketplace
...
native to the installed `plugin-runtime` `0.89.0`: permission proof, deep peer
smoke, and workflow continuation proof executed and passed in both directions,
recorded on 2026-08-08Z as `doctor-20260808T070055Z-fa01c6` — the post-release
record of the **ADR-0013 trigger re-evaluation** loop, a two-package release
(release PR #672 squash `0d27a80`; tags `plugin-runtime-v0.89.0`,
`plugin-engineer-v0.21.2`; marketplace sync `2f9b65d`, stage-doc sync
`0f2c483`), with `host_parity` `pass` and `host_parity_baseline` `current`
under Claude Code `2.1.226` / Codex `0.147.0`. Observed experience parity is
**`partial` `95%`** (7 satisfied, 1 partial, zero blocked). It is recorded as
`partial` rather than rounded up to the preceding `100%`: `runtime_handoff_artifacts`
cannot reach `satisfied` this cycle because `runtime:compat` resolves the host
baseline from the installed plugin root while `runtime:doctor` resolves it from
the repo, so an in-repo baseline refresh cannot clear compat's drift until the
next release ships the refreshed file. Two same-day records show the path to it:
...
The latest `plugin-runtime` `0.89.0` release/install proof loop is the
**ADR-0013 trigger re-evaluation** slice, whose 0.89.0-native proof
`doctor-20260808T070055Z-fa01c6` (2026-08-08Z) reads `partial` `95%` with zero
blocked criteria, `host_parity` `pass` and `host_parity_baseline` `current` — the
one `partial` being the compat baseline-source asymmetry described above, which no
in-repo edit can clear before the next release. It supersedes the
