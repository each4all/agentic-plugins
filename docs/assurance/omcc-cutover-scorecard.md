...
`plugin-runtime` `0.91.2` state is claude=installed/0.91.2,
codex=installed/0.91.2, per the 2026-08-20Z
`doctor-20260820T041120Z-c4c83e` read; the superseded 0.91.1 state was read by
...
installed `plugin-runtime` `0.91.2`: permission proof, deep peer smoke, and
workflow continuation proof executed and passed in both directions, recorded
on 2026-08-20Z as
`doctor-20260820T041120Z-c4c83e` — the post-release record of the **first
...
`da60fd3`, with `host_parity` `pass` under Claude Code `2.1.237` / Codex
`0.148.0`. `overall` reads **`warning`** — `hard_failures` is empty, and the
one warning is "latest compatibility check needs follow-up"; no hard failures
is not the same claim as `pass`. No install was performed by this recovery:
both hosts already carried 0.91.2, and the evidence is the byte comparison
rather than the version string — both caches hold an identical
124-file package (whole-tree digest
`4365588804c00b0d4d872894b81fc9b1d9c78a93a9387698d70e4b1f46a5ed1a`), and the
packaged baseline hashes
`86fdf17e4418884f01b67785eeaaacc74923a8a01efa238f514bf270da600843` identically
across both caches, the `plugin-runtime-v0.91.2` tag and the repository at
`da60fd3`, which is also the value doctor independently records as
...
The latest `plugin-runtime` `0.91.2` release/install proof loop is the
**first assurance grant** slice, whose 0.91.2-native proof
`doctor-20260820T041120Z-c4c83e` (2026-08-20Z) reads `partial` `81%` 5/9
