...
`f24727b`), with `host_parity` `pass` and `overall.hard_failures` empty under
Claude Code `2.1.234` / Codex `0.147.0`. No install was performed by this
...
**This is the first release in which the assurance verdict is visible to an
operator, and it returns a refusal.** `host_parity_baseline` reads `stale` with
...
`0.147.0`. R1 ships with an empty grant list by owner decision (2026-08-17), so
every host reads `unassured` and readiness is gated shut. That is the designed
outcome, not a defect: it is the point at which
[ADR-0053](../adr/0053-baseline-has-no-assurance-verdict.md) §Decision 6's
...
The one blocked criterion, `runtime_handoff_artifacts`, is this loop's own
measurement rather than an inherited fault — the mandated host re-measurement
took a fresh snapshot (`compat-20260818T115200Z-a2cd91`), Claude had moved
`2.1.233`→`2.1.234`, and the run legitimately reports `release_notes_required`.
Closing it means ingesting content-backed release notes and then refreshing the
packaged baseline, which is a protected runtime asset: editing it here would
start a new release obligation and manufacture another release-and-recovery lap,
so it is forward-patch work by construction. This is the third consecutive loop
to land on a baseline the host had already passed, which is the cost datum
[ADR-0052](../adr/0052-release-obligation-enforcement.md) measured (3 of 12
loops land current) rather than a new surprise.
...
(106/130), `host_parity` `pass`, `overall.hard_failures` empty, and
`host_parity_baseline` **`stale`** with `baseline-direction` `ahead`. It is the
first proof in which the assurance layer is separately visible, and it reads
**`unassured`**: no grant names the host pair claude `2.1.234` / codex
`0.147.0`, because R1 ships with an empty grant list by owner decision. The
refusal is the designed outcome — see the release/install narrative above for
why the two blocked-or-unverified criteria are a new measurement and a
self-taken snapshot rather than inherited faults. Its predecessor's two
