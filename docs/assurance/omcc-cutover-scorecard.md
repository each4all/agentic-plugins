...
rather than a live gap. The current installed `plugin-runtime` `0.97.2` state
is claude=installed/0.97.2, codex=installed/0.97.2, per the 2026-09-12Z
`doctor-20260912T224114Z-477c02` read; the superseded 0.97.1 state was read by
...
installed `plugin-runtime` `0.97.2`: permission proof, deep peer smoke, and
workflow continuation proof executed and passed in both directions, recorded
on 2026-09-12Z as `doctor-20260912T224114Z-477c02` — the post-release record
of the **cutover invocation-policy** loop. The prior record was the
...
The CURRENT loop is the **cutover invocation-policy** slice, and its
...
The latest `plugin-runtime` `0.97.2` release/installed-state proof loop is the
**cutover invocation-policy** slice, whose 0.97.2-native proof
`doctor-20260912T224114Z-477c02` (2026-09-12Z) reads `ready` `100%` 8/8
(115/115) with zero manual follow-ups, `host_parity` `pass`, `overall` `pass`
(zero hard failures, zero warnings), and `host_parity_baseline` **`stale`**
with `direction` `ahead` — the 2026-08-28 `2.1.250` / `0.150.1` baseline
against an observed `2.1.268` / `0.154.0`. None of the parity, `overall` and
baseline readings is the release's effect: the later 0.97.1 proof
...
`0.154.0`, and 0.97.2 touched no Codex hook-bearing plugin — runtime ships no
hooks — so that attestation stays current. The superseded 0.97.1 proof
