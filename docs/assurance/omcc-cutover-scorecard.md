...
plus `runtime:settings --attest-codex-hook-review` restores `ready` — and that
restoration landed the same day: the operator's `/hooks` confirmation produced
the fresh four-plugin attestation `settings-20260722T021211Z-364c8f`
(`bound_versions.codex=0.145.0`), and the post-attestation record
`doctor-20260722T021258Z-4c7514` restores observed parity to **`ready` `100%`
8/8** (zero manual follow-ups) with all three execute proofs re-passed in both
directions and `host_parity_baseline` still `current`. This
...
restores `ready` — which landed the same day: attestation
`settings-20260722T021211Z-364c8f` (`bound_versions.codex=0.145.0`) and the
post-attestation record `doctor-20260722T021258Z-4c7514` reading `ready`
`100%` 8/8, all three execute proofs re-passed both directions) was taken
against that
