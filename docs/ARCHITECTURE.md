...
- **Stage 3+** (in progress) — `plugins/orchestrator` L2 capability is shipped for macro planning, same-host engineer dispatch, macro completion, and peer-runner-supervised Plan-verify. `plugins/runtime` is the ADR-0024 L1 runtime/operator primitive; as of `plugin-runtime` v0.5.0 it ships read-only `doctor`, first `settings`, artifact-only consensus, runtime-owned context handoff artifacts with explicit budget checks and latest-handoff stale metadata, and an advisory completion footer helper. Deep peer execution, automatic plugin install/update apply mode, automatic peer execution, automatic host-session context mutation/compaction, and richer footer integration remain follow-up boundaries. `plugins/designer` remains a possible future design-domain L3 plugin. Same 4-layer composition.
