...
3. **Cross-host execution via the existing companion bridge.**
   ...
   as such, not a pretend-native capability. On **both** host paths,
   generation always runs through Codex's *integrated* `image_generation`
   tool (riding Codex's own auth and wiring); agentic-plugins **never**
   calls the OpenAI image API directly (see Alternative 6).
...
7. **Generation parameters — prompt-mediated via Codex's integrated
   tool.** OpenAI exposes image generation two ways: the
   ...
   **`gpt-image-2`** model supports (per OpenAI's image-generation guide;
   ...
   and `image:compose` **renders them into the prompt** for Codex's
   integrated tool (best-effort — the 2026-06-24 spike confirms a
   prompt-stated size was honored; exact quality tiers, masked edits, and
   transparency are not guaranteed). This prompt-mediated limitation is
   **accepted by design**: the project uses Codex's integrated gpt-image,
   and a direct OpenAI API path is explicitly out of scope (Alternative 6).
...
- Through Codex, generation parameters (size / quality / format / variant
  ...
  Alternative 6). Some controls are model-limited regardless — e.g.
  `gpt-image-2` has no transparent-background support.
...
6. **Direct OpenAI Images/Responses API for structured parameters.**
   ...
   the Codex bridge does not surface (Decision 7). **Rejected outright and
   out of scope — not an opt-in, not a fallback.** The project's intent is
   to use Codex's *integrated* gpt-image capability (riding Codex's own
   auth and tool wiring), never a separate OpenAI API call: a direct path
   would need an `OPENAI_API_KEY` plus org verification, reintroduce a
   direct external-service dependency outside the host-native/zero-config
   posture, and duplicate auth Codex already holds. The prompt-mediated
   parameter limitation (Decision 7) is accepted as the cost of the
   integrated approach.
