// plugins/designer/adapters/{claude,codex}/hooks/* unit tests (ADR-0042 PR2).
//
// Covers:
//   - session-start.mjs (both hosts): [designer-active-metadata] marker
//     pair + JSON metadata + canonical '/designer:<verb>' command form
...
//
// Run via `node --test tests/designer/test-hooks.mjs`.
...
async function withTmpRepo(name, fn) {
  const dir = await mkdtemp(join(tmpdir(), `designer-hooks-${name}-`));
...
describe('designer session-start.mjs', () => {
  it('emits the designer-active-metadata marker pair with the /designer:<verb> command form', async () => {
    await withTmpRepo('ss-claude', async (repoRoot) => {
      ...
      strictEqual(code, 0);
      match(stdout, /\[designer-active-metadata\]/);
      match(stdout, /\[\/designer-active-metadata\]/);
      ok(!stdout.includes('[engineer-active-metadata]'),
        'designer hooks must not emit the engineer marker');
      const m = stdout.match(/\[designer-active-metadata\]\s*([\s\S]*?)\s*\[\/designer-active-metadata\]/);
      ok(m, `marker pair must wrap the metadata JSON: ${stdout}`);
      const json = JSON.parse(m[1]);
      match(json.workflow_id, /^investigate-/);
      strictEqual(json.canonical_command, '/designer:investigate');
      ok(typeof json.workflow_path === 'string'
        && json.workflow_path.includes('/.agentic-plugins/state/designer/workflows/'),
        'metadata must point at the canonical designer state home');
    });
  });

  it('Codex adapter emits the same designer marker pair', async () => {
  ...
  it('gracefully no-ops on a malformed workflow file (host lifecycle must not crash)', async () => {
    await withTmpRepo('ss-malformed', async (repoRoot) => {
      const dir = join(repoRoot, '.agentic-plugins/state/designer/workflows');
...
describe('designer pre-compact.mjs', () => {
...
describe('designer hook manifests — drift defense', () => {
  it('hooks/hooks.json wires SessionStart/PreCompact/Stop to the claude adapter scripts', async () => {
    ...
    ok(!commands.includes('engineer'), 'no engineer paths may leak into designer hooks.json');
  });
