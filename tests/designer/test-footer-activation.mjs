// ADR-0043 S4 — designer completion-footer activation tests.
//
// Proves the designer terminal path CODE-SYNTHESIZES the runtime footer on top
...
//     mapped from designer's OWN manually-published terminal semantics
...
// `node --test tests/designer/test-footer-activation.mjs`.
...
function createWorkflow(root, { branch = 'feat/x', baselineHead = BASELINE_HEAD } = {}) {
  return execFileSync(
    ...
    [
      ...
      '--profile', 'general', '--original-request', 'footer activation test',
...
describe('designer completion-footer activation (ADR-0043 S4)', () => {
  it('CLI set-terminal renders the footer on STDERR, never stdout (machine channel intact)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dfooter-stderr-'));
    initRepo(root);
    const wf = createWorkflow(root);
    const res = cliSetTerminal(root, wf, 'Critique the composed design artifact');
  ...
  it('promotes elements 2/3/4/7 to CONCRETE (completion state + recommended next work + continue-vs-fresh)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dfooter-elements-'));
    initRepo(root);
    const wf = createWorkflow(root);
    const res = cliSetTerminal(root, wf, 'Critique the composed design artifact');
    ...
    ok(
      res.stderr.includes('recommended next work: Critique the composed design artifact'),
      `recommended next work must be concrete; got:\n${res.stderr}`,
    );
    // completion state is one of designer's mapped values (never the generic
    // default) — publish-needed is designer's manually-published mapping.
    ...
    match(res.stderr, /\/(designer|runtime):/);
  });
  ...
  it('declares no context state — an unmeasured default renders as unmeasured, not as a caller assertion', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dfooter-unmeasured-'));
    initRepo(root);
    const wf = createWorkflow(root);
  ...
  it('is idempotent: the Stop-hook backstop does NOT re-render what the primary rendered', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dfooter-idem-'));
    initRepo(root);
    const wf = createWorkflow(root);
    const projectionFile = join(root, '.agentic-plugins/state/designer/last-session-handoff.json');
  ...
  it('fail-closed on a MISSING/too-old runtime: no footer, still emitted, no throw', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dfooter-noruntime-'));
    initRepo(root);
    const wf = createWorkflow(root);
  ...
  it('fail-closed on a broken projection: no footer, no file, no throw', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dfooter-brokenproj-'));
    const projectionFile = join(root, 'proj.json');
  ...
  it('SessionStart reconciliation: a rendered footer suppresses the nudge; a missed footer keeps it', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dfooter-reconcile-'));
    initRepo(root);
    const wf = createWorkflow(root);
    const projectionFile = join(root, '.agentic-plugins/state/designer/last-session-handoff.json');
    ...
    const root2 = await mkdtemp(join(tmpdir(), 'dfooter-reconcile-missed-'));
    initRepo(root2);
    const wf2 = createWorkflow(root2);
    const projectionFile2 = join(root2, '.agentic-plugins/state/designer/last-session-handoff.json');
    const saved = process.env.AGENTIC_RUNTIME_ROOT;
    ...
    ok(afterMiss.line && afterMiss.line.includes('[designer-handoff-pending]'), 'a missed footer keeps the nudge');
  });

  it('fail-closed on a DEGRADED render (footer.mjs reports projection_error): no mark, nudge fires', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dfooter-degraded-'));
    initRepo(root);
    const wf = createWorkflow(root);
    const projectionFile = join(root, '.agentic-plugins/state/designer/last-session-handoff.json');
    const markerFile = `${projectionFile}.footer-rendered`;
    ...
    const stub = await writeStubRuntime(await mkdtemp(join(tmpdir(), 'dstub-rt-')), DEGRADED_FOOTER);
    const saved = process.env.AGENTIC_RUNTIME_ROOT;
    ...
    ok(pending && pending.line && pending.line.includes('[designer-handoff-pending]'),
      'a degraded render keeps the SessionStart nudge as a backstop');
  });

  // Completion-output contract §2 — designer is a manually-published lifecycle:
  // the sidecar maps completion flags from designer's OWN terminal semantics
  ...
  describe('completion-flag minimum content (manually-published mapping)', () => {
    const projection = {
      ...
      workflow_path: '.agentic-plugins/state/designer/workflows/compose-20260713T000000Z-abc123.md',
      phase: 'summary-complete',
      next_action: 'Save/commit the design spec',
    ...
    it('maps only-head_moved-unmet to publish-needed with the owner-publish next action', () => {
      ...
      ok(flags.completionNextAction.includes('Save/commit the design deliverable'), flags.completionNextAction);
    ...
    it('a known gate MIXED with an unknown one keeps both: specific action + fallback naming the unknown (Codex Plan-verify)', () => {
      ...
      ok(mixed.completionNextAction.includes('Save/commit the design deliverable'),
    ...
    it('single-lines multi-line projection values — a multi-line next_action must not kill the footer render (Codex Plan-verify)', () => {
      ...
      strictEqual(flags.recommendedNextWork, 'Save/commit the design spec then start the next deliverable');
    });

    it('E2E: an unmoved-HEAD terminal renders publish-needed and stays generic-marker-free', async () => {
      const root = await mkdtemp(join(tmpdir(), 'dfooter-publish-'));
      initRepo(root);
      // Baseline the workflow at the REAL current HEAD so head_moved is the
      // ONLY unmet gate at set-terminal time — designer's common "deliverable
      // ready, owner has not saved/committed yet" terminal.
      const head = execFileSync('git', ['rev-parse', 'HEAD'], { cwd: root, encoding: 'utf8' }).trim();
      const wf = createWorkflow(root, { baselineHead: head });
      const res = cliSetTerminal(root, wf, 'Save/commit the design spec');
      ...
      ok(
        res.stderr.includes('completion next action: Save/commit the design deliverable'),
    ...
    it('E2E: a moved-HEAD terminal renders next-work-available and names the phase', async () => {
      const root = await mkdtemp(join(tmpdir(), 'dfooter-ready-'));
      initRepo(root);
  ...
  it('a PRIMARY re-terminalization re-renders over its own rendered tombstone; a backstop does not', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dfooter-reterm-'));
    initRepo(root);
    const wf = createWorkflow(root);
    const projectionFile = join(root, '.agentic-plugins/state/designer/last-session-handoff.json');
  ...
  it("a 'claimed' (in-progress/crashed) marker does not suppress the nudge — only 'rendered' does", async () => {
    const root = await mkdtemp(join(tmpdir(), 'dfooter-claimed-'));
    initRepo(root);
    const wf = createWorkflow(root);
    const projectionFile = join(root, '.agentic-plugins/state/designer/last-session-handoff.json');
    const markerFile = `${projectionFile}.footer-rendered`;
