// plugins/designer/scripts/state.mjs unit tests (Stage 2 Deliverable E,
...
//
// Run via `node --test tests/designer/test-state.mjs`.
...
async function withTmpRepo(fn, { branch = 'test' } = {}) {
  const dir = await mkdtemp(join(tmpdir(), 'designer-state-test-'));
  try {
    gitInit(dir, branch);
    await fn(dir);
  } finally {
...
describe('state.mjs — parseWorkflowFile + validateFrontmatter (Phase 6 fix #7)', () => {
  it('round-trips a freshly created workflow', async () => {
    await withTmpRepo(async (repoRoot) => {
      ...
      // ADR-0028 §Layer-2 / PR3 M3 bumped emit to the latest SCHEMA_VERSION
      // (1.2 → 1.3); legacy 1/1.1/1.2 readers still accepted
      // (SUPPORTED_SCHEMA_VERSIONS).
      strictEqual(frontmatter.schema, SCHEMA_VERSION);
      strictEqual(frontmatter.verb, 'investigate');
      strictEqual(frontmatter.persona, 'designer');
...
describe('state.mjs — createWorkflow round-trip with special characters', () => {
  it('creates new workflow state under canonical .agentic-plugins/state by default', async () => {
    await withTmpRepo(async (repoRoot) => {
      ...
      ok(filePath.includes('/.agentic-plugins/state/designer/workflows/'), filePath);
      ...
      // designer trim — canonical-only contract: no legacyHasState field
      // is exposed at all (ADR-0042 SD7).
      ok(!('legacyHasState' in storage),
        'designer resolveWorkflowStorage must not expose a legacy home field');
    });
  });

  // designer trim — the engineer sibling's legacy-home continuity /
  // dual-home ambiguity / migration-block tests are intentionally
  // absent: designer has a canonical-only state home (ADR-0042 SD7).
  it('ignores a stray .claude/agentic-designer directory entirely (canonical-only contract)', async () => {
    await withTmpRepo(async (repoRoot) => {
      // Simulate a user hand-creating a legacy-shaped home; designer
      // must neither read nor prefer it.
      const strayDir = join(repoRoot, '.claude/agentic-designer/workflows');
      await mkdir(strayDir, { recursive: true });
      await writeWorkflowFixture(
        join(strayDir, 'investigate-20260101T000000Z-aaaaaa.md'),
        'stray/main',
      );

      strictEqual(await findActiveWorkflowByBranch(repoRoot, 'stray/main'), null,
        'a legacy-shaped home must be invisible to designer reads');
      ...
      const { filePath } = await createWorkflow({
        ...
        originalRequest: 'canonical-only write',
      });
      ok(filePath.includes('/.agentic-plugins/state/designer/workflows/'),
        `writes must land in the canonical home even when a stray legacy-shaped dir exists: ${filePath}`);
    });
  });
...
describe('state.mjs — findActiveWorkflow branch-keyed lookup (ADR-0018 §sub-2)', () => {
  ...
  it('(g) cross-branch malformed file does NOT block same-branch lookup (lightweight extractor + skip)', async () => {
    await withTmpRepo(async (repoRoot) => {
      ...
      const sibling = join(
        repoRoot,
        '.agentic-plugins/state/designer/workflows/investigate-20260101T000000Z-zzzzzz.md',
      );
      await writeFile(
        sibling,
        '---\nschema: 99\ngit_baseline:\n  branch: "other"\n---\n!!corrupt body!!\n',
...
describe('state.mjs — ADR-0017 schema 1.1 + ADR-0028 schema 1.2/1.3 constants', () => {
  ...
  it('workflow and archive dirs are canonical-only (designer trim — no LEGACY_* exports)', async () => {
    strictEqual(WORKFLOW_DIR_REL, '.agentic-plugins/state/designer/workflows');
    strictEqual(ARCHIVE_DIR_REL, '.agentic-plugins/state/designer/archive');
    const mod = await import(STATE_PATH);
    ok(!('LEGACY_STATE_DIR_REL' in mod) && !('LEGACY_WORKFLOW_DIR_REL' in mod)
      ...
      'designer state.mjs must not export legacy-home constants (ADR-0042 SD7)');
  });
});
...
describe('state.mjs — ADR-0017 frontmatter accept (read-side, schema 1 + "1.1")', () => {
  it('reads a legacy schema=1 file (hand-downgraded) with no 1.1 fields (backward compat)', async () => {
    await withTmpRepo(async (repoRoot) => {
      ...
      // ADR-0028 §Layer-2 / PR3 M3 — createWorkflow emits the latest
      // SCHEMA_VERSION ("1.2" then "1.3", etc); the legacy
      // backward-compat test still hand-downgrades to integer 1.
      const downgraded = raw.replace(/^schema: "[^"]+"\s*$/m, 'schema: 1');
...
describe('state.mjs — schema-version preservation on mutation round-trip', () => {
  it('createWorkflow emits the latest SCHEMA_VERSION (ADR-0028 PR1 → 1.2, PR3 M3 → 1.3)', async () => {
    await withTmpRepo(async (repoRoot) => {
      await createWorkflow({
        repoRoot,
        verb: 'investigate',
        host: 'claude',
        gitBaseline: MIN_BASELINE,
        originalRequest: 'ADR-0028 bumps emit',
      });
      const [filePath] = await listWorkflowFiles(repoRoot);
      const { frontmatter } = await readWorkflow(filePath);
      strictEqual(frontmatter.schema, SCHEMA_VERSION);
  ...
  it('setCheckpoint preserves the latest SCHEMA_VERSION on disk (no silent downgrade)', async () => {
    await withTmpRepo(async (repoRoot) => {
      await createWorkflow({
        repoRoot,
        verb: 'investigate',
        host: 'claude',
        gitBaseline: MIN_BASELINE,
        originalRequest: 'mutator preserves current schema',
      });
      const [filePath] = await listWorkflowFiles(repoRoot);
      await setCheckpoint({
        workflowPath: filePath,
  ...
  it('setCheckpoint preserves legacy schema="1.1" on disk (no silent promotion)', async () => {
    await withTmpRepo(async (repoRoot) => {
      await createWorkflow({
        repoRoot,
        verb: 'investigate',
      ...
      const [filePath] = await listWorkflowFiles(repoRoot);
      // ADR-0028 §Layer-2 (1.1→1.2) and PR3 M3 (1.2→1.3) both mirror the
      // ADR-0017 1→1.1 policy: mutation helpers preserve the
      // disk-recorded schema and never silently promote. Workflow files
      // created before any emit bump (still on schema "1.1") must
      // continue to read/write at 1.1 across mutation.
      const { readFile, writeFile } = await import('node:fs/promises');
      const original = await readFile(filePath, 'utf8');
      const downgraded = original.replace(/^schema: "[^"]+"\s*$/m, 'schema: "1.1"');
      strictEqual(
        downgraded.includes('\nschema: "1.1"\n'),
        true,
  ...
  it('setCheckpoint preserves legacy schema=1 on disk (no silent promotion)', async () => {
    await withTmpRepo(async (repoRoot) => {
      ...
      // era). ADR-0017 + ADR-0028 §Layer-2/PR3 — auto-promotion to the
      // latest SCHEMA_VERSION would surprise existing pipelines that
      // key on `schema === 1`.
      const { readFile, writeFile } = await import('node:fs/promises');
      const original = await readFile(filePath, 'utf8');
      const downgraded = original.replace(/^schema: "[^"]+"\s*$/m, 'schema: 1');
      strictEqual(
        downgraded.includes('\nschema: 1\n'),
        true,
...
describe('state.mjs — archiveWorkflow archiveDirectory-only mode (Codex review M2)', () => {
  ...
  it('locks the same .creation-lock as createWorkflow (Codex re-review M-1)', async () => {
    ...
    // `<repoRoot>/.agentic-plugins/state/designer/.creation-lock`. The previous
    // (incorrect) two-deep dirname produced
    // `<repoRoot>/.agentic-plugins/state/designer/.agentic-plugins/state/designer/.creation-lock`,
    ...
    await withTmpRepo(async (repoRoot) => {
      ...
      const stray = `${repoRoot}/.agentic-plugins/state/designer/.agentic-plugins`;
...
// ============================================================================
// designer trim — the ADR-0019 parent-linkage fields are NOT part of the
// designer schema (orchestrator→designer dispatch is ADR-0042 Non-Goal 2).
// This suite pins the trimmed contract: read-side unknown-key tolerance,
// no create surface, and loud CLI rejection (misdispatch defense).
// ============================================================================

describe('state.mjs — designer no-parent-linkage contract (ADR-0042 Non-Goal 2)', () => {
  it('treats an on-disk parent_workflow scalar as a forward-compat unknown, not a schema field', () => {
    const text =
      '---\n' +
      'schema: "1.1"\n' +
      ...
      'verb: "investigate"\n' +
      'profile: ""\n' +
      'original_request: "engineer-shaped file landing on a designer reader"\n' +
      ...
      '    event: "created"\n' +
      'parent_workflow: "macro-plan-20260510T060000Z-ffeedd"\n' +
      'originating_subtask: "PR1"\n' +
      '---\n' +
      '\n# designer:investigate\n';
    const { frontmatter } = parseWorkflowFile(text);
    // The keys must NOT surface as schema fields — they ride the
    // Symbol-keyed forward-compat carrier (invisible to 'in' / keys()).
    ok(!('parent_workflow' in frontmatter),
      'parent_workflow must not be a designer schema field');
    ok(!('originating_subtask' in frontmatter),
      'originating_subtask must not be a designer schema field');
  });

  it('createWorkflow never writes parent-linkage fields (legacy-shaped options are dropped)', async () => {
    await withTmpRepo(async (repoRoot) => {
      const { filePath } = await createWorkflow({
        repoRoot,
        verb: 'investigate',
        ...
        // engineer-shaped options — designer's createWorkflow signature
        // no longer destructures these; they must be silently dropped
        // at the JS layer (the CLI layer rejects loudly instead).
        parentWorkflow: 'macro-plan-20260510T060000Z-ffeedd',
        originatingSubtask: 'PR1',
      });
      const { frontmatter } = await readWorkflow(filePath);
      ok(!('parent_workflow' in frontmatter),
        'parent_workflow must never be written by designer createWorkflow');
      ok(!('originating_subtask' in frontmatter),
        'originating_subtask must never be written by designer createWorkflow');
    });
  });

  it('CLI create rejects --parent-workflow / --originating-subtask loudly (misdispatch defense)', async () => {
    await withTmpRepo(async (repoRoot) => {
      const result = spawnSync(process.execPath, [
        ...
        '--parent-workflow', 'macro-plan-20260510T060000Z-ffeedd',
        '--originating-subtask', 'PR1',
      ], { encoding: 'utf8' });
      strictEqual(result.status, 1, `expected exit 1, got ${result.status}: ${result.stderr}`);
      match(result.stderr, /does not accept --parent-workflow/);
      match(result.stderr, /ADR-0042 Non-Goal 2/);
    });
  });

  it('CLI create rejects even an empty --parent-workflow flag (shim safety)', async () => {
    await withTmpRepo(async (repoRoot) => {
      const result = spawnSync(process.execPath, [
        STATE_PATH,
        'create',
  ...
  it('CLI create rejects a non-designer --persona (canonical persona boundary)', async () => {
    await withTmpRepo(async (repoRoot) => {
      const result = spawnSync(process.execPath, [
        STATE_PATH,
        'create',
      ...
      strictEqual(result.status, 1, `expected exit 1, got ${result.status}: ${result.stderr}`);
      match(result.stderr, /only writes persona 'designer'/);
    });
  });
  ...
  it('createWorkflow without parent linkage omits both fields', async () => {
    await withTmpRepo(async (repoRoot) => {
      await createWorkflow({
        ...
        host: 'claude',
        gitBaseline: MIN_BASELINE,
        originalRequest: 'plain designer workflow',
      });
      const [filePath] = await listWorkflowFiles(repoRoot);
      const { frontmatter } = await readWorkflow(filePath);
      ok(!('parent_workflow' in frontmatter));
      ok(!('originating_subtask' in frontmatter));
      ok(!('parent_detached' in frontmatter));
    });
  });
});
...
describe('state.mjs — ADR-0020 workflow_type field (schema 1.1 additive)', () => {
  ...
  it('FRONTMATTER_KEY_ORDER places workflow_type after the ADR-0017 1.1 group (designer trim — no parent-linkage keys in between)', async () => {
    await withTmpRepo(async (repoRoot) => {
      await createWorkflow({
        repoRoot,
        verb: 'compose',
      ...
      ok(
        idxWorkflowType > idxHostHistory,
        'workflow_type must serialize after the core 1.1 group',
      );
      ok(!raw.includes('\nparent_workflow:') && !raw.includes('\noriginating_subtask:'),
        'no parent-linkage keys may serialize in designer files (ADR-0042 Non-Goal 2)');
    });
  });
});
