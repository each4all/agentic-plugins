// ADR-0043 S4 — designer activation sidecar tests.
//
// Proves the session-handoff projection FIRES from the must-run completion
// mutation (designer `set-terminal`) rather than from runbook prose, and that
...
// `node --test tests/designer/test-handoff-sidecar.mjs`.
...
import { emitTerminalHandoffSidecar } from '../../plugins/designer/scripts/session-handoff.mjs';
import { setTerminal } from '../../plugins/designer/scripts/state.mjs';
...
function createWorkflow(repoRoot, branch) {
  return execFileSync(
    ...
    [
      ...
      '--profile', 'general', '--original-request', 'handoff sidecar test',
...
describe('designer activation sidecar (ADR-0043 S4)', () => {
  it('fires and writes the projection file for the workflow at the given path', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsg-sidecar-ok-'));
  ...
  it('projects the workflow by PATH, not by current branch (cross-branch correctness)', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'dsg-sidecar-xbranch-'));
  ...
  it('is fail-closed silent when the workflow path does not resolve (no file, no throw)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsg-sidecar-none-'));
    const projectionFile = join(root, 'proj.json');
  ...
  it('a failed emit CLEARS the stale projection from a prior successful emit (ADR-0043 §2 baseline)', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'dsg-sidecar-staleclear-'));
    const workflowPath = createWorkflow(root, 'feat/x');
    const projectionFile = join(root, 'proj.json');
  ...
  it('stays non-fatal when the projection file cannot be written', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsg-sidecar-wfail-'));
    const workflowPath = createWorkflow(root, 'feat/x');
  ...
  it('direct setTerminal JS calls do NOT emit (emitHandoff is opt-in for the production CLI)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsg-sidecar-optin-'));
    const workflowPath = createWorkflow(root, 'feat/x');
    await setTerminal({
      workflowPath,
      host: 'claude',
  ...
  it('un-marking (--terminal-marker false) is not a terminal transition: no sidecar (orchestrator parity)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsg-sidecar-unmark-'));
    const workflowPath = createWorkflow(root, 'feat/x');
    const res = spawnSync(
      'node',
      [
  ...
  it('a RELATIVE --workflow-path still fires the sidecar (absolute-path resolution before home inference)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsg-sidecar-relpath-'));
    const workflowPath = createWorkflow(root, 'feat/x');
  ...
  it('CLI set-terminal fires the sidecar without polluting stdout (docs-only regression guard)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsg-sidecar-cli-'));
    ...
    const res = spawnSync(
      ...
      [
        ...
        '--workflow-path', workflowPath, '--host', 'claude',
        '--terminal-phase', 'summary-complete', '--terminal-marker', 'true',
        '--next-action', 'Critique the composed design artifact', '--event', 'updated',
      ],
      { encoding: 'utf8', cwd: root },
    );
    ...
    // The guaranteed channel: the projection file landed under the designer state root.
    const projectionFile = join(root, '.agentic-plugins/state/designer/last-session-handoff.json');
