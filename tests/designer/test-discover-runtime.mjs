// ADR-0043 §2/§4 — designer dual-consumer discoverRuntimePluginRoot tests.
//
// designer's resolver serves TWO consumers with independent floors and
...
// throwaway fixtures. Run via `node --test tests/designer/test-discover-runtime.mjs`.
...
} from '../../plugins/designer/scripts/discover-runtime.mjs';
...
const NO_HOME = () => tmp('drt-emptyhome-'); // a home with no caches
const NEUTRAL_SELF = 'file:///nowhere/scripts/discover-runtime.mjs'; // no /.claude/ or /.codex/, no sibling

describe('designer discoverRuntimePluginRoot — dual-consumer floors (ADR-0043 §2/§4)', () => {
  ...
  it('env override (valid) → returns the root for both pairs', async () => {
    const root = await mkRuntimeRoot(await tmp('drt-env-ok-'));
  ...
  it('capability independence: a notify-only runtime serves the notify ladder, never the footer ladder', async () => {
    const root = await mkRuntimeRoot(await tmp('drt-notifyonly-'), { withFooter: false });
    const home = await NO_HOME();
  ...
  it('capability independence: a footer-only runtime serves the footer ladder, never the notify ladder', async () => {
    const root = await mkRuntimeRoot(await tmp('drt-footeronly-'), { withNotify: false });
    const home = await NO_HOME();
    const env = { AGENTIC_RUNTIME_ROOT: root };
  ...
  it('independent floors: a 0.78.x runtime satisfies the notify floor but NOT the footer floor', async () => {
    ...
    const root = await mkRuntimeRoot(await tmp('drt-window-'), { version: '0.78.1' });
    const home = await NO_HOME();
    const env = { AGENTIC_RUNTIME_ROOT: root };
  ...
  it('Claude cache → picks the latest SemVer PER CAPABILITY (a newer footer-less entry is skipped)', async () => {
    const home = await tmp('drt-claude-semver-');
  ...
  it('Codex install cache → returns it when no Claude cache exists', async () => {
    const home = await tmp('drt-codex-');
  ...
  it('same-host preference: Codex selfUrl prefers the Codex cache over a Claude cache', async () => {
    const home = await tmp('drt-samehost-codex-');
    await mkClaudeCache(home, [{ version: '0.80.0' }]);
    const codexRoot = await mkCodexCache(home);
    const got = await discoverRuntimePluginRoot({
      env: {},
      home,
      selfUrl: pathToFileURL(join(home, '.codex', 'plugins', 'cache', 'agentic-plugins', 'designer', '0.18.0', 'scripts', 'discover-runtime.mjs')).href,
    });
    strictEqual(got, codexRoot, 'Codex-host self should prefer the Codex cache');
  });

  it('sibling monorepo fallback → resolves <designer>/../runtime when no env/cache', async () => {
    const mono = await tmp('drt-sibling-');
    const designerScripts = join(mono, 'plugins', 'designer', 'scripts');
    await mkdir(designerScripts, { recursive: true });
    const selfFile = join(designerScripts, 'discover-runtime.mjs');
  ...
  it('too-old runtime → null (gated), even though resolve() finds it (no stale-cache fallback)', async () => {
    const oldRoot = await mkRuntimeRoot(await tmp('drt-old-'), { version: '0.10.0' });
    const home = await NO_HOME();
  ...
  it('a prerelease of the footer floor (0.79.0-beta.1) does NOT satisfy the gate', async () => {
    const preRoot = await mkRuntimeRoot(await tmp('drt-pre-'), { version: '0.79.0-beta.1' });
    const home = await NO_HOME();
    ...
    const preAbove = await mkRuntimeRoot(await tmp('drt-pre2-'), { version: '0.80.0-rc.1' });
  ...
  it('exactly at each floor passes its own gate', async () => {
    const home = await NO_HOME();
    const atFooterFloor = await mkRuntimeRoot(await tmp('drt-floor-'), { version: MIN_RUNTIME_VERSION });
    strictEqual(
      await discoverRuntimePluginRoot({ env: { AGENTIC_RUNTIME_ROOT: atFooterFloor }, home, selfUrl: NEUTRAL_SELF }),
      atFooterFloor,
    );
    const atNotifyFloor = await mkRuntimeRoot(await tmp('drt-floor2-'), { version: NOTIFY_MIN_RUNTIME_VERSION });
    strictEqual(
