// ADR-0043 S4 hook backstop (designer-hook-backstop) tests.
//
...
// backstop rides the existing designer Stop / SessionStart hooks to narrow the
// residual gap:
...
//     missed), INDEPENDENTLY of an active workflow (the pre-S3 designer hook
...
// `node --test tests/designer/test-handoff-backstop.mjs`.
...
const SESSION_START = resolve(REPO_ROOT, 'plugins/designer/adapters/claude/hooks/session-start.mjs');
const CODEX_SESSION_START = resolve(REPO_ROOT, 'plugins/designer/adapters/codex/hooks/session-start.mjs');
const STOP = resolve(REPO_ROOT, 'plugins/designer/adapters/claude/hooks/stop.mjs');
const CODEX_STOP = resolve(REPO_ROOT, 'plugins/designer/adapters/codex/hooks/stop.mjs');
...
} = await import(resolve(REPO_ROOT, 'plugins/designer/scripts/session-handoff.mjs'));

const PROJECTION_REL = '.agentic-plugins/state/designer/last-session-handoff.json';
const PENDING_MARK = 'designer-handoff-pending';
const MIN_BASELINE = {
  branch: 'test',
...
const SAMPLE = {
  ...
  workflow_path: '.agentic-plugins/state/designer/workflows/compose-20260101T000000Z-abc123.md',
...
async function withTmpRepo(fn, { branch = 'test' } = {}) {
  const dir = await mkdtemp(join(tmpdir(), 'designer-handoff-backstop-'));
  try {
...
describe('designer hook handoff backstop — helpers (ADR-0043 S4)', () => {
  it('readPendingHandoff is fail-closed (null) with no file, and reads a valid projection', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dsg-backstop-read-'));
    try {
  ...
  it('pendingHandoffReinjectionLine builds a bounded marker line and excludes next_action', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dsg-backstop-line-'));
    try {
      ...
      strictEqual(summary.routing_recommendation, '/designer:resume');
  ...
  it('does not re-inject a fields-less projection (workflow_id fail-closed)', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dsg-backstop-malformed-'));
    try {
  ...
  it('consumePendingHandoff removes the one-shot file, PRESERVES a rendered tombstone, and removes a crashed claim', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dsg-backstop-consume-'));
    try {
      // rendered marker → tombstone survives (designer divergence: it is what
...
describe('designer SessionStart handoff backstop (ADR-0043 S4)', () => {
  it('re-injects + consumes a pending handoff even with NO active workflow (claude — the pre-S3 early-return gap)', async () => {
    await withTmpRepo(async (dir) => {
      ...
      ok(!r.stdout.includes('[designer-active-metadata]'), 'no active-metadata when no active workflow');
  ...
  it('composed lifecycle: set-terminal render → SessionStart consume → Stop does NOT re-render (Codex Plan-verify blocker)', async () => {
    await withTmpRepo(async (dir) => {
      ...
      //    workflow stays active-terminal (designer's common case).
      ...
      const wf = await createWorkflow({
        ...
        nextAction: 'Save/commit the design spec',
      });
      ...
      const setTerm = spawn('node', [
        ...
        '--next-action', 'Save/commit the design spec', '--event', 'updated',
  ...
  it('active workflow + pending handoff → BOTH the metadata line and the re-injection fire', async () => {
    await withTmpRepo(async (dir) => {
      ...
      const r = await runHook({ cwd: dir }, SESSION_START);
      strictEqual(r.code, 0);
      ok(r.stdout.includes('[designer-active-metadata]'), 'active metadata still fires');
  ...
  it('no pending handoff → no [designer-handoff-pending] line (clean no-op)', async () => {
    await withTmpRepo(async (dir) => {
      const r = await runHook({ cwd: dir }, SESSION_START);
      strictEqual(r.code, 0);
...
describe('designer Stop handoff backstop (ADR-0043 S4)', () => {
  it('fires the sidecar for a terminal workflow when the primary emit was missed', async () => {
    await withTmpRepo(async (dir) => {
      const wf = await createWorkflow({
        ...
        currentPhase: 'phase-2-presented',
        nextAction: 'Critique the composed design artifact',
      });
