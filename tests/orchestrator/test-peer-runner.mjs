...
process.env.AGENTIC_RUNTIME_ROOT = join(
  tmpdir(),
  `agentic-orchestrator-peer-runner-tests-no-runtime-${process.pid}`,
);
...
describe('peer-runner.mjs — ADR-0040 §5 peer-run terminal self-sensor', () => {
  ...
  // footer.mjs existence marker (orchestrator's resolver gates on footer.mjs),
  // a version-declaring manifest, and a notify.mjs that captures its argv +
...
`, 'utf8');
  ...
  it('emits a canonical peer-run-terminal event on runPeer completion', async () => {
    await withTmpRepo(async (repoRoot) => {
      ...
      strictEqual(event.source, 'peer-runner-orchestrator');
  ...
  it('emits from the pre-ledger missing-companion early return (no handle written)', async () => {
    await withTmpRepo(async (repoRoot) => {
      const companionsRoot = await writeUnavailableCompanions(repoRoot);
      const { runtimeRoot, capturePath } = await writeStubRuntime(repoRoot);
      const { filePath: workflowPath } = await createWorkflow({
        repoRoot,
        verb: 'plan',
        originalRequest: 'self-sensor pre-ledger test',
        gitBaseline: {
          branch: 'main',
          head: 'abc123',
          status_digest: '',
        ...
        host: 'codex',
        currentPhase: 'phase-test',
        nextAction: 'test pre-ledger emit',
      });

      const result = await runPeer({
        repoRoot,
        runId: 'sensor-preledger',
        kind: 'ensemble',
        workflowPath,
        phase: 'plan',
        ensembleType: 'plan-verify',
      ...
      strictEqual(result.handle_path, null);
      strictEqual(await exists(peerRunPaths(repoRoot, 'sensor-preledger').dir), false);

      const captured = await readCaptured(capturePath);
      strictEqual(captured.length, 1);
      const { event } = captured[0];
      ok(event.event_id.endsWith(':peer-run-terminal:sensor-preledger:failed'), event.event_id);
      strictEqual(event.source, 'peer-runner-orchestrator');
      ok(event.body.includes('peer_cli_not_found'), event.body);
      // No handle was ever written: refs names the workflow but no ledger path.
      strictEqual(event.refs.path, undefined);
      strictEqual(event.refs.run_id, 'sensor-preledger');
      ok(
        typeof event.refs.workflow_id === 'string' && event.refs.workflow_id.length > 0,
        'pre-ledger emit should still carry the originating workflow id',
      );
    });
  });
