// ADR-0031 — engineer session-handoff projection tests.
//
// Verifies that engineer computes its OWN bounded projection (fail-closed) and
// that the result satisfies the runtime seam's bounded schema (the
// cross-plugin contract). Run via `node --test tests/engineer/test-session-handoff.mjs`.
...
describe('engineer session handoff projection (ADR-0031)', () => {
  ...
  it('reports no active branch context on detached HEAD and does not auto-fresh', async () => {
    ...
    const result = await computeEngineerProjection({ repoRoot: root, branch: null });
  ...
  it('reports no active workflow when the branch has none', async () => {
    ...
    const result = await computeEngineerProjection({ repoRoot: root, branch: 'feat/x' });
  ...
  it('computes a complete bounded projection for an active workflow (not_terminal)', async () => {
    ...
    const result = await computeEngineerProjection({
      repoRoot: root, branch: 'feat/x',
      headSha: MOVED_HEAD, headSubject: 'feat: x', routing: '/engineer:resume',
    });
    strictEqual(result.status, 'ok');
    strictEqual(result.projection.workflow_kind, 'engineer');
    ...
    strictEqual(result.projection.routing_recommendation, '/engineer:resume');
  ...
  it('derives ready_to_archive vs blocked from the real terminal gate (head moved or not)', async () => {
    ...
    setTerminal(path);
    const ready = await computeEngineerProjection({
    ...
    const blocked = await computeEngineerProjection({
  ...
  it('produces a projection the runtime seam accepts unchanged (cross-plugin contract)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'eng-handoff-xcheck-'));
    createWorkflow(root, 'feat/z');
    const { projection } = await computeEngineerProjection({
    ...
    ok(normalized);
    strictEqual(normalized.workflow_kind, 'engineer');
  ...
  it('treats whitespace-only routing as absent and falls back to the default (seam-accepted)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'eng-handoff-ws-'));
  ...
  it('always returns a routing recommendation, even with no projection (ADR-0031 input (c))', async () => {
    ...
    const none = await computeEngineerProjection({ repoRoot: root, branch: 'feat/none', routing: '/orchestrator:next' });
    ...
    const detached = await computeEngineerProjection({ repoRoot: root, branch: null });
    strictEqual(detached.status, 'no_active_branch_context');
    strictEqual(detached.routing, '/engineer:resume');
  });
  ...
  it('maps a terminal workflow to blocked when the git HEAD cannot be probed (null head)', async () => {
    ...
    const result = await computeEngineerProjection({ repoRoot: root, branch: 'feat/np' });
    strictEqual(result.projection.archive_gate, 'blocked');
  });

  it('parses CLI args', () => {
    const options = parseArgs(['project', '--repo-root', '/r', '--branch', 'b', '--routing', '/engineer:resume']);
    strictEqual(options.command, 'project');
    strictEqual(options.repoRoot, '/r');
    strictEqual(options.branch, 'b');
    strictEqual(options.routing, '/engineer:resume');
  });
});
