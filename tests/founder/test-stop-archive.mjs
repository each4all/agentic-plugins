// plugins/founder/scripts/stop-archive.mjs +
...
//
// Run via `node --test tests/founder/test-stop-archive.mjs`.
...
const CODEX_STOP_PATH = resolve(
  REPO_ROOT,
  'plugins/founder/adapters/codex/hooks/stop.mjs',
);
...
describe('evaluateStopArchive — pure unit cases (ADR-0017 §sub-5 gates)', () => {
  ...
  it('all 4 hard gates pass + conventional subject → shouldArchive=true, no warnings', () => {
    const v = evaluateStopArchive({
      ...
      headSubject: 'feat(plugins/founder): something',
    });
  ...
  it('conventional subject with scope passes the soft gate', () => {
    const v = evaluateStopArchive({
      frontmatter: baseFm,
      headSha: advancedHead,
      headSubject: 'fix(plugins/founder): leak',
    });
    strictEqual(v.shouldArchive, true);
    strictEqual(v.warnings.length, 0);
  });
...
async function withTmpGitRepo(fn) {
  const dir = await mkdtemp(join(tmpdir(), 'founder-stop-archive-'));
  try {
...
function makeAdvanceCommit(repoRoot, subject = 'feat(plugins/founder): work') {
...
// ============================================================================
// founder trim guard — ADR-0036 Non-Goal 3. The engineer sibling fires the
// ADR-0019 §4 parent writeback at this point in the stop lifecycle; founder
...
describe('founder stop-archive — no parent writeback ever (ADR-0036 Non-Goal 3)', () => {
  ...
  it('archives an engineer-shaped file carrying parent keys WITHOUT attempting writeback', async () => {
    await withTmpGitRepo(async ({ repoRoot, baselineHead }) => {
      ...
      // for the founder reader) plus the terminal state.
...
// -----------------------------------------------------------------------------
// founder state.mjs CLI surface around archiving (founder trim per
// ADR-0036 Non-Goal 3):
//
...
//                      mid-flight detach, and orchestrator→founder
...
describe('founder CLI — no detach-archive subcommand (ADR-0036 Non-Goal 3)', () => {
  ...
  it('archives and emits {archived:true, to:<archive-path>} when all gates pass', async () => {
    await withTmpGitRepo(async ({ repoRoot, baselineHead }) => {
      ...
      const advancedHead = makeAdvanceCommit(
        repoRoot,
        'feat(plugins/founder): terminal commit',
      );
      ...
      const { code, stdout, stderr } = runStateCli(
        [
          ...
          '--head-subject', 'feat(plugins/founder): terminal commit',
    ...
    await withTmpGitRepo(async ({ repoRoot, baselineHead }) => {
      ...
      const { code, stdout, stderr } = runStateCli(
        [
          ...
          '--repo-root', repoRoot,
          '--head-sha', advancedHead,
          '--head-subject', 'feat(plugins/founder): work',
          '--status-digest', MIN_DIGEST,
        ],
        { cwd: repoRoot },
      );
...
describe('runStopArchiveOrphanSweep — ADR-0031 branch-deletion orphan sweep', () => {
  ...
  it('ignores files in a legacy-shaped home entirely (founder canonical-only sweep)', async () => {
    await withTmpGitRepo(async ({ repoRoot, baselineHead }) => {
      const { filePath } = await createWorkflow({
      ...
      // Relocate into a legacy-shaped home: the founder sweep walks the
      // canonical home only (ADR-0036 SD5), so this file must be invisible
      // — neither archived nor reported.
      const legacyDir = join(repoRoot, '.claude/agentic-founder/workflows');
      await mkdir(legacyDir, { recursive: true });
      await rename(filePath, join(legacyDir, basename(filePath)));
      const results = await runStopArchiveOrphanSweep({ repoRoot, host: 'claude' });
      strictEqual(results.length, 0, 'legacy-shaped homes are outside the founder sweep');
  ...
  it('archives an orphan carrying engineer-shaped parent keys as a PLAIN orphan (no parent special-casing)', async () => {
    await withTmpGitRepo(async ({ repoRoot, baselineHead }) => {
      ...
      // for the founder reader — the closed-schema serializer would
      ...
      ok(!/parent-linked|writeback|orchestrator/i.test(stderrChunks.join('')),
        'founder sweep must not special-case parent keys (ADR-0036 Non-Goal 3)');
    });
  });
