// plugins/founder/scripts/state.mjs unit tests (Stage 2 Deliverable E,
...
//
// Run via `node --test tests/founder/test-state.mjs`.
...
const STATE_PATH = resolve(REPO_ROOT, 'plugins/founder/scripts/state.mjs');
...
describe('state.mjs — parseWorkflowFile + validateFrontmatter (Phase 6 fix #7)', () => {
  it('round-trips a freshly created workflow', async () => {
    await withTmpRepo(async (repoRoot) => {
      ...
      strictEqual(frontmatter.persona, 'founder');
...
describe('state.mjs — createWorkflow round-trip with special characters', () => {
  it('creates new workflow state under canonical .agentic-plugins/state by default', async () => {
    await withTmpRepo(async (repoRoot) => {
      ...
      ok(filePath.includes('/.agentic-plugins/state/founder/workflows/'), filePath);
      ...
      // founder trim — canonical-only contract: no legacyHasState field
      // is exposed at all (ADR-0036 SD5).
      ok(!('legacyHasState' in storage),
        'founder resolveWorkflowStorage must not expose a legacy home field');
    });
  });

  // founder trim — the engineer sibling's legacy-home continuity /
  // dual-home ambiguity / migration-block tests are intentionally
  // absent: founder has a canonical-only state home (ADR-0036 SD5).
  it('ignores a stray .claude/agentic-founder directory entirely (canonical-only contract)', async () => {
    await withTmpRepo(async (repoRoot) => {
      // Simulate a user hand-creating a legacy-shaped home; founder
      // must neither read nor prefer it.
      const strayDir = join(repoRoot, '.claude/agentic-founder/workflows');
      ...
      strictEqual(await findActiveWorkflowByBranch(repoRoot, 'stray/main'), null,
        'a legacy-shaped home must be invisible to founder reads');
      ...
      ok(filePath.includes('/.agentic-plugins/state/founder/workflows/'),
...
describe('state.mjs — findActiveWorkflow branch-keyed lookup (ADR-0018 §sub-2)', () => {
  ...
  it('(g) cross-branch malformed file does NOT block same-branch lookup (lightweight extractor + skip)', async () => {
    await withTmpRepo(async (repoRoot) => {
      ...
      const sibling = join(
        repoRoot,
        '.agentic-plugins/state/founder/workflows/investigate-20260101T000000Z-zzzzzz.md',
      );
      await writeFile(
        sibling,
        '---\nschema: 99\ngit_baseline:\n  branch: "other"\n---\n!!corrupt body!!\n',
...
describe('state.mjs — ADR-0017 schema 1.1 + ADR-0028 schema 1.2/1.3 constants', () => {
  ...
  it('workflow and archive dirs are canonical-only (founder trim — no LEGACY_* exports)', async () => {
    strictEqual(WORKFLOW_DIR_REL, '.agentic-plugins/state/founder/workflows');
    strictEqual(ARCHIVE_DIR_REL, '.agentic-plugins/state/founder/archive');
    ...
    ok(!('LEGACY_STATE_DIR_REL' in mod) && !('LEGACY_WORKFLOW_DIR_REL' in mod)
      ...
      'founder state.mjs must not export legacy-home constants (ADR-0036 SD5)');
  });
});
...
describe('state.mjs — archiveWorkflow archiveDirectory-only mode (Codex review M2)', () => {
  ...
  it('locks the same .creation-lock as createWorkflow (Codex re-review M-1)', async () => {
    ...
    // `<repoRoot>/.agentic-plugins/state/founder/.creation-lock`. The previous
    // (incorrect) two-deep dirname produced
    // `<repoRoot>/.agentic-plugins/state/founder/.agentic-plugins/state/founder/.creation-lock`,
    ...
    await withTmpRepo(async (repoRoot) => {
      ...
      const stray = `${repoRoot}/.agentic-plugins/state/founder/.agentic-plugins`;
...
// ============================================================================
// founder trim — the ADR-0019 parent-linkage fields are NOT part of the
// founder schema (orchestrator→founder dispatch is ADR-0036 Non-Goal 3).
...
describe('state.mjs — founder no-parent-linkage contract (ADR-0036 Non-Goal 3)', () => {
  it('treats an on-disk parent_workflow scalar as a forward-compat unknown, not a schema field', () => {
    const text =
      ...
      'verb: "investigate"\n' +
      'profile: ""\n' +
      'original_request: "engineer-shaped file landing on a founder reader"\n' +
    ...
    ok(!('parent_workflow' in frontmatter),
      'parent_workflow must not be a founder schema field');
    ok(!('originating_subtask' in frontmatter),
      'originating_subtask must not be a founder schema field');
  });
  ...
  it('createWorkflow never writes parent-linkage fields (legacy-shaped options are dropped)', async () => {
    await withTmpRepo(async (repoRoot) => {
      const { filePath } = await createWorkflow({
        ...
        // engineer-shaped options — founder's createWorkflow signature
      ...
      const { frontmatter } = await readWorkflow(filePath);
      ok(!('parent_workflow' in frontmatter),
        'parent_workflow must never be written by founder createWorkflow');
      ok(!('originating_subtask' in frontmatter),
        'originating_subtask must never be written by founder createWorkflow');
    });
  });
  ...
  it('CLI create rejects --parent-workflow / --originating-subtask loudly (misdispatch defense)', async () => {
    await withTmpRepo(async (repoRoot) => {
      ...
      match(result.stderr, /ADR-0036 Non-Goal 3/);
    });
  });
  ...
  it('CLI create rejects a non-founder --persona (canonical persona boundary)', async () => {
    await withTmpRepo(async (repoRoot) => {
      const result = spawnSync(process.execPath, [
        STATE_PATH,
        'create',
      ...
      strictEqual(result.status, 1, `expected exit 1, got ${result.status}: ${result.stderr}`);
      match(result.stderr, /only writes persona 'founder'/);
    });
  });
  ...
  it('createWorkflow without parent linkage omits both fields', async () => {
    await withTmpRepo(async (repoRoot) => {
      await createWorkflow({
        ...
        host: 'claude',
        gitBaseline: MIN_BASELINE,
        originalRequest: 'plain founder workflow',
      });
      const [filePath] = await listWorkflowFiles(repoRoot);
      const { frontmatter } = await readWorkflow(filePath);
      ok(!('parent_workflow' in frontmatter));
...
describe('state.mjs — ADR-0020 workflow_type field (schema 1.1 additive)', () => {
  ...
  it('FRONTMATTER_KEY_ORDER places workflow_type after the ADR-0017 1.1 group (founder trim — no parent-linkage keys in between)', async () => {
    await withTmpRepo(async (repoRoot) => {
      await createWorkflow({
        repoRoot,
        verb: 'compose',
      ...
      ok(!raw.includes('\nparent_workflow:') && !raw.includes('\noriginating_subtask:'),
        'no parent-linkage keys may serialize in founder files (ADR-0036 Non-Goal 3)');
    });
  });
});
