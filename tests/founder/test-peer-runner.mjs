// plugins/founder/scripts/peer-runner.mjs unit tests (ADR-0023 PR-B/PR-C).
//
// Covers the founder peer-runner primitive without invoking real peer CLIs:
...
//
// Run via `node --test tests/founder/test-peer-runner.mjs`.
...
const PEER_RUNNER_PATH = resolve(REPO_ROOT, 'plugins/founder/scripts/peer-runner.mjs');
...
process.env.AGENTIC_RUNTIME_ROOT = join(
  tmpdir(),
  `agentic-founder-peer-runner-tests-no-runtime-${process.pid}`,
);

async function withTmpRepo(fn) {
  const dir = await mkdtemp(join(tmpdir(), 'founder-peer-runner-test-'));
  try {
    await fn(dir);
  } finally {
...
describe('peer-runner.mjs — run ledger and handle schema', () => {
  ...
  it('run always writes peer-run ledgers to the canonical founder home, even with a legacy-shaped dir present (founder trim)', async () => {
    await withTmpRepo(async (repoRoot) => {
      const companionsRoot = await writeFakeCompanions(repoRoot);
      // A stray legacy-shaped home must neither attract the ledger nor
      // block the run (founder is canonical-only per ADR-0036 SD5).
      const strayDir = join(repoRoot, '.claude', 'agentic-founder', 'peer-runs', 'stray-run');
      ...
      strictEqual(await exists(canonicalPaths.handle), true,
        'ledger must land in .agentic-plugins/state/founder/peer-runs');
      ok(canonicalPaths.handle.includes('/.agentic-plugins/state/founder/peer-runs/'));
  ...
  it('peerRunsDir rejects a legacy home request (canonical-only contract)', () => {
    ...
    strictEqual(threw, true, 'home=legacy must be an unknown home for founder');
  });
});
...
describe('peer-runner.mjs — sweep and retention', () => {
  it('sweep lets envelope.json win over stale running status and surfaces completed_uncommitted', async () => {
    await withTmpRepo(async (repoRoot) => {
      const workflowPath = join(repoRoot, '.agentic-plugins/state/founder/workflows/wf.md');
...
describe('peer-runner.mjs — ADR-0040 §5 peer-run terminal self-sensor', () => {
  ...
  // Stub runtime honoring the discover-runtime env-override gate: founder's
...
`, 'utf8');
  ...
  it('emits a canonical peer-run-terminal event on runPeer completion', async () => {
    await withTmpRepo(async (repoRoot) => {
      ...
      strictEqual(event.source, 'peer-runner-founder');
